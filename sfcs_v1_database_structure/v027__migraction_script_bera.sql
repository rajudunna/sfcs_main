...
ALTER TABLE `bai_pro2`.`hourly_downtime` 
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`embellishment_plan_dashboard`(
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`excess_cuts_log`(
	`id` int(11) NULL  , 
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`m3_transactions`(
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`mo_details`(
	`id` int(11) NOT NULL  auto_increment , 
	`date_time` datetime NULL  , 
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`mo_operation_quantites`(
	`id` int(11) NOT NULL  auto_increment , 
	`date_time` datetime NULL  , 
	`mo_no` varchar(255) COLLATE latin1_swedish_ci NULL  , 
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`module_master`(
	`id` int(11) NOT NULL  auto_increment , 
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`order_plan_schedule_level`(
...
ALTER TABLE `bai_pro3`.`pac_stat_log_input_job` 
	...
	/* Alter table in target */
ALTER TABLE `bai_pro3`.`plan_dashboard_deleted` 
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`sections_master`(
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`sewing_jobs_ref`(
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`shipment_plan_schedule_level`(
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`tbl_emb_table`(
...
/* Create table in target */
CREATE TABLE `bai_pro3`.`work_stations_mapping`(
	`id` int(11) NOT NULL  auto_increment, 
...
DELIMITER $$
DROP FUNCTION IF EXISTS `bai_pro3`.`fn_act_ship_qty`$$
CREATE FUNCTION `bai_pro3`.`fn_act_ship_qty`(in_sch_no BIGINT) RETURNS int(11)
BEGIN
	DECLARE var_name INT;
	  SET var_name = 0;
...
DELIMITER $$
DROP FUNCTION IF EXISTS `bai_pro3`.`fn_buyer_division_sch`$$
CREATE FUNCTION `bai_pro3`.`fn_buyer_division_sch`(in_schedule VARCHAR(100)) RETURNS varchar(100) CHARSET latin1
BEGIN
...
DELIMITER $$
DROP FUNCTION IF EXISTS `bai_pro3`.`fn_ims_log_bk_output`$$
CREATE FUNCTION `bai_pro3`.`fn_ims_log_bk_output`(in_ims_doc_no BIGINT,in_ims_size VARCHAR(100)) RETURNS int(11)
BEGIN
	DECLARE var_name INT;
	  SET var_name = 0;
	   SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ;
...
DELIMITER $$
DROP FUNCTION IF EXISTS `bai_pro3`.`fn_ims_log_output`$$
CREATE FUNCTION `bai_pro3`.`fn_ims_log_output`(in_ims_doc_no BIGINT,in_ims_size VARCHAR(100)) RETURNS int(11)
BEGIN
	DECLARE var_name INT;
	  SET var_name = 0;
	  SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ;
...
DELIMITER $$
DROP FUNCTION IF EXISTS `bai_pro3`.`stripSpeciaChars`$$
CREATE FUNCTION `bai_pro3`.`stripSpeciaChars`(`dirty_string` VARCHAR(2048),allow_space TINYINT,allow_number TINYINT,allow_alphabets TINYINT,no_trim TINYINT) RETURNS varchar(2048) CHARSET utf8
BEGIN
/**
...
/* Alter table in target */
ALTER TABLE `bai_rm_pj1`.`inspection_supplier_db` 
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `bai3_finishing`.`sync_fg_to_m3`$$
CREATE PROCEDURE `bai3_finishing`.`sync_fg_to_m3`()
BEGIN
...
/* Create table in target */
CREATE TABLE `brandix_bts`.`default_operation_workflow`(
...
/* Create table in target */
CREATE TABLE `brandix_bts`.`ops_short_cuts`(
	`id` int(11) NOT NULL  auto_increment , 
...
/* Create table in target */
CREATE TABLE `brandix_bts`.`parent_work_center_id`(
	`id` int(11) NOT NULL  auto_increment , 
...
CREATE
    TRIGGER `brandix_bts`.`bts_sfcs_del` BEFORE DELETE ON `bundle_transactions_20_repeat` 
    FOR EACH ROW BEGIN
	IF OLD.operation_id=1 OR OLD.operation_id=3 OR OLD.operation_id=4 OR OLD.operation_id=6 OR OLD.operation_id=7 THEN
		INSERT INTO `brandix_bts`.`bts_to_sfcs_sync`(sync_bundle_id,sync_operation_id,sync_operation_code,sync_rep_id,insert_time_stamp) VALUES (OLD.bundle_id,OLD.operation_id,2,OLD.id,NOW());
...
CREATE
    TRIGGER `brandix_bts`.`bts_sfcs_insert` AFTER INSERT ON `bundle_transactions_20_repeat` 
    FOR EACH ROW BEGIN
...
CREATE
    TRIGGER `brandix_bts`.`update_act_module` BEFORE INSERT ON `bundle_transactions_20_repeat` 
    FOR EACH ROW BEGIN
...
CREATE
    TRIGGER `brandix_bts`.`update_rejections` BEFORE UPDATE ON `bundle_transactions_20_repeat` 
    FOR EACH ROW BEGIN
	...
	--	END IF;
		...
		IF OLD.operation_id=3 OR OLD.operation_id=4 OR OLD.operation_id=6 OR OLD.operation_id=7 THEN
			INSERT INTO `brandix_bts`.`bts_to_sfcs_sync`(sync_bundle_id,sync_operation_id,sync_operation_code,sync_rep_id,insert_time_stamp) VALUES (OLD.bundle_id,OLD.operation_id,1,OLD.id,NOW());
		END IF;
	 END IF;
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `brandix_bts_uat`.`sp_snap_view_uat`$$
CREATE PROCEDURE `brandix_bts_uat`.`sp_snap_view_uat`()
BEGIN
...
DELIMITER $$
DROP FUNCTION IF EXISTS `brandix_bts_uat`.`fn_bai_mini_cumulative`$$
CREATE FUNCTION `brandix_bts_uat`.`fn_bai_mini_cumulative`(in_order_id VARCHAR(300),in_mini_order_num INT) RETURNS int(11)
BEGIN
DECLARE retval INT;
...
DELIMITER $$
DROP FUNCTION IF EXISTS `brandix_bts_uat`.`fn_bai_pro3_sch_details`$$
CREATE FUNCTION `brandix_bts_uat`.`fn_bai_pro3_sch_details`(scheduleno INT,rettype VARCHAR(30)) RETURNS varchar(50) CHARSET latin1
BEGIN
...
DELIMITER $$
DROP FUNCTION IF EXISTS `brandix_bts_uat`.`fn_bai_pro3_smv_details`$$
CREATE FUNCTION `brandix_bts_uat`.`fn_bai_pro3_smv_details`(scheduleno INT,color VARCHAR(100)) RETURNS varchar(50) CHARSET latin1
BEGIN
...
DELIMITER $$
DROP FUNCTION IF EXISTS `brandix_bts_uat`.`fn_get_sec`$$
...
DELIMITER $$
DROP FUNCTION IF EXISTS `brandix_bts_uat`.`fn_know_size_title`$$
CREATE FUNCTION `brandix_bts_uat`.`fn_know_size_title`(order_id INT,color_code VARCHAR(300),size_id INT) RETURNS varchar(50) CHARSET latin1
BEGIN
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `central_administration_sfcs`.`list_function`$$
CREATE PROCEDURE `central_administration_sfcs`.`list_function`()
begin
DECLARE v_fn_id int;
...
DELIMITER $$
DROP PROCEDURE IF EXISTS `central_administration_sfcs`.`test1`$$
CREATE PROCEDURE `central_administration_sfcs`.`test1`()
begin
declare done int default false;
...
DELIMITER $$
DROP FUNCTION IF EXISTS `central_administration_sfcs`.`fun_emp_mod`$$
CREATE FUNCTION `central_administration_sfcs`.`fun_emp_mod`(`id` VARCHAR(100)) RETURNS varchar(200) CHARSET latin1
    DETERMINISTIC
BEGIN
...
/* Create table in target */
CREATE TABLE `m3_inputs`.`mo_details`(
	`id` int(11) NOT NULL  auto_increment , 
...
/* Alter table in target */
ALTER TABLE `m3_inputs`.`order_details` 
...
/* Create table in target */
CREATE TABLE `m3_inputs`.`order_details_original`(
...
/* Create table in target */
CREATE TABLE `m3_inputs`.`order_details_temp`(
...
/* Create table in target */
CREATE TABLE `m3_inputs`.`shipment_plan_original`(
	`sno` bigint(20) NOT NULL  auto_increment , 
...
/* Create table in target */
CREATE TABLE `m3_inputs`.`shipment_plan_temp`(
	`sno` bigint(20) NOT NULL  , 
	`Customer_Order_No` longtext COLLATE latin1_swedish_ci NULL  , 
	`CO_Line_Status` longtext COLLATE latin1_swedish_ci NULL  , 
	`Ex_Factory` longtext COLLATE latin1_swedish_ci NULL  , 
