...
CREATE TABLE `cwh_to_rmwh_temp` (
  ...
  PRIMARY KEY (`id`),
  KEY `schedule` (`schedule`)
) ENGINE=InnoDB AUTO_INCREMENT=180933 DEFAULT CHARSET=latin1;
...
CREATE TABLE `m3_fab_issue_track` (
  ...
  PRIMARY KEY (`tran_id`),
  KEY `doc_ref` (`doc_ref`)
) ENGINE=MyISAM AUTO_INCREMENT=62955 DEFAULT CHARSET=latin1;
...
CREATE TABLE `store_returns` (
  ...
  `log_stamp` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`tid`)
) ENGINE=MyISAM AUTO_INCREMENT=134518 DEFAULT CHARSET=latin1;
...
/*!50003 CREATE FUNCTION `fn_roll_id_fab_scan_status`(doc_id BIGINT,doc_type VARCHAR(15), roll_id BIGINT) RETURNS int(11)
BEGIN
