	...
	$.ajax({
			...
			{	
                ...
                if(response['status']){
					...
					document.getElementById('mapped_color').value = color;
                    ...
                    for(var i=0;i<data.length;i++){
                        ...
                        var readonly ='';
                        var temp_var_bal = data[i].send_qty-(data[i].reported_qty+data[i].rejected_qty);
                        s_no++;
...
function check_pack()
{
	...
	else
	{
		...
		var function_text = "<?php echo getFullURL($_GET['r'],'emb_scanning_process.php','R'); ?>";
