...
<script> 
    ...
    function firstbox() 
    { 
        window.location.href ="<?= getFullURLLevel($_GET['r'],'schedule_mix_bek_delete.php',0,'N'); ?>&style="+encodeURIComponent(window.btoa(document.test.style.value)) 
    } 

    function secondbox() 
    { 
        window.location.href ="<?= getFullURLLevel($_GET['r'],'schedule_mix_bek_delete.php',0,'N'); ?>&style="+encodeURIComponent(window.btoa(document.test.style.value))+"&schedule="+document.test.schedule.value 
    } 

    function thirdbox() 
    { 
        window.location.href ="<?= getFullURLLevel($_GET['r'],'schedule_mix_bek_delete.php',0,'N'); ?>&style="+encodeURIComponent(window.btoa(document.test.style.value))+"&schedule="+document.test.schedule.value+"&color="+encodeURIComponent(window.btoa(document.test.color.value)) 
    } 
    </script> 
...
{ 
	...
	if($cut_status==0 && $plan_status==0)
	{
		...
		{     
			...
			//echo $sql4533."<br>"; 
			$sql_result4533=mysqli_query($link, $sql4533) or exit("Sql Error111"); 
			...
			// echo $sql4529."<br>"; 
			$sql_result452291=mysqli_query($link, $sql452291) or exit("Sql Error124"); 
			...
			// echo $sql451."<br>"; 
			$sql_result451=mysqli_query($link, $sql451) or exit("Sql Error125"); 
			
			$sql45132="DELETE from $bai_pro3.bai_orders_db_club where order_tid in ('".implode("','",$order_tids)."')"; 
			// echo $sql451."<br>"; 
			$sql_result451=mysqli_query($link, $sql45132) or exit("Sql Error125"); 
			
			$sql4521="DELETE from $bai_pro3.bai_orders_db_club_confirm where order_tid in ('".implode("','",$order_tids)."')"; 
			// echo $sql451."<br>"; 
			$sql_result451=mysqli_query($link, $sql4521) or exit("Sql Error126");
			...
			// echo $sql452."<br>"; 
			$sql_result452=mysqli_query($link, $sql452) or exit("Sql Error127"); 
			 
			echo "<script type=\"text/javascript\">  
					sweetAlert('Successfully Removed the Clubbed Colour','','success'); 
					setTimeout(openUrl, 1500);
					function openUrl(){
						 window.location.href = '".getFullURLLevel($_GET['r'],'schedule_mix_bek_delete.php',0,'N')."';
					}
				</script>";     
		} 
		else 
