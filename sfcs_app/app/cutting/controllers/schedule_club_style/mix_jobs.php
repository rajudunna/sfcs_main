...
if(isset($_POST['submit']) && short_shipment_status($_POST['style'],$_POST['schedule'],$link))
{
	...
	$sql212="select * from $bai_pro3.mix_temp_desti";
	$sql_result212=mysqli_query($link, $sql212) or exit("Sql Error4".mysqli_error($GLOBALS["___mysqli_ston"]));
	$result=mysqli_num_rows($sql_result212);
	if($result==0)
	{	
		if(sizeof($ready_cat_ref)>0 &&(sizeof($cat_id_ref)==sizeof($ready_cat_ref))) 	
		{
			...
			for($l=0;$l<sizeof($ready_cat_ref);$l++)
			{
				...
				{				
					...
					{ 
						...
						{ 
							...
							{ 
								...
								{ 
									$sql7="insert into $bai_pro3.mix_temp_source (doc_no,cat_ref,cutt_ref,mk_ref,size,qty,plies,cutno,ratio) values ($doc_no,$cat_ref,$cut_ref,$mk_ref,'".$sizes_array[$ii]."',".$qts[$sizes_array[$ii]].",".$plies.",".$cutno.",".$ratio.")"; 
							...
							{ 
								...
								{ 
									$sql71="insert into $bai_pro3.mix_temp_source (doc_no,cat_ref,cutt_ref,mk_ref,size,qty,plies,cutno,type,ratio) values ($doc_no,$cat_ref,$cut_ref,$mk_ref,'".str_replace("p_","",$qts_ex_size[$sizes_array[$kk]])."',".$qts_ex_val[$sizes_array[$kk]].",".$plies.",".$cutno.",1,".$ratio.")";
							...
							{								
								...
								if(mysqli_num_rows($sql_result114)>0)
								{
									...
									{ 
										...
										$ratio=$sql_row11['ratio'];
										if($req_qty>$available)
										{
											$update_value=0;
											if($req_qty > 0)
											{
												$sqlx3="insert into $bai_pro3.mix_temp_desti(allo_new_ref,cat_ref,cutt_ref,mk_ref,size,qty,order_tid,order_del_no,order_col_des,destination,plies,doc_no,cutno,ratio) values ($doc_no,$cat_ref,$cut_ref,$mk_ref,'".$sizes_array[$i]."',$available,'".$order_tid1."','".$order_del_no."','".$order_col."','".$destination_id_new."',$plies_ref,$doc_no,$cutno,$ratio)"; 
										...
										else
										{
											...
											if($req_qty > 0)
											{
												$sqlx3="insert into $bai_pro3.mix_temp_desti(allo_new_ref,cat_ref,cutt_ref,mk_ref,size,qty,order_tid,order_del_no,order_col_des,destination,plies,doc_no,cutno,ratio) values ($doc_no,$cat_ref,$cut_ref,$mk_ref,'".$sizes_array[$i]."',$req_qty,'".$order_tid1."','".$order_del_no."','".$order_col."','".$destination_id_new."',$plies_ref,$doc_no,$cutno,$ratio)";
												mysqli_query($link, $sqlx3) or exit(message_sql());
						...
						{ 
							...
							if(mysqli_num_rows($sql_result114)>0)
							{
								while($sql_row11=mysqli_fetch_array($sql_result114)) 
								{
									...
									$ratio[$sql_row11['doc_no']][$sizes_array[$i]]=$sql_row11['ratio'];
								...
								if($eligble>0)
								{
									for($jj=0;$jj<sizeof($docs);$jj++)
									{
										...
										{	
											...
											{
												$sqlx3="insert into $bai_pro3.mix_temp_desti(allo_new_ref,cat_ref,cutt_ref,mk_ref,size,qty,order_tid,order_col_des,order_del_no,destination,plies,doc_no,cutno,type,ratio) values (".$docs[$jj].",".$cat_ref.",".$cut_ref[$docs[$jj]][$sizes_array[$i]].",".$mk_ref[$docs[$jj]][$sizes_array[$i]].",'".$sizes_array[$i]."',".$eligble.",'".$order_tids[$dels[$j]]."','".$colrsnew[$dels[$j]]."','".$dels[$j]."','".$destination_id_new[$dels[$j]]."',".$plies_ref[$docs[$jj]][$sizes_array[$i]].",".$docs[$jj].",".$cutno[$docs[$jj]][$sizes_array[$i]].",1,".$ratio[$docs[$jj]][$sizes_array[$i]].")"; 
												mysqli_query( $link, $sqlx3) or exit(message_sql());	
											...
											{
												$sqlx3="insert into $bai_pro3.mix_temp_desti(allo_new_ref,cat_ref,cutt_ref,mk_ref,size,qty,order_tid,order_col_des,order_del_no,destination,plies,doc_no,cutno,type,ratio) values (".$docs[$jj].",".$cat_ref.",".$cut_ref[$docs[$jj]][$sizes_array[$i]].",".$mk_ref[$docs[$jj]][$sizes_array[$i]].",'".$sizes_array[$i]."',".$available[$docs[$jj]][$sizes_array[$i]].",'".$order_tids[$dels[$j]]."','".$colrsnew[$dels[$j]]."','".$dels[$j]."','".$destination_id_new[$dels[$j]]."',".$plies_ref[$docs[$jj]][$sizes_array[$i]].",".$docs[$jj].",".$cutno[$docs[$jj]][$sizes_array[$i]].",1,".$ratio[$docs[$jj]][$sizes_array[$i]].")"; 
												mysqli_query( $link, $sqlx3) or exit(message_sql());	
												$sqlx71="update $bai_pro3.mix_temp_source set qty=0 where doc_no=".$docs[$jj]." and size='".$sizes_array[$i]."' and type=1";
												mysqli_query($link, $sqlx71) or exit(message_sql());
							...
							for($jj=0;$jj<sizeof($docs);$jj++)
							{
								...
								{	
									$sqlx31="insert into $bai_pro3.mix_temp_desti(allo_new_ref,cat_ref,cutt_ref,mk_ref,size,qty,order_tid,order_col_des,order_del_no,destination,plies,doc_no,cutno,type,ratio) values (".$docs[$jj].",".$cat_ref.",".$cut_ref[$docs[$jj]][$sizes_array[$i]].",".$mk_ref[$docs[$jj]][$sizes_array[$i]].",'".$sizes_array[$i]."',".$available[$docs[$jj]][$sizes_array[$i]].",'".$order_tids[$max_del]."','".$colrsnew[$max_del]."','".$max_del."','".$destination_id_new[$max_del]."',".$plies_ref[$docs[$jj]][$sizes_array[$i]].",".$docs[$jj].",".$cutno[$docs[$jj]][$sizes_array[$i]].",1,".$ratio[$docs[$jj]][$sizes_array[$i]].")"; 
								...
								while($sql_row472=mysqli_fetch_array($sql_result472)) 
								{
									...
									while($sql_row1x12=mysqli_fetch_array($sql_result12)) 
									{
										if($qty>0)
										{
											...
											{
												$insert_sql="INSERT INTO $bai_pro3.`mix_temp_desti` (`allo_new_ref`, `cat_ref`, `cutt_ref`, `mk_ref`, `size`, `qty`, `order_tid`, `order_del_no`, `order_col_des`, `destination`, `plies`, `doc_no`, `cutno`,type,ratio) select allo_new_ref,cat_ref,cutt_ref,mk_ref,size,".$qty.",'".$sql_row471['order_tid']."','".$sql_row471['order_del_no']."','".$sql_row471['order_col_des']."',destination,plies,doc_no,cutno,type,ratio from $bai_pro3.`mix_temp_desti` where `mix_tid` = ".$sql_row1x12['mix_tid']."";
					...
					*/
					...
					$sql12="SELECT cutt_ref,order_del_no,order_col_des,order_tid,GROUP_CONCAT(distinct doc_no) as docs,ratio FROM $bai_pro3.`mix_temp_desti` where cat_ref=".$cat_ref." GROUP BY order_tid,ratio order by order_tid*1"; 
			...
			$sql212="truncate $bai_pro3.mix_temp_desti"; 
			mysqli_query( $link, $sql212) or exit(message_sql()); 

			$sql3212="truncate $bai_pro3.mix_temp_source"; 
			mysqli_query( $link, $sql3212) or exit(message_sql()); 		
	...
	else
	{
		...
		echo " <div class='alert alert-info alert-dismissible'>Splitting is doing by another user please wait sometime and try again.<br></div>";
	}
	mysqli_commit($link);
}
	catch(Exception $e)
