...
if(isset($_POST['fix']))
{
	...
	if(sizeof($selected)>1)
	{
		...
		{			
			...
			$sql_check3="select order_del_no,order_col_des from $bai_pro3.bai_orders_db_club_confirm where order_del_no in (".implode(",",$schedule_array).") and order_col_des=\"".$color."\"";
			$sql_check_res3=mysqli_query($link, $sql_check3) or exit("Sql Error113".mysqli_error($GLOBALS["___mysqli_ston"]));
