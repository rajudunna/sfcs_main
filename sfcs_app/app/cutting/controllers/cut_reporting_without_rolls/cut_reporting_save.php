...
if($old_logic == 1){
    ...
    if($target == 'schedule_clubbed' || $target == 'style_clubbed'){
        ...
        $exists_sizes='';
        $parent_qty_sizewise=[];
        $parent_docs_query = "SELECT $a_sizes_str from $bai_pro3.plandoc_stat_log psl LEFT JOIN bai_pro3.cat_stat_log csl ON csl.tid = psl.cat_ref where doc_no = $doc_no and category IN ($in_categories)";
        $parent_docs_result = mysqli_query($link,$parent_docs_query);
        while($parent_row = mysqli_fetch_array($parent_docs_result)){
            foreach($sizes_array as $size){
                if($parent_row['a_'.$size]>0) {
                    $exists_sizes .= 'a_'.$size.',';
                    $parent_qty_sizewise['a_'.$size] = $parent_row['a_'.$size] * $reporting_plies;
                }
            }
        }
        $exists_sizes = rtrim($exists_sizes,',');
        $existed_sizes=explode(',',$exists_sizes);
        $child_docsforsorting=[];
        $child_sizes_array=[];
        $child_docs_query = "SELECT doc_no,$exists_sizes from $bai_pro3.plandoc_stat_log psl LEFT JOIN bai_pro3.cat_stat_log csl ON csl.tid = psl.cat_ref where org_doc_no = $doc_no and category IN ($in_categories)";
        $child_docs_result = mysqli_query($link,$child_docs_query);

        while($row = mysqli_fetch_array($child_docs_result)){
            $child_docs[] = $row['doc_no'];
            foreach($existed_sizes as $size){
                if($row[$size]>0) {
                    $child_sizes_array[$row['doc_no']][$size]=$row[$size];
                }
            }
            $child_docsforsorting[$row['doc_no']]=sizeof($child_sizes_array[$row['doc_no']]);
        }
        arsort($child_docsforsorting);
        foreach($child_docsforsorting as $child_doc =>$value) {
            $childDocketReportingPlies = 0;
            $pending_plies=0;
            if(array_sum($parent_qty_sizewise) >0){
                $size_qty_query = "SELECT p_plies,a_plies,act_cut_status,$exists_sizes from $bai_pro3.plandoc_stat_log  where doc_no = $child_doc ";        
                $sizes_qty_result = mysqli_query($link,$size_qty_query); 
                while($row = mysqli_fetch_array($sizes_qty_result)){
                    $eligible=true;
                    ...
                    if($pending_plies >0){
                        $eligible=true;
                        $new_array=[];          
                        foreach($child_sizes_array[$child_doc] as $size => $child_ratio){
                            if($parent_qty_sizewise[$size] <= 0){
                                $eligible=false;
                                break;
                            }else{
                                $new_array[$child_doc][$size]=$parent_qty_sizewise[$size]/$child_ratio;
                            }
                        }
                        $min = min($new_array[$child_doc]);
                        $index = array_search($min, $new_array[$child_doc]); 
                        $eligibletoreport=floor($new_array[$child_doc][$index]);
                        $childDocketReportingPlies =  min($pending_plies, $eligibletoreport);  
                        if($eligible && $pending_plies>0)
                        {
                            $update_childs_query = "UPDATE $bai_pro3.plandoc_stat_log set act_cut_status = 'DONE',a_plies = IF(a_plies = p_plies,$childDocketReportingPlies,a_plies+$childDocketReportingPlies),fabric_status=5 $update_manual_flag where doc_no = $child_doc ";							
                            ...
                            foreach($child_sizes_array[$child_doc] as $key => $val){
                                $parent_qty_sizewise[$key]=$parent_qty_sizewise[$key]-($childDocketReportingPlies*$child_sizes_array[$child_doc][$key]);
                            }
                        }
                        unset($size_update_string);
                        unset($reported);		
