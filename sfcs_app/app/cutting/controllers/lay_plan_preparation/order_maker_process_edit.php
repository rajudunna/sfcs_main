		...
		{	
			...
			mysqli_query($link, $sql) or exit("Sql Error111".mysqli_error($GLOBALS["___mysqli_ston"]));
				...
				for($i=0;$i<sizeof($in_mklength);$i++)
				{
					...
					{
						$sql_check2="select marker_ref_tid from $bai_pro3.marker_ref_matrix where marker_ref_tid='".$iLastid."-".$in_pwidth[$i]."'";
						$sql_check_res2=mysqli_query($link, $sql_check2) or exit("Sql Error112122".mysqli_error($GLOBALS["___mysqli_ston"]));
