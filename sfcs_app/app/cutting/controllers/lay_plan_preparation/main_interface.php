...
foreach($val_schedules as $schedule){
	...
	{
		$sql_excess_cut="select excess_cut_qty from $bai_pro3.excess_cuts_log where schedule_no = '$schedule' 
					and color = '$color'";
		$result3 = mysqli_query($link,$sql_excess_cut) or exit("Unable to get the color codes");
		while($row = mysqli_fetch_array($result3))
		{
			$excess_cut = $row['excess_cut_qty'];
		}
			...
			if($sql_num_check==0)
			{
				...
				//To get Encoded Color
			     $main_color = color_encode($color);
					if($sql_row['mo_status']=="Y")
					{
						echo "<td>
								<center>
									<a class='btn btn-info btn-xs' href='".getFullURL($_GET['r'], "order_cat_edit_form.php", "N")."&cat_tid=".$sql_row['tid']."&style=".$style."&schedule=".$schedule."&color=".$main_color."'>Edit</a>
								</center>
							</td>";
					}
					else
				...
				else
				{
					...
					if (mysqli_num_rows($result_details) > 0)
					{
						...
						echo "<td>
								<center>
									<a class='btn btn-success btn-xs' href='".getFullURL($_GET['r'], "main_interface.php", "N")."&color=$main_color&style=$get_style&schedule=$get_schedule'>Go To</a>
								</center>
							</td>";
					}
				}
...
//	echo "<td class=\"  \"><center>".$sql_row['tid']."</center></td>";
	...
	if($color1 == "success"){
	    ...
	    //To get Encoded Color
	    $main_color = color_encode($color);
		echo "<td class=\"  \"><center><a class=\"btn btn-xs btn-info\" href=\"".getFullURL($_GET['r'], "order_cut_form2.php", "N")."&tran_order_tid=$main_tran_order_tid&check_id=$check_id&style=$style&schedule=$schedule&color=$main_color\">Update</a></center></td>";
	}
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	...
	*/
	...
	//Encoding color
	$main_color = color_encode($color);
	echo "<td class=\"  \"><center><a class=\"btn btn-xs btn-info\" href=\"".getFullURL($_GET['r'], "order_allocation_form2.php", "N")."&tran_order_tid=$main_tran_order_tid&check_id=$cuttable_ref&cat_id=$cat_id&total_cuttable_qty=$total_cuttable_qty&style=$style&schedule=$schedule&color=$main_color\">Add Ratios</a></center></td>";
	...
	$pliespercut=0;
	$sql15="select * from bai_pro3.allocate_stat_log where cat_ref=$cat_id and recut_lay_plan='no'";
	// echo $sql15;
...
foreach($cats_ids as $key=>$value)
{
	...
	{		
	$tot_size=array();
	$sql="select * from $bai_pro3.allocate_stat_log where order_tid=\"$tran_order_tid\" and cat_ref=$value and recut_lay_plan='no' order by tid";
	mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
	// echo $sql;
	$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
    while($sql_row=mysqli_fetch_array($sql_result))
		...
		<div id="excess_cut" class="panel-collapse collapse-in collapse in" aria-expanded="true">
			...
			<?php
				...
				if($check=='1'){
					...
					echo "<input type='submit' name='submit' value='Update' class='btn btn-warning btn-sm editor_edit'>";
				}
				echo "</form>";
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	...
	{
		echo "<td><center><span class='label label-danger'><i class=\"fa fa-times-circle\"></i>&nbsp;&nbsp; Update Excess Cut</span></center></td>";
	}
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	...
	// echo $sql2;
	$sql_result2=mysqli_query($link, $sql2) or exit("Sql Error44".mysqli_error($GLOBALS["___mysqli_ston"]));
	while($sql_row2=mysqli_fetch_array($sql_result2))
	{
	...
	$path3="".getFullURL($_GET['r'], "Book2_pdf.php", "R")."?order_tid=$main_tran_order_tid&cat_ref=$cat_tid_new&cat_title=$category_new&clubbing=$clubbing&color=$color&schedule=$schedule&style=$style";
