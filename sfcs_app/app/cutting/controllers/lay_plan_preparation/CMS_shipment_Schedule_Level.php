...
<?php
	...
	$sql="select distinct style_no from $bai_pro3.shipment_plan_schedule_level";
	$sql_result=mysqli_query($link, $sql) or exit("Sql Error4".mysqli_error($GLOBALS["___mysqli_ston"]));
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		$style=str_pad($sql_row['style_no'],"15"," ");
		
		$sql1="select distinct schedule_no from $bai_pro3.shipment_plan_schedule_level where style_no=\"$style\" ";
		...
		while($sql_row1=mysqli_fetch_array($sql_result1))
		{
			...
			$sql2="select distinct color from $bai_pro3.shipment_plan_schedule_level where schedule_no=\"$sch_no\" and style_no=\"$style\"";
			...
			while($sql_row2=mysqli_fetch_array($sql_result2))
			{
				...
				$sql_check="select movex_style,style_id,buyer_id from $bai_pro2.movex_styles where movex_style=\"".$style."\" and style_id=\"".$style_id."\" and buyer_id=\"".$buyer_id_new."\"";
				...
				$sql22="select distinct size_code from $bai_pro3.shipment_plan_schedule_level where color=\"$color\" and schedule_no=\"$sch_no\" and style_no=\"$style\" ORDER BY CONVERT(bai_pro3.stripSpeciaChars(size_code,0,1,0,0) USING utf8)*1,FIELD(size_code,'xs','s','m','l','xl','xxl','xxxl')";
				...
				while($sql_row22=mysqli_fetch_array($sql_result22))
				{
					...
					// if($order_qty>=0)
					{
						$sql_check3="select order_tid from $bai_pro3.bai_orders_db where order_tid=\"$ssc_code\"";
