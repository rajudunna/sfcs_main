...
if(isset($_POST['update']))
{
	...
	$allocate_ref=$_POST['allocate_ref'];
		...
		{
			$sql1111="select * from $bai_pro3.maker_stat_log where order_tid=\"$tran_order_tid\" and allocate_ref=\"$allocate_ref\"";
			...
			// echo $sql;die();
				...
				for($i=0; $i<$row_count; $i++)
				{
					...
					if($in_skgrp != '' && $in_width != '' && $in_mklen && $in_mkver != '') {
						...
						$sql1112="select parent_id from $bai_pro3.maker_details where parent_id=".$allocate_ref."";
				...
				for($i=0;$i<sizeof($in_mklength);$i++)
				{
					...
					{
						$sql1113="select marker_ref_tid from $bai_pro3.marker_ref_matrix where marker_ref_tid='".$iLastid."-".$in_pwidth[$i]."'";
