...
if(isset($_POST['update']))
{
	...
	$allocate_ref=$_POST['allocate_ref'];
		...
		{
				$purl = getFullURLLevel($_GET['r'], "main_interface.php", "0", "N")."&color=".$color."&style=".$style."&schedule=".$schedule;
				echo "<script type=\"text/javascript\"> 
		...
		if(strlen(trim($in_mkver))>=1) //System will not update, if no data is not available
		{
			...
			//echo $sql;
				...
				$aurl = getFullURLLevel($_GET['r'], "main_interface.php", "0", "N")."&color=".$color."&style=".$style."&schedule=".$schedule;
			echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",0); function Redirect() {   sweetAlert('Successfully Updated','','success');
				Ajaxify('".$aurl."'); }</script>";
		}else{
			//echo "<h2 class='label label-danger'>Marker Version is not available.</h2>";
			$elurl = getFullURL($_GET['r'], "order_makers_form2.php", "N")."&tran_order_tid=".$tran_order_tid."&cat_ref=".$cat_ref."&cuttable_ref=".$cuttable_ref."&allocate_ref=".$allocate_ref;
			echo "<script type='text/javascript'>
