...
$sql="select min(roll_width) as width from bai_rm_pj1.fabric_cad_allocation where doc_no in (".implode(",",$docs).") and doc_type=\"normal\"";
 //echo $sql;
	$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
	while($sql_row1x=mysql_fetch_array($sql_result))
	{
...
if(mysql_num_rows($sql_result22)==0)
{
	$purlength=$mklength;
}
?>
...
<table border=0 cellpadding=0 cellspacing=0 width=1063 style='border-collapse:
 ...
 <tr height=25 style='height:18.75pt'>
  ...
  <td colspan=6 rowspan=3 class=xl8217319x valign="top" align="left"><img src="bai_logo.jpg" width="120" height="60"></td>
...
$ctex_width=array();
$tkt_width=array();
$sql="select distinct * from bai_rm_pj1.docket_ref where doc_no in (".implode(",",$docs).") and doc_type='normal'  group by roll_id";
$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
while($sql_row=mysql_fetch_array($sql_result))
{
...
<?php
	...
	$sql="update plandoc_stat_log set print_status=\"".date("Y-m-d")."\" where doc_no in (".implode(",",$docs).") and print_status is null";
	mysql_query($sql,$link) or exit("Sql Error".mysql_error());
