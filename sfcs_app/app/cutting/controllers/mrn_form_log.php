			...
			<div class="panel panel-primary panel-body">
				<div class="col-md-12">
					<?php
						...
						$total=0;
						$sql="select status,count(rand_track_id) as count from $wms.mrn_track where month(req_date)=month(\"$date\") and year(req_date)=year(\"$date\") and plant_code='".$plant_code."' group by status";
						...
						*/
						...
						while($sql_row=mysqli_fetch_array($sql_result))
						{
							...
							echo "<td><a class='btn btn-info btn-xs' href=\"$link_remarks&ref_tid=$ref_tid\" onclick=\"return popituprem('$link_remarks&ref_tid=$ref_tid')\">Remarks</a></td>";
								$sql_ref_tid="select * from $wms.remarks_log where tid=\"$ref_tid\" and level=\"Approved\" and plant_code='".$plant_code."'";
