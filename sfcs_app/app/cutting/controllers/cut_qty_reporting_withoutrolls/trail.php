...
$doc_no_refs = $_GET['doc_no_ref'];
$go_back_to = $_GET['go_back_to'];
$bundle_no = array();
$op_code = '15';
$cut_done_qty = array();

$doc_no_refs = explode(',',$doc_no_refs);

foreach($doc_no_refs as $doc_no_ref){
    ...
    if(mysqli_num_rows($qry_to_find_in_out_result) > 0)
    {
        ...
        foreach ($cut_done_qty as $key => $value)
        {
            ...
            //received_qty_cumulative=$recevied_qty
            $update_qry_cps = "update $bai_pro3.cps_log set remaining_qty = remaining_qty+$value,reported_status='$reported_status' where id = $id_to_update_cps";
