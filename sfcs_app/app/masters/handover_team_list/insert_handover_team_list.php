...
if (empty($emp_id) || empty($emp_call_name) ) 
{
	...
	if($team_id>0){
		//update
		$sql = "update $pps.tbl_fg_crt_handover_team_list set emp_id='$emp_id',emp_call_name='$emp_call_name',emp_status='$emp_status',updated_user='$username',updated_at='".date('Y-m-d')."' where plant_code='$plantcode' and team_id=$team_id";
		//echo $sql;exit;
		...
		echo "<script>setTimeout(function () { 
			...
			$sql = "INSERT INTO $pps.tbl_fg_crt_handover_team_list (emp_id, emp_call_name,selected_user, emp_status,plant_code,created_user,created_at) VALUES('$emp_id','$emp_call_name','$selected_user','$emp_status','$plantcode','$username','".date('Y-m-d')."')";
