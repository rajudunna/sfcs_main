	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			$reason=$row["reason"];
			echo "<tr><td>".$sno++."</td><td>".$row["code"]."</td><td>".$row["rdept"]." </td><td>".$row["reason"]."</td><td><a href='$url==&rowid=$rowid&code=$code&department=$department&reason=$reason' class='btn btn-warning btn-xs editor_edit'>Edit</a> /
			 <a href='$url1&rowid=$rowid&code=$code&department=$department&reason=$reason' class='btn btn-danger btn-xs' onclick='return confirm_delete(event,this);'>Delete</a></td></tr>";
		}
		echo "</tbody></table>";
	} else {
		echo "0 results";
