...
if (empty($code) || empty($department) || empty($reason)) {
	...
	echo"<script>setTimeout(function () { 
		...
		if($dr_id>0)
		{	
			$query1="select code from  $bai_pro2.downtime_reason where code='$code'  and id != $dr_id";
			$sql_result1=mysqli_query($conn,$query1);
			if (mysqli_num_rows($sql_result1)>0){
				$url=getFullURL($_GET['r'],'down_time_reason_add.php','N');
				echo"<script>setTimeout(function () {
					swal({
						title: 'Code already existed for these Department',
						text: 'Error!',
						type: 'warning',
						confirmButtonText: 'OK'
					},
			...
			$query1="select code from $bai_pro2.downtime_reason  where  code='$code' ";
			$sql_result1=mysqli_query($conn, $query1);
			if(mysqli_num_rows($sql_result1)>0){
				$url=getFullURL($_GET['r'],'down_time_reason_add.php','N');
				echo"<script>setTimeout(function () { 
					swal({
					title: 'Code Already Existed',
					text: 'Message!',
					type: 'warning',
					confirmButtonText: 'OK'
					},
				...
				$query1="select reason from $bai_pro2.downtime_reason  where  reason='$reason' ";
				$sql_result1=mysqli_query($conn, $query1);
				if(mysqli_num_rows($sql_result1)>0){
					$url=getFullURL($_GET['r'],'down_time_reason_add.php','N');
					echo"<script>setTimeout(function () { 
