...
if (strlen(trim($emp_id)) == 0 || strlen(trim($emp_name)) == 0) {
	...
	if($tbl_id>0){
		$query = "SELECT emp_id  from $mdm.tbl_leader_name where plant_code='$plant_code' and emp_id='$emp_id' and id != $tbl_id ";
		...
		if(mysqli_num_rows($sql_result)>0){
			...
			$sql = "UPDATE $mdm.tbl_leader_name set emp_id='$emp_id',emp_name='$emp_name',updated_user='$username',updated_at='".date('Y-m-d')."' where plant_code='$plant_code' and id = $tbl_id";
			// echo $sql;die();
		...
		$query = "SELECT emp_id from $mdm.tbl_leader_name where plant_code='$plant_code' and emp_id='$emp_id' ";
		$sql_result=mysqli_query($conn, $query);
		if(mysqli_num_rows($sql_result)>0){
			$url=getFullURL($_GET['r'],'cutting_table_add.php','N');
			echo"<script>setTimeout(function () { 
