...
if (empty($reason_cat) || empty($reason_desc) || empty($reason_code) || empty($reason_order) || empty($form_type) || empty($m3_reason_code) ) 
{
	...
	if($sno>0){
		...
		if($count > 0){
			...
			echo "<script>setTimeout(function () { 
				swal({
					title: 'Reason Order already Existed!',
					text: 'Message!',
					type: 'warning',
					confirmButtonText: 'OK'
				},
			...
			$sql = "update $bai_pro3.bai_qms_rejection_reason set reason_cat='$reason_cat',reason_desc='$reason_desc',
			reason_code='$reason_code',reason_order='$reason_order',form_type='$form_type',m3_reason_code='$m3_reason_code' where sno=$sno";
			//echo $sql;exit;
