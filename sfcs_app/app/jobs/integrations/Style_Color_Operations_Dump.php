...
$sql="SELECT order_style_no,order_col_des FROM $bai_pro3.bai_orders_db GROUP BY order_style_no,order_col_des ORDER BY order_style_no,order_col_des";
...
while($sql_row=mysqli_fetch_array($result))
{
	...
	if(mysqli_num_rows($bundle_creation_data_check_result)==0)
	{
		...
		{		
			$tbl_style_ops_master_insert="insert ignore $brandix_bts.tbl_style_ops_master(parent_id,operation_name,operation_order,style,color, operation_code,default_operration,ops_sequence,barcode) values('".$select_default_operations_row['id']."','".$select_default_operations_row['operation_name']."','".$select_default_operations_row['operation_order']."','".$style."','".$color."','".$select_default_operations_row['operation_code']."','".$select_default_operations_row['default_operration']."','".$select_default_operations_row['ops_sequence']."','".$select_default_operations_row['barcode']."')";
