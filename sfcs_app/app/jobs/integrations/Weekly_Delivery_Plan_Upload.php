...
while($sql_row=mysqli_fetch_array($sql_oms_result))
{
	...
	{
		$sql_insert="INSERT INTO $pps.shipment_plan(created_at,created_user,plant_code,ssc_code_week_plan) value(\"".date('Y-m-d')."\",'".$username."','".$plantcode."','".$Style_No.$Schedule_No.$Colour."-".$Size."-".$Ex_Factory."-".$Destination."')";
	...
	$sql_insert_week="INSERT INTO $pps.week_delivery_plan(shipment_plan_id,size_code,rev_exfactory,original_order_qty,act_exfact,plant_code,created_user,created_at) SELECT ship_tid,size,ex_factory_date,ord_qty,ex_factory_date,plant_code,'".$username."',\"".date('Y-m-d')."\" FROM $pps.shipment_plan where ship_tid not in (select shipment_plan_id from $pps.week_delivery_plan where plant_code='".$plantcode."')";
