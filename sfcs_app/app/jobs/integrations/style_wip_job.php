...
if(mysqli_num_rows($check_data_result) ==0)
{
	...
	$get_temp_data ="INSERT IGNORE INTO $brandix_bts.open_style_wip (style,SCHEDULE,color,size,operation_code,good_qty,rejected_qty,status) 
	SELECT style,SCHEDULE,color,size_title,operation_id,SUM(recevied_qty) AS good_qty,SUM(rejected_qty) AS rejected_qty,'open' 
	FROM $brandix_bts.bundle_creation_data_temp and schedule in (SELECT DISTINCT order_del_no FROM `bai_pro3`.bai_orders_db_confirm where order_stat='') GROUP BY style,SCHEDULE,color,size_title,operation_id";
