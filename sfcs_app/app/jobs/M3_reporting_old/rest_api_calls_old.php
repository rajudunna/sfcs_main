...
class rest_api_calls {
	...
	include($include_path.'\sfcs_app\app\jobs\M3_reporting_old\m3_api_const_old.php');
...
function writing_logs_to_file($unique_id,$url_or_response)
{
	$include_path=getenv('config_job_path');
	include($include_path.'\sfcs_app\app\jobs\M3_reporting_old\m3_api_const_old.php');
	$directory = $include_path.'\sfcs_app\app\m3_old_log_files\\'.$facility_code_old;
	if (!file_exists($directory)) {
		mkdir($directory, 0777, true);
	}
	$date = date("d_m_Y");
	$current_date = date("Y-m-d H:i:s");
	$file_name_string = $facility_code_old.'_'.$date.'_api_log.txt';
	$my_file = $include_path.'\sfcs_app\app\m3_old_log_files\\'.$facility_code_old.'\\'.$file_name_string;
