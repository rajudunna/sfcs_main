...
<script>
	...
	function getCSVData() {
		var data = $('#example').table2CSV({delivery: 'value',filename: 'current_week.csv'});
        downloadFile('current_week.csv', 'data:text/csv;charset=UTF-8,' + encodeURIComponent(data));
...
//if(isset($_POST['filter']) or isset($_POST['filter2']))
	...
	//Process to show yesterday results
	// if(isset($_GET['ystd_process']))
	// {
	// 	$cache_date="ystd_report";
...
$edate=date('Ymd', strtotime($edate));
		...
		$po_number1="'".implode("','",$po_number)."'";
			...
			$pack_operation='200';
			$qrytoGetMinOperation="SELECT operation_code FROM $tms.`task_job_status` WHERE task_jobs_id in ($task_jobs_id1) AND plant_code='$plant_code' AND is_active=1 ORDER BY operation_seq ASC LIMIT 0,1";
         ...
         */
		$qrytoGetMaxOperation="SELECT operation_code FROM $tms.`task_job_status` WHERE task_jobs_id in($task_jobs_id1) AND plant_code='$plant_code' AND is_active=1 ORDER BY operation_seq DESC LIMIT 0,1";
...
// 			// {
// 			// 	//$sqlx1="select sum(carton_act_qty) as scanned,size_code from packing_summary where order_del_no=\"$schedule\" and trim(order_col_des)=\"$color\" and status=\"DONE\" group by size_code";
...
// 					$color=$sql_row['color'];
// 					echo "<tr>";
// 					echo "<td class=\"lef\">".$buyer_desc."</td>";
...
print("</br></br></br></br></br>Execution took ".$duration." milliseconds.");
?>
</div></div>
</body>
</html>