...
/**getting all plants to send this  alert*/
$qryGetPlants="SELECT plant_code FROM $pms.plant WHERE is_active=1";
$plantsResult=mysqli_query($link_hrms, $qryGetPlants) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$plantsCount=mysqli_num_rows($plantsResult);
if($plantsCount>0){
    while($plantRow=mysqli_fetch_array($plantsCount))
            {
                $plant_code=$plantRow["plant_code"];
