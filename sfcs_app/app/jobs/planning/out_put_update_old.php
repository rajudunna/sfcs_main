...
// $sql="select distinct speed_schedule as speed_schedule from $pps.speed_del_dashboard where speed_schedule > 0 and plant_code='$plant_code'";
// // echo $sql."<br>";
...
$sql="select ord_qty as \"order\", ssc_code_new,ship_tid,schedule_no,style,color,size from $pps.shipment_plan where ship_tid in (select ship_tid from $pps.week_delivery_plan_ref where ex_factory_date_new between \"$start_date_w\" and \"$end_date_w\") order by schedule_no and plant_code='$plant_code";
// echo $sql."<br>";	
$sql_result=mysqli_query($link, $sql) or exit("Sql Error4".mysqli_error($GLOBALS["___mysqli_ston"]));
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		...
		$sql1="select original_order_qty,priority from $bai_pro4.week_delivery_plan where shipment_plan_id='.$ship_tid' and plant_code='$plant_code'";
		...
		//echo "-".date("H:i:s");
		$sqlx1="select IF(operation='$cut_operation',good_quantity+rejected_quantity,0) AS \"cut_total\", IF(operation='$in_operation',good_quantity+rejected_quantity,0) AS \"input_total\" from $pts.transaction_log where schedule='$schedule' and color='$color' and style='$style' and plant_code=\"$plant_code\"";
		// echo $sqlx1."<br/>";
		...
		$get_out_qty_details="select sum(good_quantity+rejected_quantity) as out_qty from $pts.transaction_log where schedule='$schedule_no' and color='$color' and style='$style' and size='$size' and plant_code='$plant_code' and operation=".$out_operation." group by schedule,color";
		...
		$sql3="update $table_ref set  priority='$status' where shipment_plan_id='$ship_tid' and size_code='$size' and plant_code='$plant_code'";
