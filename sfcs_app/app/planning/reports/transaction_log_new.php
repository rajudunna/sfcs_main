...
<input type="submit" id="exp_exc" class="btn btn-info" value="Export to Excel" onclick="getData()">
	...
	if(mysqli_num_rows($sql_result)>0)
	{
		...
		echo "<table id=\"table1\"  border=1 class=\"table\" cellpadding=\"0\" cellspacing=\"0\" style='margin-top:10pt;'><thead><tr class='tblheading' style='color:white;'><th>Operation</th><th>Date</th><th>Time<th>Module</th><th>Section</th><th>Shift</th><th>Style</th><th>Schedule</th><th>Color</th><th>Cut No</th><th>Input Job No</th><th>Size</th><th>SMV</th><th>Quantity</th><th>SAH</th></tr></thead><tbody>";
		$total_qty=0;
		do{
			for($ii=$hour_from;$ii<=$hour_to;$ii++)
			{
				...
				while($rows12=mysqli_fetch_array($sql_result2212))
				{
					...
					$time_query1=" AND TIME(date_time) BETWEEN ('".$rows12['start_time']."') and ('".$rows12['end_time']."')";
				}
				...
				while($sql_row23=mysqli_fetch_array($sql_result23))
				{
				  ...
				  $sfcs_smv=round($sql_row23['sfcs_smv'],3);
				  ...
				  $sql143="select color_code from $bai_pro3.bai_orders_db_confirm where order_del_no=\"".$schedule."\" and order_tid=\"".$order_tid."\" ";
				  $sql_result132=mysqli_query($link, $sql143) or exit("Sql Error7".mysqli_error($GLOBALS["___mysqli_ston"]));
				  while($sql_row132=mysqli_fetch_array($sql_result132))
				  {
					$color_code=$sql_row132['color_code'];
				  }
				   $sah=round($output_qty*$sfcs_smv/60,3);
				   ...
				   echo "<tr bgcolor=\"$bgcolor\"><td>$operationName</td><td>$sdate</td><td>".$time_display." ".$day_part."</td><td>$assigned_module</td><td>$section_name</td><td>$shift</td><td>$style</td><td>".$schedule."</td><td>$color</td><td>".chr($color_code).leading_zeros($cut_number,3)."</td><td>$display_prefix1</td><td>$finalized_title_size_value</td><td>$smv</td>
				   <td>".$output_qty."</td><td>".$sah."</td></tr>";
				   $total_qty=$total_qty+$output_qty;							
				   $total_qty_sah=$total_qty_sah+$sah;		
