		...
		if (isset($_POST['submit'])) {
			...
			while ($row = mysqli_fetch_array($res)) {
				...
				for ($i = 0; $i < sizeof($dates_between); $i++) {
					...
					$sql_get_planned_qty = "SELECT sum(planned_qty) as planned_qty FROM $pps.monthly_production_plan LEFT JOIN $pps.monthly_production_plan_upload_log as upload_log ON upload_log.monthly_pp_up_log_id = monthly_production_plan.monthly_pp_up_log_id where row_name = '" . $workstationCode . "' AND  product_code IN ('" . $styles . "') AND plant_code = '" . $plantcode . "' and planned_date ='" . $dates_between[$i] . "'";
