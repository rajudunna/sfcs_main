...
-->
		...
		if (isset($_POST['submit'])) {
			...
			$sql = "SELECT DISTINCT(row_name) AS work_station,`group`, GROUP_CONCAT(DISTINCT product_code) AS style FROM $pps.monthly_production_plan where  plant_code = '" . $plantcode . "' GROUP BY row_name";
			...
			while ($row = mysqli_fetch_array($res)) {
				...
				for ($i = 0; $i < sizeof($dates_between); $i++) {
					...
					$sql_get_planned_qty = "SELECT sum(planned_qty) as planned_qty FROM $pps.monthly_production_plan where row_name = '" . $workstationCode . "' AND  product_code IN ('" . $style . "') AND plant_code = '" . $plantcode . "' and planned_date ='" . $dates_between[$i] . "'";
