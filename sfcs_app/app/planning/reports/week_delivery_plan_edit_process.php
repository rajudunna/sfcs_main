...
$sql="select remarks from week_delivery_plan where ref_id=$id";
echo $sql."----1"."<br>";
...
switch($code)
{
	...
	case "C":
	{
		...
		$sql1="update week_delivery_plan set remarks=\"".implode("^",$remarks_new)."\" where ref_id=$id";
		echo $sql1."<br>";
		mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		
		//TO Track Operations
		$sql1="insert into query_edit_log(query_executed) values ('111".$sql1."/".$username."')";
		echo $sql1."<br>";
		mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		//TO Track Operations
...
if(isset($_POST['update']))
{
	...
	for($i=0;$i<sizeof($rev_exfa);$i++)
	{
		...
		{
			$sql_remarks_ref="select * from weekly_delivery_plan_remarks where schedule_no='".$schedule_no[$i]."' and color_des='".$color[$i]."' and size_ref='".$title_size[$i]."' and ref_id='".$REF[$i]."'";
