...
//$authorized3=array("bai2mr2","rajithago","kiranm","kirang","baimr1","baimr2","rajithago","srikanthb","roshanm","ravipu"); // planning remarks
...
$start_date_w=date("Y-m-d",strtotime("+7 days", strtotime($start_date_w)));
$end_date_w=date("Y-m-d",strtotime("+7 days", strtotime($end_date_w)));
//echo $start_date_w."$".$end_date_w; */

if(isset($_GET['start_date_w_new']))
{
	$start_date_w=$_GET['start_date_w_new'];
	$end_date_w=$_GET['end_date_w_new'];
	$title=$_GET['title'];
}
else
{
	$start_date_w=$_POST['start_date_w_new'];
	$end_date_w=$_POST['end_date_w_new'];
	$title=$_POST['title'];
}
...
<script src="jquery.columnmanager/jquery.min.js"></script>
...
<body onload="javascript:dodisablenew();" bgcolor="#EEEEEE">
<!-- <div id="ages"></div> -->
<?php
include("../dbconf3.php");
?>
...
<div id="page_heading"><span style="float: left"><h3> Week Delivery Status Report</h3></span><span style="float: right"><b>?</b>&nbsp;</span></div>
<!--<h2><?php echo $title; ?> Week Delivery Status Report</h2>-->
<form method="post" name="input" action="<?php echo $_SERVER['PHP_SELF']; ?>">


<!-- Section: <input type="text" name="section" size=12 value="">  -->

Buyer Division: <select name="division">
<option value='All' <?php if($division=="All"){ echo "selected"; } ?> >All</option>
<option value='DB' <?php if($division=="DB"){ echo "selected"; } ?> >DBA</option>
...
<option value='CK' <?php if($division=="CK"){ echo "selected"; } ?> >CK</option>
</select>

<input type="hidden" name="start_date_w_new" value="<?php echo $start_date_w; ?>">
<input type="hidden" name="end_date_w_new" value="<?php echo $end_date_w; ?>">
<input type="hidden" name="title" value="<?php echo $title; ?>">
<!-- Pending Deliveries (Yes) : <input type="checkbox" name="pending" value="1" <?php if($pending==1){ echo "checked";} ?>> -->

<input type="submit" value="Show" name="submit" onclick="document.getElementById('submit').style.display='none'; document.getElementById('msg').style.display='';">  <!--11-21-2013 by dharani-->
...
{
echo '<a href="week_delivery_plan_view3_excel_wk.php?start_date_w='.$start_date_w.'&end_date_w='.$end_date_w.'">Export to Excel</a> | <a href="week_cache/">Archive</a>';
	if(isset($_GET['division']))
	{
		$division=$_GET['division'];
	}
...
echo '<div id="targetone" class="target">toggle columns:</div>';
echo '<form method="post" name="test" action="week_delivery_plan_edit_process.php">
<p style="float:right"><input type="checkbox" name="option"  id="option" onclick="javascript:enableButton();">Enable<input type="submit" name="update" value="Update"></p>
<table id="tableone" cellspacing="0" class="mytable"><thead>';
...
while($sql_rowza=mysqli_fetch_array($sql_resultza))
{
   ...
   // echo "extra ship=".$extra_ship_ord;
	//echo "total ord=".$total_ord
	//echo "total ord qty=".$total_qty_ord;
...
/*$authorized=array("kiranm","sasidharch","lilanku","kirang","kirang","ramakrishnak","edwinr","amulyap"); //For Packing
$authorized2=array("kiranm","muralim","baiuser","bainet","nalakasb","sureshn","kirang");
$authorized3=array("kiranm","kirang","rajithago");*/

//$authorized=array("kirang","ramakrishnak","edwinr","divyamohanm","sasidharch","lilanku","kirang","ramakrishnak","edwinr","amulyap","sureshn","baimr1","baimr2"); //For Packing
//$authorized2=array("bai2mr1","bainet","nalakasb","muralim","baimr1","bainet","nalakasb","baimr1","baimr2","sureshn","srikanthb","roshanm","ravipu"); // commitments
//$authorized3=array("bai2mr2","rajithago","kiranm","kirang","baimr1","baimr2","rajithago","srikanthb","roshanm","ravipu"); // planning remarks
...
//if(in_array(strtolower($username),$authorized) and ((date("H")<=10 and date("H")>=8) or (date("H")<=18 and date("H")>=15)))
...
else
{
	...
	$edit_rem2="<td><input type=\"text\" name=\"C[]\" value=\"".$remarks[2]."\"><input type=\"hidden\" name=\"REF[]\" value=\"".$edit_ref."\"><input type=\"hidden\" name=\"REM_REF[]\" value=\"".implode("^",$remarks)."\"><input type=\"hidden\" name=\"B[]\" value=\"".$remarks[1]."\"><input type=\"hidden\" name=\"A[]\" value=\"".$remarks[0]."\"><input type=\"hidden\" name=\"code[]\" value=\"C\"><input type=\"hidden\" name=\"rev_exfa[]\" value=\"".$rev_ex_factory_date."\"></td>";
}
...
echo '</table>';
echo "</form>";
//echo "<script>function getCSVData(){ var csv_value=jQuery('#tableone').table2CSV({delivery:'value'}); jQuery(\"#csv_text\").val(csv_value);}</script>";
