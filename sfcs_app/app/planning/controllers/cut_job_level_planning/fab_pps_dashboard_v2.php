...
include(getFullURLLevel($_GET['r'],'functions.php',1,'R')); 
// include("helper.php");
?>
...
while($sql_rowx=mysqli_fetch_array($sql_resultx))
{
	...
	$sql1d="SELECT module_id as modx from plan_modules where module_id in (".$section_mods.") order by module_id*1";
	//echo $sql1d."<br>";
	$sql_num_checkd=0;
	...
	for($x=0;$x<sizeof($mods);$x++)
	{
		...
		$sql1="SELECT * from plan_dash_doc_summ where module=$module and act_cut_issue_status<>\"DONE\" ".$order_div_ref." GROUP BY order_del_no,acutno,clubbing order by priority limit $priority_limit";
		//echo $sql1."<br>";
		//Filter view to avoid Cut Completed and Fabric Issued Modules
		if($_GET['view']==1)
		{
			$sql1="SELECT * from plan_dash_doc_summ where module=$module and act_cut_issue_status<>\"DONE\" ".$order_div_ref." order by priority";
			$view_count=0;
		}		
		//filter to show only cut completed
		if($_GET['view']==3)
		{
			$sql1="SELECT * from plan_dash_doc_summ where module=$module and act_cut_status<>\"DONE\" ".$order_div_ref." order by priority limit $priority_limit";
			$view_count=0;
		}
		//echo $sql1."<br>";
		...
		while($sql_row1=mysqli_fetch_array($sql_result1))
		{
			...
			$sql_doc1="select * from plandoc_stat_log where order_tid like \"%".$schedule."%\" and acutno=$cut_no";
			$result_doc1=mysqli_query($link, $sql_doc1) or exit("Sql Error6".mysqli_error($GLOBALS["___mysqli_ston"]));
			
			$sql_doc="select * from plandoc_stat_log where order_tid like \"%".$schedule."%\" and acutno=$cut_no and act_cut_status=\"DONE\"";
			//echo $sql_doc."<br>";
			$result_doc=mysqli_query($link, $sql_doc) or exit("Sql Error6".mysqli_error($GLOBALS["___mysqli_ston"]));
			 ...
			 {
				 $sql11x="select * from fabric_priorities where doc_ref in (select doc_no from plandoc_stat_log where order_tid like \"%".$schedule."%\" and acutno=$cut_no) and hour(issued_time)+minute(issued_time)>0";
				 ...
				 {
					$id12="lgreen";	
					$sql11x1="select * from fabric_priorities where doc_ref in (select doc_no from plandoc_stat_log where order_tid like \"%".$schedule."%\" and acutno=$cut_no)";
					$sql_result11x1=mysqli_query($link, $sql11x1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			...
			if($fabric_status!=5)
			{
				...
				{
					$sql11="select ft_status from bai_orders_db_confirm where order_joins=\"J$schedule\"";
					//echo $sql11."<br>";
					$sql_result11=mysqli_query($link, $sql11) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
					while($sql_row11=mysqli_fetch_array($sql_result11))
					{
			...
			switch ($fabric_status)
			{
				...
				case "5":
				{
					$sql1x1="select doc_ref from fabric_priorities where doc_ref=$doc_no and hour(issued_time)+minute(issued_time)>0";
			...
			//echo $id;
			$sql11x="select * from fabric_priorities where doc_ref=\"$doc_no\"";
			$sql_result11x=mysqli_query($link, $sql11x) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			//echo $sql11x."<br>";
		...
		{
			$total_qty=0;
			$sql11="select order_col_des,color_code,doc_no,(p_xs+p_s+p_m+p_l+p_xl+p_xxl+p_xxxl+p_s06+p_s08+p_s10+p_s12+p_s14+p_s16+p_s18+p_s20+p_s22+p_s24+p_s26+p_s28+p_s30)*p_plies as total from order_cat_doc_mk_mix where category in ('Body','Front') and order_del_no=$schedule and clubbing=".$sql_row1['clubbing']." and acutno=".$sql_row1['acutno'];
			$sql_result11=mysqli_query($link, $sql11) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row11=mysqli_fetch_array($sql_result11))
			{
		...
		$sqlt="SELECT * from plan_dash_doc_summ where (order_style_no like \"L%Y%\" or order_style_no like \"L%Z%\" or order_style_no like \"O%Y%\" or order_style_no like \"O%Z%\" or order_style_no like \"L%X%\" or order_style_no like \"L%W%\" or order_style_no like \"O%X%\" or order_style_no like \"O%W%\") and module=$module and doc_no=$doc_no and act_cut_issue_status<>\"DONE\"" ;
			//echo $sqlt;	
...
$sqly="select buyer_div from plan_modules where module_id=$module";
//echo $sqly."<br>";
mysqli_query($link, $sqly) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_resulty=mysqli_query($link, $sqly) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
while($sql_rowy=mysqli_fetch_array($sql_resulty))
...
<?php
include "include_legends_rms.php";
?>
</body>
</html>