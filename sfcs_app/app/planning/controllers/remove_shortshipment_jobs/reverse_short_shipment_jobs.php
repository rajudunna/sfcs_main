<?php
    ...
    if($update_revers_qry_result) {
        ...
        {      
            ...
            {
                $sql12=" select * from $bai_pro3.plan_dashboard_input WHERE input_job_no_random_ref in (".implode(",",$ips_tms_jobs).")";
                $sql12_result = mysqli_query($link, $sql12) or exit("Sql Error122".mysqli_error($GLOBALS["___mysqli_ston"]));
            ...
            if(sizeof($ims_jobs)>0)
            {
                ...
                $sql122=" select * from $bai_pro3.ims_log WHERE input_job_rand_no_ref in (".implode(",",$ims_jobs).")";
                $sql122_result = mysqli_query($link, $sql122) or exit("Sql Error12".mysqli_error($GLOBALS["___mysqli_ston"]));
                if(mysqli_num_rows($sql122_result)==0)
                {
