...
<?php
  ...
  $sql_result_hod=mysqli_query($link,$sql_hods) or die("Error".$sql_hods."-".mysqli_error($GLOBALS["___mysqli_ston"]));
  while($row_hods=mysqli_fetch_array($sql_result_hod))
  {
		...
		$calendar2.='<tr><th class=xl636519>'.$section_display_name.'</th><th>:</th><td><input type="textbox" name="section_'.$row_hods["sec_id"].'" class="integer" style="border=1px solid #999999;" onkeypres="return IsNumeric(event);" size="8" value="'.$plan_sah_mod[($row_hods["sec_id"]-1)].'"/></td></tr>';
  } 
