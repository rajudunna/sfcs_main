	...
	for($i=0;$i<sizeof($list_db);$i++)
	{
		...
		{	
			...
			if (mysqli_num_rows($result_org_module) > 0)
			{
				...
				$update_bcd="UPDATE brandix_bts.`bundle_creation_data` SET assigned_module='$original_module' WHERE (assigned_module='0' or assigned_module='') AND input_job_no_random_ref='$input_job_no_random_ref'";
		...
		else
		{
			...
			else
			{
				...
				$update_bcd="UPDATE brandix_bts.`bundle_creation_data` SET assigned_module='$items[0]' WHERE (assigned_module='0' or assigned_module='') AND input_job_no_random_ref='$items[1]'";
