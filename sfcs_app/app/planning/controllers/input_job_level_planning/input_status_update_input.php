...
<script type="text/javascript" src="<?= $_SERVER['DOCUMENT_ROOT']."/sfcs_app/common/js/sweetalert.min.js"; ?>"></script>
<script>	
	function pop_test(e)
	{
		var count = 0;
...
<link rel="stylesheet" href="<?= $_SERVER['DOCUMENT_ROOT']."sfcs_app/common/css/TableFilter_EN/filtergrid.css" ?>">
...
<body>
<form action="index.php?r="<?= $_SERVER['PHP_SELF']; ?> method="POST">
<?php 
include($_SERVER['DOCUMENT_ROOT']."/sfcs_app/common/config/config.php");?>
...
if(in_array($username,$special_users))
{
		...
		echo "</select>";
		if (isset($_POST ["status"])){$pvalue=$_POST['status'];}
...
{	
 	// echo $checkCount;
	echo "<br/><div style=\"Color:Blue;font-size:18px;text-weight:bold;\">Total No of JOBs in ".$module_no." = ".$no_of_ims_job."</div><br/>";
	echo "<br/><div style=\"Color:Blue;font-size:16px;text-weight:bold;\">".$display_job_color_status."</div><br/>";
	if($checkCount>0)
	//if(true)
	{
		//if($job_color_status=="blue")
...
{	//echo 'test';
	echo "<br/><div style=\"Color:red;font-size:18px;text-weight:bold;\">Total No of JOBs in ".$module_no." = ".$no_of_ims_job."</div><br/>";
