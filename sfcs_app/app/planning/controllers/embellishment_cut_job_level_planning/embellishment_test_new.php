...
$sql="select distinct order_style_no from $bai_pro3.plan_doc_summ where CONCAT(TRIM(order_style_no),TRIM(order_col_des)) IN (SELECT DISTINCT CONCAT(TRIM(style),TRIM(color)) FROM brandix_bts.tbl_style_ops_master WHERE operation_code IN (SELECT DISTINCT operation_code FROM brandix_bts.tbl_orders_ops_ref WHERE category IN ('Send PF','Receive PF')))";
