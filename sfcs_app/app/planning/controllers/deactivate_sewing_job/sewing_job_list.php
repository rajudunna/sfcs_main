...
if(isset($_POST['Save']))
{
    ...
    foreach($_POST['style'] as $key => $value){
        if($_POST['remove_type'][$key] == '3'){
            ...
            // $module = $_POST['module'];
            $remove_type=3;
            $job_deacive = "SELECT * FROM $pts.`job_deactive_log` where schedule = '$schedule' and input_job_no='$input_job_no' and remove_type = '0'";
            ...
            if($sql_num_check>0){
                while($sql_row=mysqli_fetch_array($job_deacive_result))
                {
                    ...
                    $update_revers_qry = "update $pts.job_deactive_log set remove_type='$remove_type',input_qty='$input_qty',out_qty='$output_qty',rejected_qty='$rejected_qty',wip='$wip' where id=".$reverse_deactive_job_id;
                ...
                $job_deacive_hold = "SELECT * FROM $pts.`job_deactive_log` where schedule = '$schedule' and input_job_no='$input_job_no' and remove_type = '3'";
            ...
            if($deactive_job_id){
                ...
                if(sizeof($remove_ref_nums)>0)
                {
                    ...
                    $ims_chck_qry = "select distinct input_job_rand_no_ref as ims_jobs from $pms.ims_log where input_job_rand_no_ref in (".implode(",",$remove_ref_nums).")";
                    ...
                    if(sizeof($ims_jobs)>0){
                        $backup_ims_query="INSERT IGNORE INTO $pms.ims_log_backup SELECT * FROM $pms.ims_log WHERE input_job_rand_no_ref in (".implode(",",$ims_jobs).")";
                        mysqli_query($link, $backup_ims_query) or exit("Sql Error16".mysqli_error($GLOBALS["___mysqli_ston"]));
                        $update_ims_qry = "update $pms.ims_log_backup set short_shipment_status = '$remove_type' where input_job_rand_no_ref in (".implode(",",$ims_jobs).") and ims_status <> 'DONE'";
                        $update_ims_qry_res =mysqli_query($link, $update_ims_qry) or exit("Sql Error17".mysqli_error($GLOBALS["___mysqli_ston"]));
                        $del_ims_sqlx = "delete from $pms.ims_log where input_job_rand_no_ref in (".implode(",",$ims_jobs).")";
                    ...
                    $ims_chck_qry_bkp = "select distinct input_job_rand_no_ref as ims_jobs from $pms.ims_log_backup where input_job_rand_no_ref in (".implode(",",$remove_ref_nums).") and short_shipment_status=0";
                    ...
                    if(sizeof($ims_jobs_bkp)>0) {
                        //MAINTAINING 4 STATUS FOR BACKUP ONLY
                        $update_ips_qry = "update $pms.ims_log_backup set short_shipment_status = '4' where input_job_rand_no_ref in (".implode(",",$remove_ref_nums).") and short_shipment_status=0";
                        $update_ips_qry_result = mysqli_query($link, $update_ips_qry) or exit("Sql Error113".mysqli_error($GLOBALS["___mysqli_ston"]));
                    }
                    unset($ims_jobs);
                    unset($ims_jobs_bkp);
            ...
            $remove_type='0';
            $job_deacive = "SELECT * FROM $pts.`job_deactive_log` where schedule = '$schedule' and input_job_no='$input_job_no' and remove_type = '3'";
            $job_deacive_result=mysqli_query($link, $job_deacive) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
            $sql_num_check=mysqli_num_rows($job_deacive_result);
            if($sql_num_check>0){
                while($sql_row=mysqli_fetch_array($job_deacive_result))
                {
                    $reverse_deactive_job_id = $sql_row['id'];
                    $update_revers_qry = "update $pts.job_deactive_log set remove_type=$remove_type where id=".$reverse_deactive_job_id;
                    $update_revers_qry_result = mysqli_query($link, $update_revers_qry) or exit("update error".mysqli_error($GLOBALS["___mysqli_ston"]));
                    ...
                    if($deactive_job_ids){
                        ...
                        if(sizeof($remove_ref_nums1)>0)
                        {
                            ...
                            //To remove Jobs in IMS
                            $ims_chck_qry = "select distinct input_job_rand_no_ref as ims_jobs from $pms.ims_log_backup where input_job_rand_no_ref in (".implode(",",$remove_ref_nums1).") and short_shipment_status=3";
                            ...
                            if(sizeof($ims_jobss)>0){
                                $backup_ims_query="INSERT IGNORE INTO $pms.ims_log SELECT * FROM $pms.ims_log_backup WHERE input_job_rand_no_ref in (".implode(",",$ims_jobss).") and short_shipment_status=3";
                                // echo $backup_ims_query.'<br/>';
                                mysqli_query($link, $backup_ims_query) or exit("Sql Error16".mysqli_error($GLOBALS["___mysqli_ston"]));
                                $update_ims_qry = "update $pms.ims_log set short_shipment_status = '$remove_type' where input_job_rand_no_ref in (".implode(",",$ims_jobss).") and ims_status <> 'DONE'";
                                // echo $update_ims_qry.'<br/>';
                                $update_ims_qry_res =mysqli_query($link, $update_ims_qry) or exit("Sql Error17".mysqli_error($GLOBALS["___mysqli_ston"]));
                                $del_ims_sqlx = "delete from $pms.ims_log_backup where input_job_rand_no_ref in (".implode(",",$ims_jobss).") and short_shipment_status=3";
                            ...
                            $ims_chck_qry_bkp = "select distinct input_job_rand_no_ref as ims_jobs from $pms.ims_log_backup where input_job_rand_no_ref in (".implode(",",$remove_ref_nums1).") and short_shipment_status=4";
                            ...
                            if(sizeof($ims_jobs11)>0) {
                                //MAINTAINING 4 STATUS FOR BACKUP ONLY
                                $update_ips_qry = "update $pms.ims_log_backup set short_shipment_status = '$remove_type' where input_job_rand_no_ref in (".implode(",",$ims_jobs11).") and short_shipment_status=4";
                                $update_ips_qry_result = mysqli_query($link, $update_ips_qry) or exit("Sql Error113".mysqli_error($GLOBALS["___mysqli_ston"]));
                            }
                        }
