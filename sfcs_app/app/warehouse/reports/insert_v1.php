...
echo "<div class='panel panel-default'>
		<div class='panel-body'>
			...
			<div class='row'>
				...
				<div class='col-md-3'>
					<b>Balance to enter :</b> <input type='number' name='balance_new11' id='balance_new11' readonly='readonly' value='".$diff."' style='width: 80px;border:0px;'>
		...
		<div class="panel-body">
			<div class="row">
				...
				<div class="col-md-3">
					...
					<div id="upload">Select file:<input type="file" name="file" id="file" accept=".csv" /></br><a href="'.$url=  getFullURL($_GET['r'],'template.csv','R').'" class="btn btn-warning btn-xs"><i class="fa fa-download"></i>  Download Template for roll upload</a></div>
				</div>
				<div class="col-md-3">
	...
	for($i=0; $i<sizeof($labels);$i++)
	{
		if($labels[$i] != null){
			if($names[$i]=="qty")
			{
				...
				echo '<td><div class="col-md-6 col-md-offset-3"><input type="'.$status[$i].'" name="'.$names[$i].'['.$j.']" id=name="'.$names[$i].'['.$j.']" value="" onChange="quantity(this);" class="form-control float"></div>';
				if($counter == 1){
					echo "
					<div class='col-md-3' id='parent'>
						<div id='floater'><b class='label label-info'>Balance Qty :</b> <input type='number' name='balance_new11_qty' id='balance_new11_qty' readonly='readonly' value='".$diff."' style='width: 80px;border:0px;color:red'>
						</>
					</div>";
				}
				echo '</td>';
