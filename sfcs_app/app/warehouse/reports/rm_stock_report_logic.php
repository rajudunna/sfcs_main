...
while ($sql_row1 = $stock_report_inventory_result->fetch_assoc())
{
    ...
    $single_data = ["location"=>$location,"lotno"=>$lot_no,"style"=>$style_no,"batchno"=>$batch_no,"sku"=>$item,"itemdescription"=>$item_desc,"itemname"=>$item_name,"box_roll_no"=>$boxno,"measuredwidth"=>$ref3,"receivedqty"=>number_format($qty_rec, 2),"issuedqty"=>number_format($qty_issued, 2),"returnqty"=>number_format($qty_return, 2),"balanceqty"=>number_format($qty_balance, 2),"shade"=>$shade,"invoice"=>$invoice,"status"=>$status,"grndate"=>$grn_date,"remarks"=>$remarks,"labelid"=>$tid,"productgroup"=>$product,"buyer"=>$buyer,"supplier"=>$supplier];
...
// if($max_id>0){
		...
		$current_date=date("Y-m-d");
		$stock_report_inventory="SELECT * FROM $bai_rm_pj1.stock_report WHERE DATE(log_time)='$current_date'";
		//echo "Qry : ".$stock_report_inventory;
		$stock_report_inventory_result =$link->query($stock_report_inventory);
		if(mysqli_num_rows($stock_report_inventory_result)> 0){
			while ($sql_row1 = $stock_report_inventory_result->fetch_assoc())
			{
				$lot_no=trim($sql_row1['lot_no']);
				$qty_rec=$sql_row1['qty_rec'];
				...
				//if( array_search( $single_data, $main_data, true ) === false ){
				if(array_search($single_data, $main_data) !== false){
					//echo "Yasss...duplicate";
				}else{
					array_push($main_data,array_map('utf8_encode', $single_data));
					//echo "Not Duplicate..!";
				}
