...
//echo "DB : ".$wms;exit;
	...
	if(isset($_GET['loc_id'])){
		...
		$qry_getloc="SELECT location_id FROM $wms.location_db WHERE sno=".$_GET['loc_id'];
		//echo $qry_getloc;
		...
		if(isset($_POST['formsbmt'])){
			...
			if($_POST['flag_status']!=0){
					...
					$qry_valid="SELECT * FROM $wms.location_db WHERE location_id='$location_name' and status='$location_status'";
					...
					if($rowcount>0){
						...
						$qry_validateaction="SELECT SUM(qty_rec) as qty_rec,SUM(qty_issued) as qty_issue,SUM(qty_ret) as qty_ret FROM $wms.store_in where ref1='$location_name'";
						...
						if($tot_qty>0){
							...
							$qry_updateloc="UPDATE $wms.location_db SET location_id=\"$location_name\",status=\"$location_status\",product=\"$product\" WHERE sno=".$sno;
				...
				//validation for existing record
				$qry_valid="SELECT * FROM $wms.location_db WHERE location_id='$location_name'";
				...
				if($rowcount>0){
					...
					//insert qry here
					$qry_insertloc="INSERT INTO $wms.location_db (location_id,STATUS,product) VALUES ('$location_name','$location_status','$product')";
			...
			echo '<div class="panel-body">';
				...
				echo '<div class="col-sm-12" id="table">';
					echo '<table id="table1" class="table table-bordered">';
						...
						$qry_locations=" select * from $wms.location_db";
						//echo $qry_locations;
