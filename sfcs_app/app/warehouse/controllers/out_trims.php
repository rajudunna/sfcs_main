	...
	if(is_numeric(substr($code,0,1)))
	{
		...
		if(mysqli_num_rows($sql_result)>0)
		{
			...
			if((($qty_rec-($qty_iss+$qty_issued))+$qty_ret)>=0 && $qty_iss > 0)
			{
				...
				$sql1="insert into $wms.store_out (tran_tid,qty_issued,cutno,date,updated_by,log_stamp,Style,Schedule,plant_code,created_user,updated_user,updated_at,created_at) values (\"$code\", ".($qty_iss).", \"$location\",\"".date("Y-m-d")."\",'".$username."_online','".date("Y-m-d H:i:s")."',\"$style\",\"$schedule\",'".$plant_code."','".$username."','".$username."',NOW(),NOW())";
				$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
