...
if(isset($_POST['cartonid']))
{
	...
	if(is_numeric(substr($code,0,1)))
	{
		...
		if(mysqli_num_rows($sql_result)>0)
		{
			...
			$code=ltrim($code,"0");
			$sql="select qty_rec,qty_issued,qty_ret,partial_appr_qty from $wms.store_in where roll_status in (0,2) and tid=\"$code\"";
			// echo "<br>".$sql;
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row=mysqli_fetch_array($sql_result))
			{
			...
			if((($qty_rec-($qty_iss+$qty_issued))+$qty_ret)>=0 && $qty_iss > 0)
			{
				...
				$sql1="update $wms.store_in set qty_issued=".($qty_issued+$qty_iss).", status=$status, allotment_status=$status where tid=\"$code\"";
				// echo "<BR>".$sql1;
				$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
				
				$sql1="insert into $wms.store_out (tran_tid,qty_issued,cutno,date,updated_by,log_stamp,Style,Schedule) values (\"$code\", ".($qty_iss).", \"$location\",\"".date("Y-m-d")."\",'".$username."_online','".date("Y-m-d H:i:s")."',\"$style\",\"$schedule\")";
				$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			...
			else
			{
				...
				echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",500); function Redirect() {  location.href = \"?r=".$_GET['r']."&location=$location&code=$code&bal=$balance\"; }</script>";
			}
		}
		else
		{
...
if(isset($_POST['check2']))
{
	...
	if(is_numeric(substr($code,0,1)))
	{
		...
		if(mysqli_num_rows($sql_result)>0)
		{
			...
			if((($qty_rec-($qty_iss+$qty_issued))+$qty_ret)>=0 && $qty_iss > 0)
			{
				...
				//echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",300); function Redirect() {  location.href = \"out_trims.php?location=$location&code=$code&bal=$balance\"; }</script>";
			}
			else
			{
				echo "<h4>Status  : <span class='label label-warning'>Failed</span> $code</h4>";
				echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",500); function Redirect() {  location.href = \"".getFullURL($_GET['r'],'out_trims.php','N')."&location=$location&code=$code&bal=$balance\"; }</script>";
			}
	...
	{
		echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",500); function Redirect() {  location.href = \"".getFullURL($_GET['r'],'out_trims.php','N')."&location=$code\";; }</script>";
	}
}
...
<input type="submit" name="check2" value="Check Out" onclick='check_docket(event)' class="btn btn-primary" style="margin-top:22px;">
