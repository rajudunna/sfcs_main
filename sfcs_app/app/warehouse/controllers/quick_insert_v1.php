...
if(isset($_POST['submit']) or isset($_GET['ref']))
{
	if(isset($_POST['submit']))
	{
		if(isset($_POST['lot_no'])){
			$lot_no = $_POST['lot_no'];
			$main_sql = "select * from $wms.sticker_report where plant_code='$plantcode' and lot_no = '$lot_no' or rec_no = '$lot_no' ";
		}else{
			$ref=$_POST['reference'];
			$main_sql = "select * from $wms.sticker_report where plant_code='$plantcode' and inv_no=\"".$ref."\" or po_no=\"".$ref."\" or batch_no=\"".$ref."\"";
		}
	}else{
		$ref=$_GET['ref'];
		$main_sql = "select * from $wms.sticker_report where plant_code='$plantcode' and inv_no=\"".$ref."\" or po_no=\"".$ref."\" or batch_no=\"".$ref."\"";
...
$sql="select * from $wms.location_db where plant_code='$plantcode' and status=1 order by sno";
...
echo "<td><a href=\"$url?lot_no=$ref\" onclick=\"Popup=window.open('$url?lot_no=$ref"."','Popup','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes, width=920,height=400, top=23'); if (window.focus) {Popup.focus()} return false;\"><h2>Print Labels</h2></a></td>";
		...
		while($sql_row1=mysqli_fetch_array($main_result))
		{
			...
			$sql="select coalesce(sum(qty_rec),0) as \"in\" from $wms.store_in where plant_code='$plantcode' and lot_no=\"".$sql_row1['lot_no']."\"";
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row=mysqli_fetch_array($sql_result))
			{
				$in=$sql_row['in'];
...
if(!empty($_POST['put']) && isset($_POST['put']))
{
  ...
  $ref=$_POST['ref'];
	for($i=0;$i<sizeof($qty);$i++)
	{
		...
		$sql="insert into $wms.store_in (lot_no, ref1, ref2, qty_rec, date, remarks, log_user,plant_code,created_user,created_at) values ('$lot_no[$i]', '$ref1', '$box[$i]', $qty[$i], '$date', '$remarks[$i]','$user_name','$plantcode','$username','".date('Y-m-d')."')";
		$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
