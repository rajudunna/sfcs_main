...
$sql121="select distinct week(log_date) as wkno from bai_qms_db_reason_track WHERE QMS_TID IN (SELECT QMS_TID FROM $bai_pro3.bai_qms_db WHERE log_date between \"$sdate\" and \"$edate\" AND qms_tran_type=3 AND SUBSTRING_INDEX(SUBSTRING_INDEX(remarks,'-',2),'-',-1) in ($team)) AND qms_reason IN ($reason_codes) and length(supplier) > 0 and log_date between \"$sdate\" and \"$edate\"";
//echo $sql;
...
while($sql_row121=mysqli_fetch_array($sql_result121))
{
	...
	$sql13="SELECT SUM(qms_qty) as qty,WEEK(log_date)+1 as wkno FROM bai_qms_db_reason_track WHERE QMS_TID IN (SELECT QMS_TID FROM $bai_pro3.bai_qms_db WHERE log_date between \"$sdate\" and \"$edate\" AND qms_tran_type=3 AND SUBSTRING_INDEX(SUBSTRING_INDEX(remarks,'-',2),'-',-1) in (".$team.")) AND qms_reason IN ($reason_codes) and length(supplier) > 0 and supplier=\"".$sql_row131["supplier"]."\" and week(log_date)=\"$wkno\" and log_date between \"$sdate\" and \"$edate\" GROUP BY WEEK(log_date) order by WEEK(log_date)+0";
	//echo $sql13."<br>"; 
...
<th><div id=\"container1$i1\" style=\"width: 450px; height: 250px; margin: 0 auto\"></div></th>";
