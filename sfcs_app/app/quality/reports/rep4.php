...
if(isset($_POST['search']))
{
	...
	if($choice==1)
	{
		...
		$trim_tot=0;
		$mac_tot=0;
		$sql="select sum(act_out) as \"output\", sum(rework_qty) as \"rework\", group_concat(distinct module) as \"module\",section from $pts.grand_rep where plant_code='$plantcode' and date between \"$sdate\" and \"$edate\" and shift in ($team) group by section";
		 // echo $sql;
		...
		while($sql_row=mysqli_fetch_array($sql_result))
		{
			...
			$sql1="select ref1, qms_qty from $pps.bai_qms_db where plant_code='$plantcode' and qms_tran_type=3 and substring_index(remarks,\"-\",1) in (".$sql_row['module'].") and log_date between \"$sdate\" and \"$edate\" and substring_index(substring_index(remarks,\"-\",2),\"-\",-1) in ($team)";
			 // echo $sql1;
	...
	if($choice==2)
	{
		...
		$qms_tot=0;
		$rework_tot=0;
		$sql="select sum(bac_qty) as \"output\", group_concat(distinct delivery) as \"delivery\",bac_style from $pts.bai_log where plant_code='$plantcode' and bac_date between \"$sdate\" and \"$edate\" and bac_shift in ($team) group by bac_style order by bac_style+1";
		...
		while($sql_row=mysqli_fetch_array($sql_result))
		{
			...
			$sql1="select ref1, qms_qty from $pps.bai_qms_db where plant_code='$plantcode' and qms_tran_type=3 and qms_schedule in (".$sql_row['delivery'].") and log_date between \"$sdate\" and \"$edate\" and substring_index(remarks,\"-\",1)>0 and substring_index(substring_index(remarks,\"-\",2),\"-\",-1) in ($team)";
			//echo $sql1."<br/>";
			$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row1=mysqli_fetch_array($sql_result1))
			{
			...
			$rework=0;
			$sql1="select sum(bac_qty) as \"rework\" from $pts.bai_quality_log where plant_code='$plantcode' and delivery in (".$sql_row['delivery'].") and bac_date between \"$sdate\" and \"$edate\" and bac_shift in ($team)";
			$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row1=mysqli_fetch_array($sql_result1))
			{
