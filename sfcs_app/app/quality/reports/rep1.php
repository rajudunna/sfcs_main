...
if(isset($_POST['filter']))
{
	...
	$sql="select qms_tid,qms_style,qms_schedule,qms_color,substring_index(remarks,\"-\",1) as \"module\",substring_index(substring_index(remarks,\"-\",2),\"-\",-1) as \"shift\",substring_index(remarks,\"-\",-1) as \"form\",qms_size,log_date,ref1,SUBSTRING_INDEX(doc_no,'D',-1) as doc_no from $pps.bai_qms_db where plant_code='$plantcode' and qms_tran_type=3 and log_date between \"$sdate\" and \"$edate\" order by log_date,substring_index(remarks,\"-\",1)+0,substring_index(remarks,\"-\",-1),qms_style,qms_schedule,qms_color,qms_size";
	// echo $sql;
	...
	if(mysqli_num_rows($sql_result) > 0) {
		...
		while($sql_row=mysqli_fetch_array($sql_result))
		{
			...
			$rep_qty=0;
			$sqlxx="select  sum(qms_qty) as qms_qty from $pps.bai_qms_db where plant_code='$plantcode' and qms_tran_type=2 and qms_schedule='$schedule' and qms_size='$size_value1'";
			// echo $sqlxx;
			...
			for($i=0;$i<sizeof($temp);$i++)
			{
				...
				*/
				...
				{
					$sql_lots="select distinct lot_no as lot_no from $wms.store_in where plant_code='$plantcode' and tid in (".implode(",",$roll_ids).")";
					// echo "<br>".$sql_lots."<br>";
					...
					$sql_lots1="select distinct lot_no as lot_no from $wms.store_in_backup where plant_code='$plantcode' and tid in (".implode(",",$roll_ids).")";
					 //echo "<br>".$sql_lots."<br>";
				...
				if(sizeof($lot_nos) > 1)
				{
					$sql_sup="select group_concat(distinct item_name) as item_name,group_concat(distinct supplier) as sup,group_concat(distinct batch_no) as batch_no from $wms.sticker_report where plant_code='$plantcode' and lot_no in ('".implode("','",$lot_nos)."')";
					 //echo "<br>".$sql_sup."<br>";
