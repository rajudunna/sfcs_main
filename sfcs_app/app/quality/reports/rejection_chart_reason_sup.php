...
$sql13="SELECT SUM(qms_qty) as qty,supplier as supplier FROM $pps.bai_qms_db_reason_track WHERE plant_code='$plantcode' and QMS_TID IN (SELECT QMS_TID FROM bai_qms_db WHERE log_date between \"$sdate\" and \"$edate\" AND qms_tran_type=3 AND SUBSTRING_INDEX(SUBSTRING_INDEX(remarks,'-',2),'-',-1) in ($team)) AND qms_reason IN ($reason_codes) and length(supplier) > 0 and log_date between \"$sdate\" and \"$edate\" GROUP BY supplier order by supplier";
//echo $sql13."<br>"; 
