...
$sql131="SELECT distinct supplier as supplier FROM $pps.bai_qms_db_reason_track WHERE plant_code='$plantcode' and qms_reason IN ($reason_codes) and length(supplier) > 0 and log_date between \"$sdate\" and \"$edate\"";
//echo $sql131."<br>";
...
$sql2="SELECT DISTINCT qms_reason as def FROM $pps.bai_qms_db_reason_track WHERE plant_code='$plantcode' and qms_reason IN ($reason_codes) and supplier=\"".$supplier_name."\" and log_date between \"$sdate\" and \"$edate\" order by qms_reason";
//echo $sql2."<br>";
...
$sql121="select distinct week(log_date) as wkno from $pps.bai_qms_db_reason_track WHERE plant_code='$plantcode' and QMS_TID IN (SELECT QMS_TID FROM $pps.bai_qms_db WHERE plant_code='$plantcode' and log_date between \"$sdate\" and \"$edate\" AND qms_tran_type=3 AND SUBSTRING_INDEX(SUBSTRING_INDEX(remarks,'-',2),'-',-1) in ($team)) AND qms_reason IN ($reason_codes) and log_date between \"$sdate\" and \"$edate\"";
