...
if(isset($_GET['section']))
{
	...
	$sql12="SELECT section_display_name FROM $bai_pro3.sections_master WHERE sec_name=".$_GET['section'];
	...
	echo "<h3><u>Module Breakup of ".$section_display_name."</u></h3>";
	...
	$sql="select sum(act_out) as \"output\", sum(rework_qty) as \"rework\", group_concat(distinct module) as \"module\",section from $pts.grand_rep where plant_code='$plantcode' and date between \"$sdate\" and \"$edate\" and section=".$_GET['section']." and shift in ($team) group by module*1";
	...
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		...
		$sql1="select group_concat(ref1 SEPARATOR \"$\") as \"ref1\", sum(qms_qty) as \"qms_qty\" from $pps.bai_qms_db where plant_code='$plantcode' and qms_tran_type=3 and substring_index(remarks,\"-\",1)=\"".$sql_row['module']."\" and log_date between \"$sdate\" and \"$edate\" and substring_index(substring_index(remarks,\"-\",2),\"-\",-1) in ($team)";
		//echo $sql1."<br/>";
...
if(isset($_GET['style']))
{
	...
	$rework_tot=0;
	//2016-01-18/Naresh B/SR#65771237 
	$sql="select sum(bac_qty) as \"output\", bac_no as \"module\" from $pts.bai_log where plant_code='$plantcode' and bac_date between \"$sdate\" and \"$edate\" and bac_style='".$style_input."' and delivery in (".$_GET['delivery'].") and bac_shift in ($team) group by module*1";
	...
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		...
		$sql1="select group_concat(ref1 SEPARATOR \"$\") as \"ref1\", sum(qms_qty) as \"qms_qty\" from $pps.bai_qms_db where plant_code='$plantcode' and qms_tran_type=3 and substring_index(remarks,\"-\",1) in (".$sql_row['module'].") and qms_schedule in (".$_GET['delivery'].") and log_date between \"$sdate\" and \"$edate\" and substring_index(substring_index(remarks,\"-\",2),\"-\",-1)in ($team)";
		...
		$rework=0;
		$sql1="select COALESCE(sum(bac_qty),0) as \"rework\" from $pts.bai_quality_log where plant_code='$plantcode' and delivery in (".$_GET['delivery'].") and bac_no in (".$sql_row['module'].") and bac_date between \"$sdate\" and \"$edate\" and bac_shift in ($team)";
//echo $sql1."<br/>";
