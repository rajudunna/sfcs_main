...
if(isset($_POST['update']))
{
	...
	if($color_new1=='1')
	{
		for($i=0;$i<sizeof($qty);$i++)
		{
			if($qty[$i]>0)
			{
				$sql="insert into $pps.fca_audit_fail_db (style,schedule,tran_type,size,done_by,pcs,color,plant_code,created_user) values (\"".$style_new."\",\"".$schedule_new."\",1,\"".$size[$i]."\",\"".$username."\",\"".$qty[$i]."\",\"".$color_new[$i]."\",\"".$plant_code."\",\"".$username."\")";
				// echo "<br/>query1=".$sql;
	...
	else
	{
		for($i1=0;$i1<sizeof($qty);$i1++)
		{
			...
			if($qty[$i1]>0)
			{
				$sql1="insert into $pps.fca_audit_fail_db (style,schedule,tran_type,size,done_by,pcs,color,plant_code,created_user) values (\"".$style_new."\",\"".$schedule_new."\",1,\"".$size[$i1]."\",\"".$username."\",\"".$qty[$i1]."\",\"".$color_new."\",\"".$plant_code."\",\"".$username."\")";
				//$sql1="insert into fca_audit_fail_db set style=\"$style_new\", schedule=\"$schedule_new\",color=\"$color_new\", tran_type=1, size=\"".$size[$i1]."\", done_by=\"$username\", pcs=\"".$qty[$i1]."\"";
				// echo "<br/>query2=".$sql1;
