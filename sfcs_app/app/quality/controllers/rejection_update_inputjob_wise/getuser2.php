...
if(isset($_GET['qq']))
{
	...
	switch($q)
	{
		case 1:
		{
			$y++;
			$temp=array();
			$return="$x$y$<select id=\"color[$x]\" name=\"color[$x]\" onchange=\"showUser(3,$x,$y,this.value)\"><option value=\"0\">Select</option>";
		...
		{
			$y++;
			$return="$x$y$<select name=\"schedule[$x]\" onchange=\"showUser(2,$x,$y,this.value)\"><option value=\"0\">Select</option>";
			$sql="select distinct order_del_no as \"schedule\" from $bai_pro3.bai_orders_db_confirm where order_style_no=\"$val\" order by order_del_no";
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row=mysqli_fetch_array($sql_result))
			{
		...
		case 3:
		{
			...
			$return.="</select>";
			$y++;
			$return.="$$x$y$<select id=\"size[$x]\" name=\"size[$x]\"><option value=\"0\">Select</option></select>";
			echo $return;
			break;
		}
...
if(isset($_GET['job_number_validate']))
{
	$doc_number = explode(",",$_GET['job_number_validate']);
	$doc_number_sub = substr($doc_number[0], 0, 1);
	if($doc_number_sub == 'D' || $doc_number_sub == 'R')
	{
		$doc_number[0] = substr($doc_number[0], 1);
	}
	$select_qty_qry = "select remaining_qty from $bai_pro3.cps_log where doc_no = '$doc_number[0]' and size_code = '$doc_number[1]' and operation_code = '$doc_number[3]'";
	// echo $select_qty_qry;
	$result_select_qty_qry = $link->query($select_qty_qry);
	if($result_select_qty_qry->num_rows > 0)
	{
		while($row = $result_select_qty_qry->fetch_assoc()) 
		{
			if($row['remaining_qty'] < $doc_number[2])
			{
				echo 1;
			}
			else
