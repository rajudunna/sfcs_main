...
<div class="row">
	...
	if(isset($_POST['submit']))
	{
		...
		if ($style=='NIL' or $schedule=='NIL' or $color=='NIL') {
			...
			$sql="select sum(a_xs*p_plies) AS \"xs\",sum(a_s*p_plies) AS \"s\",sum(a_m*p_plies) AS \"m\",sum(a_l*p_plies) AS \"l\",sum(a_xl*p_plies) AS \"xl\",sum(a_xxl*p_plies) AS \"xxl\",sum(a_xxxl*p_plies) AS \"xxxl\",sum(a_s01 * p_plies) as \"s01\", sum(a_s02 * p_plies) as \"s02\", sum(a_s03 * p_plies) as \"s03\", sum(a_s04 * p_plies) as \"s04\", sum(a_s05 * p_plies) as \"s05\", sum(a_s06 * p_plies) as \"s06\", sum(a_s07 * p_plies) as \"s07\", sum(a_s08 * p_plies) as \"s08\", sum(a_s09 * p_plies) as \"s09\", sum(a_s10 * p_plies) as \"s10\", sum(a_s11 * p_plies) as \"s11\", sum(a_s12 * p_plies) as \"s12\", sum(a_s13 * p_plies) as \"s13\", sum(a_s14 * p_plies) as \"s14\", sum(a_s15 * p_plies) as \"s15\", sum(a_s16 * p_plies) as \"s16\", sum(a_s17 * p_plies) as \"s17\", sum(a_s18 * p_plies) as \"s18\", sum(a_s19 * p_plies) as \"s19\", sum(a_s20 * p_plies) as \"s20\", sum(a_s21 * p_plies) as \"s21\", sum(a_s22 * p_plies) as \"s22\", sum(a_s23 * p_plies) as \"s23\", sum(a_s24 * p_plies) as \"s24\", sum(a_s25 * p_plies) as \"s25\", sum(a_s26 * p_plies) as \"s26\", sum(a_s27 * p_plies) as \"s27\", sum(a_s28 * p_plies) as \"s28\", sum(a_s29 * p_plies) as \"s29\", sum(a_s30 * p_plies) as \"s30\", sum(a_s31 * p_plies) as \"s31\", sum(a_s32 * p_plies) as \"s32\", sum(a_s33 * p_plies) as \"s33\", sum(a_s34 * p_plies) as \"s34\", sum(a_s35 * p_plies) as \"s35\", sum(a_s36 * p_plies) as \"s36\", sum(a_s37 * p_plies) as \"s37\", sum(a_s38 * p_plies) as \"s38\", sum(a_s39 * p_plies) as \"s39\", sum(a_s40 * p_plies) as \"s40\", sum(a_s41 * p_plies) as \"s41\", sum(a_s42 * p_plies) as \"s42\", sum(a_s43 * p_plies) as \"s43\", sum(a_s44 * p_plies) as \"s44\", sum(a_s45 * p_plies) as \"s45\", sum(a_s46 * p_plies) as \"s46\", sum(a_s47 * p_plies) as \"s47\", sum(a_s48 * p_plies) as \"s48\", sum(a_s49 * p_plies) as \"s49\", sum(a_s50 * p_plies) as \"s50\" from plandoc_stat_log where cat_ref in (select tid from $bai_pro3.cat_stat_log where order_tid=\"$order_tid\" and category in (\"Body\",\"Front\"))";
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row=mysqli_fetch_array($sql_result))
			{
	...
	if(isset($_POST['update']))
	{
		...
		$usr_msg="<br/><span style='color:black;text-weight:bold;text-align:center;'><b>The following entries are failed to update due to M3 system validations:<b></span>
