...
if (isset($_POST['show']) || isset($_GET['show'])) {
    ...
    if ($count > 0) {
        ...
        $table .= "</tr>";
        if ($count == 0) {
            $table .= '<div class="alert alert-danger alert-dismissible fade in">
		...
		<td>$supplier_replace_approved_qty</td><td>$supplier_claim_approved_qty</td><td>$supplier_claim_no</td>";
                ...
                if (in_array($authorized, $has_permission)) {
                    if ($complaint_status == 0) {
                        ...
                        $table .= "<td><a class='btn btn-xs btn-primary' href='" . getFullURLLevel($_GET['r'], 'controllers/supplier_claim_update_form.php', 1, 'N') . "&stat=$complaint_status&sno=$complaint_no'>Not Agreed</a></td>";

                    } else if ($complaint_status == 2) {
                        $table .= "<td><a class='btn btn-xs btn-primary' href='" . getFullURLLevel($_GET['r'], 'controllers/supplier_claim_update_form.php', 1, 'N') . "&stat=$complaint_status&sno=$complaint_no'>Hold</a></td>";

                    } else if ($complaint_status == 3) {
                        $table .= "<td><a class='btn btn-sm btn-primary' href='" . getFullURLLevel($_GET['r'], 'controllers/supplier_claim_update_form.php', 1, 'N') . "&stat=$complaint_status&sno=$complaint_no'>Agreed</a></td>";

                    } else {
                        $table .= "<td><a class='btn btn-sm btn-primary' href='" . getFullURLLevel($_GET['r'], 'controllers/supplier_claim_update_form.php', 1, 'N') . "&stat=$complaint_status&sno=$complaint_no'>Not Agreed<</a></td>";
                    }
                    $url = getFullURL($_GET['r'], 'Supplier_Print_PDF.php', 'N');

                    $table .= "<td><a class='btn btn-xs btn-primary' href=\"$url&sno=$complaint_no&status=2\" onclick=\"retur popitup('$url&sno=$complaint_no&status=2')\" target=\"_blank\">Print</a></td>";

                    if ($mail_status == 0) {
                        $url_mail = $url . "&sno=$complaint_no&status=0&sdate=$startdate&edate=$enddate&stat=$status&bat=$batch";
                        $table .= "<td><a id='mail_button' class='btn btn-xs btn-warning' href=\"$url_mail\" onclic=\"return popitup('$url&sno=$complaint_no&status=0')\">Not Sent</a></td>";
                    } else {
                        $table .= "<td>Sent</td>";
                    }
                } else {
