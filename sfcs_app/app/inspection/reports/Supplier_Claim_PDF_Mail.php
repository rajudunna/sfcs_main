...
$date=date("Y-m-d");
echo $ref_no;
//email_attachment('BAIManufacturingTeam@brandix.com, BAICoreTeam@brandix.com, baisupplychainteam@brandix.com, isteamindia@brandix.com','Dear All, <br/><br/> Please Find The BAI Weekly Delivery Plan Status Report of This Week. <br/><br/> Message Sent Via: http://bainet', 'BAI Weekly Delivery Plan Status Report','Shop Floor System Alert', 'ictsysalert@brandix.com', 'BAI_Weekly_Delivery_Plan_Status.xls', $default_filetype='application/zip');

$sql1="update $bai_rm_pj1.inspection_complaint_db set mail_status=1 where complaint_no=\"$comaplint_no\"";
...
$sql2="select SUM(qty_rec) AS rec_qty,COUNT(tid) as rolls,SUM(ref5) AS insp_qty FROM $bai_rm_pj1.store_in WHERE lot_no IN (".$reject_lot_no.")";	
//echo $sql2."<br>";
...
<table border=2 cellpadding=0 cellspacing=0 width=981 class=xl655354315
 ...
 while($row1=mysqli_fetch_array($result1))
 {	
 	$sql2="select * from $bai_rm_pj1.inspection_complaint_db_log where complaint_track_id=$comaplint_no and complaint_reason=\"".$row1["sno"]."\"";
	$result2=mysqli_query($link, $sql2) or die("Error1=".mysqli_error($GLOBALS["___mysqli_ston"]));
	$rows=mysqli_num_rows($result2);
	
	// if($rows > 0)
	// {
	// 	$src='../'.getFullURLLevel($_GET['r'],'common/images/Wrong1.jpg',1,'R');
	// 	// $src="http'../'.://baitestsrv01:8080/projects/beta/rm_projects/bai_rm_pj1/mpdf7/Wrong1.jpg";
	// 	//$src="X";
	// }
	// else
	// {
	// 	$src= '../'.getFullURLLevel($_GET['r'],'common/images/Wrong2.jpg',1,'R'); 

	// 	// $src="http://baitestsrv01:8080/projects/beta/rm_projects/bai_rm_pj1/mpdf7/Wrong2.jpg";
	// 	//$src=" ";
	// }
	...
	$htmlstr.="<td height=35 colspan=$colspan class=xl794315 style='height:37.1pt;border-left:none'>".$row1["complaint_reason"]."</td>";
	
	//$htmlstr.="<td height=35 colspan=$colspan class=xl794315 style='height:37.1pt;border-left:none'>".$src."&nbsp;&nbsp;".$row1["complaint_reason"]."</td>";
 ...
 <tr height=20 style='height:15.0pt'>
  ...
  border-left:none'>COMMENT</td>
	...
	while($row1=mysqli_fetch_array($result1))
	{
		...
		while($row2=mysqli_fetch_array($result2))
		{
			$sql3="select * from $bai_rm_pj1.inspection_complaint_db_log where complaint_track_id=$comaplint_no and complaint_reason=\"".$row2["sno"]."\"";
...
echo "<script>sweetAlert('Mail Sent successfully','','info')</script>";
$sdate = $_GET['sdate'];
$edate = $_GET['edate'];
$stat = $_GET['stat'];
$bat = $_GET['bat'];
