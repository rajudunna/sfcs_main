...
<body>
				...
				<div class="row">
					...
					if (isset($_POST['submit'])) {
						...
						if ($lot_no_inp != '' && ($po_no == '' && $supplier_invoice == '' && $supplier_batch == '')) {
							$sql_lot_no = "SELECT lot_no FROM $wms.sticker_report WHERE lot_no=\"" . ($lot_no_inp) . "\"";
							// echo $sql_lot_no."<br/>";
						...
						if ($po_no != '' && ($lot_no_inp == '' && $supplier_invoice == '' && $supplier_batch == '')) {
							$sql_lot_no = "SELECT lot_no FROM $wms.sticker_report WHERE po_no=\"" . ($po_no) . "\"";
							$sql_result = mysqli_query($link, $sql_lot_no) or exit(message_sql());
						...
						if ($supplier_invoice != '' && ($lot_no_inp == '' && $po_no == '' && $supplier_batch == '')) {
							$sql_lot_no = "SELECT lot_no FROM $wms.sticker_report WHERE inv_no=\"" . ($supplier_invoice) . "\"";
						...
						if ($supplier_batch != '' && ($lot_no_inp == '' && $po_no == '' && $supplier_invoice == '')) {
							$sql_lot_no = "SELECT lot_no FROM $wms.sticker_report WHERE batch_no=\"" . ($supplier_batch) . "\"";
	...
	if (isset($_POST['bindingdata'])) {
		...
		if ($count1 > 0) {
			...
			$insert_main_pop = "insert into $wms.`main_population_tbl` (no_of_rolls,qty,supplier,invoice_no,batch,lot_no,rm_color) VALUES('" . $hid_roll . "','" . $hid_total . "','" . rtrim(implode(',', array_unique($pos_array)), ",") . "','" . rtrim(implode(',', array_unique($invoice_no_array)), ",") . "','" . rtrim(implode(',', array_unique($batch_no_array)), ",") . "','" . rtrim(implode(',', array_unique($lot_nos_array)), ",") . "', '" . rtrim(implode(',', array_unique($rm_color)), ",") . "')";

			mysqli_query($link, $insert_main_pop) or exit(message_sql());

			$lastinsert_id = $link->insert_id;

			$insertbinditems = "INSERT IGNORE INTO $wms.inspection_population(lot_no,supplier_po,po_line,po_subline,supplier_invoice,item_code,item_desc,item_name,supplier_batch,rm_color,supplier_roll_no,sfcs_roll_no,rec_qty,status,parent_id,store_in_id) VALUES";
			...
			for ($j = 0; $j < $count1; $j++) {
				...
				$update_4point_status = "update wms.store_in set four_point_status=1 where tid in ('" . $main_id . "')";
