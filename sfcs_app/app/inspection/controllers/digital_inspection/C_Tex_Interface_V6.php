...
<?php
	...
	if(isset($_POST['submit']) || isset($_POST['put']) || isset($_POST['confirm']))
	{
		...
		$get_ids="select store_in_id from $wms.inspection_population where parent_id=$parent_id and supplier_batch in ("."'".str_replace(",","','",$lot_no)."'".")";
		//echo $get_ids;
...
$sql="select * from $wms.inspection_db where batch_ref in ("."'".str_replace(",","','",$lot_no)."'".") ";
...
$get_roll_details = "select distinct(store_in_id) as roll_numbers,status from $wms.inspection_population where parent_id=$parent_id and lot_no in ("."'".str_replace(",","','",$lot_ref_batch)."'".")";
...
$sql="select *, if((length(ref4)=0 and qty_allocated <=0),1,0) as \"print_check\" from $wms.store_in where lot_no in ("."'".str_replace(",","','",$lot_ref_batch)."'".") and tid in ("."'".str_replace(",","','",$roll_num)."'".") order by tid,ref2";
//echo $sql;
...
$sql1="select *, if((length(ref4)=0 and qty_allocated <= 0),1,0) as \"print_check\" from $wms.store_in_backup where lot_no in ("."'".str_replace(",","','",$lot_ref_batch)."'".") order by ref2+0";
...
// 	  { ?>
 ...
 $get_details_points = "select sum(rec_qty) as qty from $wms.`inspection_population` where parent_id=$parent_id and status=3";
	...
	{ 					
		if($row522['qty']>0)
		{
			...
			$get_min_value = "select width_s,width_m,width_e from $wms.roll_inspection_child where store_in_tid in (select store_in_id from $wms.`inspection_population` where parent_id=$parent_id)";
			...
			$back_color="";		
			$four_point_count = "select sum(points) as pnt from $wms.four_points_table where insp_child_id in (select store_in_id from $wms.`inspection_population` where parent_id=$parent_id)";	
 ...
 for($i=0;$i<sizeof($values);$i++)
 {
	...
	$get_status = "select status from $wms.inspection_population where parent_id=$parent_id and lot_no='".$temp[7]."' and store_in_id=$temp[0]";
	//echo $get_status;
	...
	if($temp[2]=='')
	{
		$get_details_points = "select rec_qty from $wms.`inspection_population` where store_in_id=$temp[0] and status<>0";
		$details_result_points = mysqli_query($link, $get_details_points) or exit("get_details--1Error" . mysqli_error($GLOBALS["___mysqli_ston"]));
		if(mysqli_num_rows($details_result_points)>0)
		{
			...
			$get_min_value = "select width_s,width_m,width_e from $wms.roll_inspection_child where store_in_tid=$temp[0]";
			$min_value_result=mysqli_query($link,$get_min_value) or exit("get_min_value Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($row_min=mysqli_fetch_array($min_value_result))
			{
			   $width_s = $row_min['width_s'];
			...
			$min_value = min($width_s,$width_m,$width_e);
			$inch_value=round($min_value/(2.54),2);
			$four_point_count = "select sum(points) as pnt from $wms.four_points_table where insp_child_id=".$temp[0]."";
			$status_details_result2=mysqli_query($link,$four_point_count) or exit("get_status_details Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			if(mysqli_num_rows($status_details_result2)>0)
			{	
				while($row52=mysqli_fetch_array($status_details_result2))
...
if(isset($_POST['put']) || isset($_POST['confirm']))
{
	...
	if($head_check>0)
	{
		...
		if(mysqli_affected_rows($link))
		{
			...
			$sql="select log_date from $wms.inspection_db where month(log_date)=".date("m")." and year(log_date)=".date("Y");
			...
			$count=mysqli_num_rows($sql_result);
		  	  for($i=0;$i<sizeof($suppliers);$i++)
			  {
				...
				if($supplier==$x[1] and $x[1]!=0)
				{
					...
					$sql="select log_date from $wms.inspection_db where month(log_date)=".date("m")." and year(log_date)=".date("Y")." and unique_id like \"%".strtoupper($letters)."%\"";
					$sql_result=mysqli_query($link, $sql) or exit("Sql Error6=".mysqli_error($GLOBALS["___mysqli_ston"]));
		  	...
		  	$sql="update $wms.inspection_db set unique_id=\"$count\" where batch_ref=\"$lot_no_new\"";
		  	//echo $sql;
	...
	$sql="update $wms.inspection_db set status=0 where batch_ref=\"$lot_no_new\"";
	mysqli_query($link, $sql) or exit("Sql Error7=".mysqli_error($GLOBALS["___mysqli_ston"]));
	if(isset($_POST['confirm']))
	{
		...
		$sql1="update $wms.inspection_db set status=1 where batch_ref=\"$lot_no_new\"";
		...
		if($ele_check[$i]>0)
		{
			...
			if($partial_rej_qty[$i]>0 and $partial_rej_qty[$i]>$ele_t_length[$i] )// when partial qty rejected then new row is inserted with rejected qty and remaning with approved qty updated
			{
				 $sql= "insert INTO $wms.store_in ( ref1,lot_no, ref2, qty_issued, qty_ret, DATE, log_user, remarks, log_stamp, STATUS, allotment_status, qty_allocated, upload_file, m3_call_status, split_roll, qty_rec,ref3,ref4, ref5, ref6, shrinkage_length, shrinkage_width,shrinkage_group,roll_joins, roll_status,partial_appr_qty,rejection_reason, external_reason_code,shade_grp,act_width_grp, roll_remarks)
				  ...
				  FROM $wms.store_in WHERE tid=".$ele_tid[$i];
