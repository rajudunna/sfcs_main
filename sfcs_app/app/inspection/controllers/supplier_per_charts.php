...
//if(isset($_POST['filter']))
{
	...
	$sql_sup_ref="select * from $pms.inspection_supplier_db WHERE PRODUCT_CODE=\"Fabric\" order by seq_no";
    ...
    //echo  preg_grep('/^$mon1.*/', $array);
 	$supplier=array();
	$rec_qty=array();
	$sql="select ROUND(SUM(rec_qty),0) AS qty,supplier FROM $wms.sticker_report WHERE DATE(grn_date) BETWEEN \"".$sdate."\" AND \"".$edate."\" AND product_group=\"Fabric\"  ".$suppliers_list_ref_query." GROUP BY supplier ORDER BY field ".$sup_ref_x_fin."";
	//echo $sql."<br>";
	$sql_result=mysqli_query($link, $sql) or exit("Sql Error".$sql.mysqli_error($GLOBALS["___mysqli_ston"]));
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		$rec_qty[]=$sql_row["qty"];
		$sql_sup="select * from $pms.inspection_supplier_db where supplier_m3_code=\"".$sql_row["supplier"]."\" and product_code=\"Fabric\"";
		...
		$sql_col1="select * FROM $pms.inspection_supplier_db where supplier_code=\"".$supplier_name_ref."\" and product_code=\"Fabric\""; 
		//echo $sql_col."<br>";
	...
	$sql="select distinct supplier as sup from $wms.sticker_report WHERE DATE(grn_date) BETWEEN \"".$sdate."\" AND \"".$edate."\" AND product_group=\"Fabric\" ".$suppliers_list_ref_query." and length(supplier) > 0 ORDER BY field ".$sup_ref_x_fin."";
	//echo $sql."<br>";
	$sql_result=mysqli_query($link, $sql) or exit("Sql Error".$sql.mysqli_error($GLOBALS["___mysqli_ston"]));
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		$supplier_name=$sql_row["sup"];
		$sql_sup1="select * from $pms.inspection_supplier_db where supplier_m3_code=\"".$sql_row["sup"]."\" and product_code=\"Fabric\"";
		...
		$sql_col="select * FROM $pms.inspection_supplier_db where supplier_code=\"".$supplier_name_ref1."\" and product_code=\"Fabric\""; 
		//echo $sql_col."<br>";
		...
		$sql1="select count(distinct batch_no) as batch_no from $wms.sticker_report WHERE DATE(grn_date) BETWEEN \"".$sdate."\" AND \"".$edate."\" AND product_group=\"Fabric\" and supplier=\"".$supplier_name."\"";
		//echo $sql1."<br>";
		...
		$sql1x="select distinct batch_no as batch_no from $wms.sticker_report WHERE DATE(grn_date) BETWEEN \"".$sdate."\" AND \"".$edate."\" AND product_group=\"Fabric\" and supplier=\"".$supplier_name."\"";
		//echo $sql1."<br>";
		...
		{	
			...
			for($i1=0;$i1<sizeof($complaint_reason);$i1++)
			{
				...
				$sql5="select * FROM $wms.inspection_complaint_db a,$wms.inspection_complaint_db_log b WHERE a.complaint_no=b.complaint_track_id AND a.supplier_name=\"".$supplier_name."\" AND b.complaint_reason='".$comaplint_sno[$i1]."' AND a.reject_batch_no='".$batch_ref."'";
				...
				{
					$sql3="select GROUP_CONCAT(CONCAT(\"'\",a.reject_lot_no,\"'\")) as lots FROM $wms.inspection_complaint_db a,$wms.inspection_complaint_db_log b WHERE a.complaint_no=b.complaint_track_id AND a.supplier_name=\"".$supplier_name."\" AND a.reject_batch_no='".$batch_ref."' AND b.complaint_reason='".$comaplint_sno[$i1]."' GROUP BY b.complaint_track_id";
					//echo $sql3."</br>";
