...
$sql="select * from $pps.inspection_db where plant_code='$plantcode' and batch_ref=\"".trim($lot_no)."\"";
...
$sql="select *, if((length(ref4)=0 and qty_allocated <=0),1,0) as \"print_check\" from $wms.store_in where plant_code='$plantcode' and lot_no in ("."'".str_replace(",","','",$lot_ref_batch)."'".") order by ref2+0";
...
$sql1="select *, if((length(ref4)=0 and qty_allocated <= 0),1,0) as \"print_check\" from $wms.store_in_backup where plant_code='$plantcode' and lot_no in ("."'".str_replace(",","','",$lot_ref_batch)."'".") order by ref2+0";
...
$sql="select  COUNT(DISTINCT REPLACE(ref2,\"*\",\"\"))  as \"count\" from $wms.store_in where plant_code='$plantcode' and  lot_no in ("."'".str_replace(",","','",$lot_ref_batch)."'".")";
...
for($i=0;$i<$shade_count;$i++)
 {
	  $sql_sc="select count(*) as cnt from $wms.store_in where plant_code='$plantcode' and  lot_no in ("."'".str_replace(",","','",$lot_ref)."'".") and ref4=\"".$scount_temp2[$i]."\"";
	//echo $sql_sc;
...
if(isset($_POST['put']) || isset($_POST['confirm']))
{
	...
	if($head_check>0)
	{
		$sql_check="select batch_ref from $pps.inspection_db where plant_code='$plantcode' and  batch_ref=\"$lot_no_new\"";
		...
		{
			$sql="insert into $pps.inspection_db(batch_ref) values (\"$lot_no_new\")";
		...
		if(mysqli_affected_rows($link))
		{
			...
			$sql="select log_date from $pps.inspection_db where plant_code='$plantcode' and  month(log_date)=".date("m")." and year(log_date)=".date("Y");
			...
			$count=mysqli_num_rows($sql_result);
		  	  for($i=0;$i<sizeof($suppliers);$i++)
			  {
				...
				if($supplier==$x[1] and $x[1]!=0)
				{
					...
					$sql="select log_date from $pps.inspection_db where plant_code='$plantcode' and  month(log_date)=".date("m")." and year(log_date)=".date("Y")." and unique_id like \"%".strtoupper($letters)."%\"";
					$sql_result=mysqli_query($link, $sql) or exit("Sql Error6=".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	$sql="update $pps.inspection_db set status=0,updated_user= '".$username."' where plant_code='$plantcode' and  batch_ref=\"$lot_no_new\"";
	mysqli_query($link, $sql) or exit("Sql Error7=".mysqli_error($GLOBALS["___mysqli_ston"]));
	if(isset($_POST['confirm']))
	{
		...
		$sql1="update $pps.inspection_db set status=1,updated_user= '".$username."' where plant_code='$plantcode' and  batch_ref=\"$lot_no_new\"";
	...
	{	
		if($ele_check[$i]>0)
		{
			...
			{
				$sql="update $wms.store_in set rejection_reason=\"".$rejection_reason[$i]."\", shrinkage_length=\"".$shrinkage_length[$i]."\",shrinkage_width=\"".$shrinkage_width[$i]."\",shrinkage_group=\"".$shrinkage_group[$i]."\",roll_remarks=\"".$roll_remarks[$i]."\", roll_status=\"".$roll_status_ref[$i]."\",partial_appr_qty=\"".$partial_rej_qty[$i]."\",roll_joins=\"".$roll_joins[$i]."\",ref5=\"".$ele_c_length[$i]."\", ref6=\"".$ele_t_width[$i]."\", ref3=\"".$ele_c_width[$i]."\", updated_user= '".$username."'$add_query where plant_code='$plantcode' and  tid=".$ele_tid[$i];
