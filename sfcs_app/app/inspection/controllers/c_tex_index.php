...
<?php
		...
		$sql = "select distinct lot_no AS \"lot_ref_batch\" from $wms.store_in 
				WHERE lot_no IN(select DISTINCT lot_no from $wms.sticker_report WHERE batch_no=\"".trim($lot_no)."\" and product_group ='Fabric' and plant_code='".$plant_code."') 
				GROUP BY lot_no";
...
if(isset($_POST['submit1']))
{
	...
	$sql_query = "select count(*) as count from $wms.store_in where lot_no="."'".$lot_no1."' and plant_code='".$plant_code."'";
	...
	if($count >0)
	{
		$sql="select distinct batch_no as \"batch_no\" from $wms.sticker_report where lot_no=\"".trim($lot_no1)."\" and product_group ='Fabric' and plant_code='".$plant_code."'";
		$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		while($sql_row=mysqli_fetch_array($sql_result))
		{
		...
		if ($num_rows == 0)
		{
			$sql1="select * from $wms.sticker_report where lot_no=\"".trim($lot_no1)."\" and product_group not in ('Fabric') and plant_code='".$plant_code."'";
