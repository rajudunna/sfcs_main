...
if(isset($_POST['update']))
{
	...
	for($i=1;$i<=sizeof($reason_name);$i++)
	{
		...
		$sql11="select * from $wms.supplier_performance_track where tid='".trim($batch_ref[$i])."-".$month_ref[$i]."'";
		...
		$sql1="select * from $wms.supplier_performance_track where tid='".trim($batch_ref[$i])."-".$month_ref[$i]."'";
		//echo $sql1."<br>";
		$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error4".$sql1.mysqli_error($GLOBALS["___mysqli_ston"]));
		
		if(mysqli_num_rows($sql_result1) >0)
		{
			$sql2="delete from $wms.supplier_performance_track where tid='".trim($batch_ref[$i])."-".$month_ref[$i]."'";
