...
if (isset($_GET['schedule']) && isset($_GET['seq_no']))
{
	...
	echo "<div class='table table-responsive'><div class='row'><table class='table table-bordered'>";
	echo "<tr class='info'>";
	echo "<th>Schedule</th>";
	...
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		...
		if($scanning_methods=='Bundle Level')
		{
			if ($print_btn_color == 'danger')
			{
				if(strtolower($barcode_4x2) == 'yes')
				{
					echo '<td><span class="badge progress-bar-dangzzer">Printing Done</span></td>
	...
	echo ' </table></div></div></form></div></div>';
