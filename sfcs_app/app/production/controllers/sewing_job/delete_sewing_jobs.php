...
<?php
    ...
    if($_GET['schedule'] && $_GET['seq_no'] && ($_GET['option'] == 'delete'))
    {
        ...
        if (count($tid) > 0)
        {
			...
			{
				$sq_input_jobs="select doc_no,size_code,type_of_sewing,sum(carton_act_qty) as qty FROM $bai_pro3.pac_stat_log_input_job WHERE doc_no IN (".implode(',',array_unique($docket_no).") group by type_of_sewing,doc_no,size_code"; 
			...
			// echo $delete_plan_dashboard_qry."<br>"; 
			$sql_check_avail=mysqli_query($link, $delete_plan_dashbrd_qry) or exit("select pac_stat_log_input_job");
			if(mysqli_num_rows($sql_check_avail)>0)
			{
				...
				$delete_plan_input_qry="DELETE FROM bai_pro3.`plan_dashboard_input` WHERE input_job_no_random_ref IN (SELECT input_job_no_random FROM $bai_pro3.`pac_stat_log_input_job` WHERE tid IN (".implode(",",$final_doc)."))"; 
				// echo $delete_plan_input_qry."<br>"; 
