				...
				{	
					...
					while($row1=mysqli_fetch_array($result1))
					{
						...
						else
						{
							...
							$job_counter_tmp= echo_title("$bai_pro3.packing_summary_input","MIN(CAST(barcode_sequence AS DECIMAL))","order_col_des in ('".str_replace(",","','",implode(",",$cols_tot))."') and pac_seq_no = $seq_no and  order_del_no",$schedule,$link);	
							if($input_job_no_tmpn>0)
							{
								$input_job_no=$input_job_no_tmpn;
								$job_counter=$job_counter_tmp;
