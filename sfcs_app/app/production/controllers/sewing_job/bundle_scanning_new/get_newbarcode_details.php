<?php 
    ...
    include 'functions_scanning.php';
    ...
    // assigned module with barcode                    
    $new="select * from brandix.bts.bundle_creation_data where assigned_module=$b_module[$i] and barcode_number = $barcode_number";
    // echo $new."his";
    $new_result=mysqli_query($link,$new) or exit("Error108".mysqli_error($GLOBALS["___mysqli_ston"]));
     while($new_qry_row=mysqli_fetch_array($new_result))
     {
        $module=$new_qry_row['module'];
        $barcode_number=$new_qry_row['barcode_number'];                 

     }
     if (key == 'assigned_module')
      {
        ($new_result->num_row>0)
      }

   else
    {
    echo "<script>swal('select module and  assigned module not matched for barcode','','warning');</script>";
    }
