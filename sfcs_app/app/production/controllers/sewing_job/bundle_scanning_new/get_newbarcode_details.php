                                        ...
                                        if($rej_insert_result){
                                           ...
                                           // echo $query;exit;
                                            ...
                                            $result_query = $link->query($query) or exit('query error in updating');
                                             //RK - if entire bundle is rejected close the next bundles - 26-09-2020
                                            $get_total_rejection_qty_query="select send_qty,rejected_qty,style,color from $brandix_bts.bundle_creation_data where bundle_number =".$bundle_individual_number." and operation_id = ".$op_no;
                                            ...
                                            if($get_total_rejection_qty_query_res->num_rows > 0){
                                                ...
                                                while($ops_order_data=mysqli_fetch_array($ops_order_res)){
                                                    $ops_order=$ops_order_data['operation_order'];
                                                }
                                                //echo $get_total_rejection_qty_query_res->num_rows;exit;
                                                while($current_ops_data=mysqli_fetch_array($get_total_rejection_qty_query_res)){
                                                    $currenet_bundle_send_qty=$current_ops_data['send_qty'];
                                                    $currenet_bundle_rej_qty=$current_ops_data['rejected_qty'];
                                                   // echo $currenet_bundle_send_qty."==".$currenet_bundle_rej_qty;
                                                    if($currenet_bundle_send_qty==$currenet_bundle_rej_qty){
                                                        $query = "UPDATE $brandix_bts.bundle_creation_data SET bundle_qty_status=$bundle_status where bundle_number =".$bundle_individual_number." and operation_id in (select operation_code from $brandix_bts.tbl_style_ops_master where style='$b_style' and color = '".$b_colors."' AND CAST(operation_order AS CHAR) >= '$ops_order')";
                                                        //echo $query;exit;
                                                         $result_query = $link->query($query) or exit('query error in updating'); 
                                                    }
                                                }
                        ...
                        {   
                            ...
                            else
                            {
                                ...
                                while($row = $result_style_data->fetch_assoc()) 
                                {
                                    ...
                                    if($parallel_balance_report<0)
                                    {
                                        ...
                                        //echo $type_of_sewing_query;exit;
                           ...
                           //echo $b_op_id;exit;
                            ...
                            else
                            {
                                ...
                                {
                                    $schedule_count_query = "SELECT input_job_no_random_ref FROM $brandix_bts.bundle_creation_data WHERE input_job_no_random_ref = $b_job_no AND operation_id =$b_op_id";
                                   // echo $schedule_count_query;exit;
                                    $schedule_count_query = $link->query($schedule_count_query) or exit('query error');
                                        ...
                                        if($b_tid[$key] == $bundle_no){
                                            if($concurrent_flag == 0)
                                            {
                                                ...
                                                if($result_select_send_qty->num_rows >0)
                                                {
                                                    ...
                                                    //echo $previously_scanned;exit;
                                                    
                                                    $left_over_qty="0";
                                                    if($previously_scanned == 0){
                                                        //echo $b_send_qty."==".$b_old_rej_qty_new;exit;
                                                        if($b_send_qty == $b_old_rej_qty_new){
                                                            $result_array['status'] = 'This Bundle Qty Is Completely Rejected';
                                                            $result_array['color_code'] = "#f31c06";
                                                        }else{
                                                    ...
                                                    //echo $result_query;exit;
                                                    if($result_query)
                                                    {
                                                       // echo $b_rep_qty[$key];exit;
                                                        if($b_rep_qty[$key] > 0)
                                                        {
                                                            $bulk_insert_post_temp .= '("'.$b_style.'","'. $b_schedule.'","'.$b_colors[$key].'","'.$b_size_code[$key].'","'. $b_sizes[$key].'","'. $sfcs_smv.'","'.$b_tid[$key].'","'.$b_in_job_qty[$key].'","'.$b_in_job_qty[$key].'","'.$previously_scanned .'","'.$b_rej_qty[$key].'","'.$left_over_qty.'","'. $b_op_id.'","'.$b_doc_num[$key].'","'.date('Y-m-d').'","'.$b_a_cut_no[$key].'","'.$b_inp_job_ref[$key].'","'.$b_job_no.'","'.$b_shift.'","'.$b_module[$key].'","'.$b_remarks[$key].'","'.$username.'")';	
                                                            $result_query_001_temp = $link->query($bulk_insert_post_temp) or exit('bulk_insert_post query error in updating');
                                                    ...
                                                    //echo $post_ops_code." != null && ".$post_ops_code.">0";exit;
                                            ...
                                            try{
                                                ...
                                                {   
                                                    ...
                                                    foreach($bundle_no as $key=>$value)
                                                    {
                                                        ...
                                                        $qry_nop="select (present+jumper) as nop FROM $bai_pro.pro_attendance WHERE module=".$b_module[$key]." and date='".$bac_dat."' and shift='".$b_shift."'";
                                                        $qry_nop_result=mysqli_query($link,$qry_nop) or exit("Bundles Query Error14".mysqli_error($GLOBALS["___mysqli_ston"]));
                                                        while($nop_qry_row=mysqli_fetch_array($qry_nop_result))
                                                        {
                                                            $avail=$nop_qry_row['nop'];
