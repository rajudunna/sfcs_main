...
<?php
	...
	$sql1216="SELECT category,group_concat(operaiton_code) as codes FROM $brandix_bts.tbl_orders_ops_ref WHERE default_operaiton='Yes' group by category order by category*1"; 
	...
	for($l=0;$l<sizeof($category_name);$l++)
	{
		if($category_name[$l]==$split_proces_name)
		{
			...
			$order_tid='';
			$sql12="SELECT * FROM $bai_pro3.bai_orders_db_confirm where order_tid='".$order_tid."'";
					...
					if(sizeof($mo_no)>0)
					{
						...
						if(sizeof($mo_no)==1)
						{
							for($k=0;$k<sizeof($ops);$k++)
							{
								...
								{
									$sql12="SELECT (p_".$sizes_array[$i]."*p_plies) as qty,doc_no FROM $bai_pro3.plandoc_stat_log WHERE p_".$sizes_array[$i].">0 and order_tid='".$order_tid."' group by doc_no";
									echo $sql12."<br>";						
									$result12=mysqli_query($link, $sql12) or die("Error".mysqli_error($GLOBALS["___mysqli_ston"])); 
									while($row12=mysqli_fetch_array($result12)) 
									{
										$sql="INSERT INTO `bai_pro3`.`mo_operation_quantites` (`date_time`, `mo_no`, `ref_no`, `bundle_quantity`, `op_code`, `op_desc`) VALUES ('".date("Y-m-d H:i:s")."', '".$mo_no[0]."','".$row12['doc_no']."', '".$row12['qty']."', '".$ops_m_id[$mo_no[0]][$k]."', '".$ops_m_name[$mo_no[0]][$k]."')";
						...
						{
							$bal=0;$qty_tmp=0;
							$sql1234="SELECT (p_".$sizes_array[$i]."*p_plies) as qty,doc_no FROM $bai_pro3.plandoc_stat_log WHERE p_".$sizes_array[$i].">0 and order_tid='".$order_tid."' group by doc_no";
										...
										{	
											...
											{	
												if($ops_m_id[$mo_no[$kk]][$ops[$jj]]>0)
												{
													...
													if($qty>0)
													{
														$sql="INSERT INTO `bai_pro3`.`mo_operation_quantites` (`date_time`, `mo_no`, `ref_no`, `bundle_quantity`, `op_code`, `op_desc`) VALUES ('".date("Y-m-d H:i:s")."', '".$mo_no[$kk]."' , '".$row1234['doc_no']."', '".$qty."', '".$ops_m_id[$mo_no[$kk]][$ops[$jj]]."', '".$ops_m_name[$mo_no[$kk]][$ops[$jj]]."')";
														$result1=mysqli_query($link, $sql) or die("Error".mysqli_error($GLOBALS["___mysqli_ston"]));
													}
												}
											}	
							...
							$sql12341="SELECT (p_".$sizes_array[$i]."*p_plies) as qty,doc_no FROM $bai_pro3.plandoc_stat_log WHERE p_".$sizes_array[$i].">0 and order_tid='".$order_tid."' group by doc_no";
							...
							if(mysqli_num_rows($result12341)>0)
							{
								...
								{
									$qty=$row12341['qty'];
									$sql1234100="SELECT sum(bundle_quantity) as qty,doc_no FROM $bai_pro3.plandoc_stat_log WHERE p_".$sizes_array[$i].">0 and order_tid='".$order_tid."' group by doc_no";
									...
									if(($qty-$qty_fill)>0)
									{
										for($jjj=0;$jjj<sizeof($ops);$jjj++)
										{
											...
											{
												$sql="INSERT INTO `bai_pro3`.`mo_operation_quantites` (`date_time`, `mo_no`, `ref_no`, `bundle_quantity`, `op_code`, `op_desc`) VALUES ('".date("Y-m-d H:i:s")."', '".$lastmo."', '".$row12341['doc_no']."', '".($qty-$qty_fill)."', '".$ops_m_id[$lastmo][$ops[$jjj]]."', '".$ops_m_name[$lastmo][$ops[$jjj]]."')";
												$result1=mysqli_query($link, $sql) or die("Error".mysqli_error($GLOBALS["___mysqli_ston"]));
											}							
										}							
									}
