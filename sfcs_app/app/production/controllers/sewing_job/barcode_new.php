...
<?php
	...
	$html = '
		...
		while($barcode_rslt = mysqli_fetch_array($sql_barcode))
		{
			...
			//Dummy sticker Ends
			$operation_det="SELECT tor.operation_name as operation_name,tor.operation_code as operation_code FROM $brandix_bts.tbl_style_ops_master tsm LEFT JOIN $brandix_bts.tbl_orders_ops_ref tor ON tor.id=tsm.operation_name WHERE style='$style ' AND color='$color' and tsm.barcode='Yes' and tor.category='sewing' AND tor.display_operations='yes' ORDER BY CAST(tsm.operation_order AS CHAR)";
