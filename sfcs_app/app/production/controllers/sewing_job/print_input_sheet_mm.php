...
<div class="panel-body">
        ...
        <?php $imgurl = getFullURLLevel($_GET['r'],'common/images/Book1_29570_image003_v2.png',2,'R'); ?> 
        ...
        /*for($j=0;$j<sizeof($schedule_split);$j++) 
        { 
            $total_qty=0; 
            $sql_dest="select trim(destination) as dest from bai_orders_db where order_del_no=\"".$schedule_split[$j]."\""; 
            $result_dest=mysql_query($sql_dest,$link) or die("Error-".$sql6."-".mysql_error()); 
            while($sql_row_dest=mysql_fetch_array($result_dest)) 
            { 
                $destination_name=$sql_row_dest["dest"]; 
            } 
            echo "<tr height=20 style='height:15.0pt'><td height=20 style='height:15.0pt'>".$schedule_split[$j]."</td><td>".$destination_name."</td>"; 
            for($i=0;$i<sizeof($size_array);$i++) 
            { 
                $sql6="SELECT * FROM packing_summary_input where size_code='".$size_array[$i]."' and order_del_no=".$schedule_split[$j]." group by tid order by input_job_no"; 
                $result6=mysql_query($sql6,$link) or die("Error-".$sql6."-".mysql_error()); 
                $rows_count=mysql_num_rows($result6); 
                if($rows_count > 0) 
                { 
                    $sql5="SELECT round(sum(carton_act_qty),0) as qty FROM packing_summary_input where size_code='".$size_array[$i]."' and order_del_no=".$schedule_split[$j]." group by tid order by input_job_no"; 
                    $result5=mysql_query($sql5,$link) or die("Error-".$sql5."-".mysql_error());             
                    while($sql_row5=mysql_fetch_array($result5)) 
                    { 
                        echo "<td align=\"center\">".$sql_row5["qty"]."</td>"; 
                        $total_qty=$total_qty+$sql_row5["qty"]; 
                    } 
                } 
                else 
                { 
