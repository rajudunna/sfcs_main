...
// echo "<thead><tr><th>Color</th><th>Cut Number</th><th>Size</th><th>Input Job Quantity</th><th>Input Job Number</td></tr></thead>";
//$sql1="select cut_num from $brandix_bts.tbl_miniorder_data_archive where mini_order_ref=".$mini_order_ref." order by cut_num asc"; 
$sql11="SELECT color FROM $brandix_bts.tbl_miniorder_data WHERE mini_order_ref=".$mini_order_ref." AND mini_order_num > 0 group BY color"; 
//echo $sql11."<br>"; 
...
{ 
    ...
    { 
        ...
        $sql12="SELECT size FROM $brandix_bts.tbl_miniorder_data WHERE mini_order_ref=".$mini_order_ref." AND mini_order_num > 0 and cut_num=".$row13["cut_num"]." group BY size order by size*1"; 
        //echo $sql12."<br>"; 
        ...
        { 
            ...
            { 
                ...
                // echo "1Ref=".$size_color_qty."-".$input_job_quantiy."<br>"; 
                ...
                { 
                    ...
                    {         
                        ...
                        // echo "<tr><td>".$row1["color"]."</td><td>".$row1["cut_num"]."</td><td>".$row1["size"]."</td><td>".$input_job_quantiy."</td><td>".$input_job_no."</td></tr>"; 
                    ...
                    { 
                        ...
                        // echo "<tr><td>".$row1["color"]."</td><td>".$row1["cut_num"]."</td><td>".$row1["size"]."</td><td>".$size_color_qty."</td><td>".$input_job_no."</td></tr>"; 
                    }             
                } 
                else 
                { 
                        ...
                        $rand=$schedule.date("ymdH").$row1["cut_num"].$input_job_no; 
                        $sql1q="insert into $bai_pro3.pac_stat_log_input_job(doc_no,size_code,carton_act_qty,input_job_no,input_job_no_random,destination,packing_mode,old_size) values(\"".$docket_number."\",\"".$size_code."\",\"".$size_color_qty."\",\"".$row1["cut_num"].$input_job_no."\",\"".$rand."\",\"".$destination."\",\"".$packing_mode."\",\"".$size_code_ref."\")"; 
                        //echo "1=".$sql1q."<br>"; 
                        mysqli_query($link, $sql1q) or exit("Error".mysqli_error($GLOBALS["___mysqli_ston"])); 
                        // echo "<tr><td>".$row1["color"]."</td><td>".$row1["cut_num"]."</td><td>".$row1["size"]."</td><td>".$size_color_qty."</td><td>".$input_job_no."</td></tr>"; 
                }             
...
$sql5="select group_concat(doc_no) as doc_no from $bai_pro3.plandoc_stat_log where order_tid in (select order_tid from $bai_pro3.bai_orders_db_confirm where order_del_no=".$schedule." ) group by acutno"; 
// echo "1=".$sql5."<br>"; 
