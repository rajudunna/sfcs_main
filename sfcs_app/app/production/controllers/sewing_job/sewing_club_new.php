...
function tableConstruction(bundet){
    ...
    if(bundet)
    {
		...
		for(var i=0;i<bundet.data.length;i++)
        {
			...
			var markup1 = "<tr class="+hidden_class+"><td data-title='schedule'>"+bundet.data[i].schedule+"</td><td data-title='cutJob'>"+bundet.data[i].cutJob+"</td><td><button type='button' id='inpjob' name='inpjob' class='btn btn-primary btn-sm' value="+bundet.data[i].inputJob+" onclick='showdet(this,"+bundet.data[i].inputJob+,+bundet.data[i].schedule+");'></button></td><td data-title='quantity'>"+bundet.data[i].quantity+"</td><td><input type='checkbox' id='club' name='club[]' value="+bundet.data[i].inputJob+","+bundet.data[i].schedule+" ></td></tr>";
