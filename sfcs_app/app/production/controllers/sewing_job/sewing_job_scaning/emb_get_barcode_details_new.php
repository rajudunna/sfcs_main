                ...
                if($b_tid[$key] == $bundle_no)
                {
                  $smv_query = "select smv,manual_smv from $brandix_bts.tbl_style_ops_master where style='$b_style' and color='$mapped_color' and operation_code = $b_op_id";
                        ...
                        if($result_select_send_qty->num_rows >0)
                        {
							...
							$check_data_qry="select * from $bai_pro3.emb_bundles where doc_no='$b_doc_num[$key]' and ops_code='$b_op_id' and size='$b_size_code[$key]'";
							...
							if($check_data_qry_result->num_rows > 0)
							{
								while($qry_result_row=mysqli_fetch_array($check_data_qry_result))
								{
									...
									$update_emb_bundles="UPDATE $bai_pro3.emb_bundles SET good_qty='$b_rep_qty[$key]',reject_qty='$b_rej_qty[$key]',update_time='". date('Y-m-d')."' where doc_no='$b_doc_num[$key]' and ops_code='$b_op_id' and size='$b_size_code[$key]'";
									$result_query = $link->query($update_emb_bundles) or exit('query error in updating emb_bundles');	
									
									//insert data into emb_bundles_temp
									$insert_emb_bundles="INSERT INTO $bai_pro3.emb_bundles_temp(tid,  doc_no,  size,    ops_code,  barcode,  quantity,  good_qty,  reject_qty,  insert_time,  update_time,  club_status,  log_user,  tran_id,  status) VALUES ('".$tid."','".$b_doc_num[$key]."','".$b_size_code[$key]."','".$b_op_id."','".$barcodeno."','".$orgqty."','".$goodqty."','".$rejectqty."','".date('Y-m-d')."','','".$clubstatus."','".$username."','".$tranid."','".$status."')";
