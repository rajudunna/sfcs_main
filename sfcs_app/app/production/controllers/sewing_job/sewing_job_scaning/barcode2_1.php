...
<?php
	...
	$html = '
			<html>
				...
				$update_psl_query = "UPDATE $bai_pro3.emb_bundles set print_status=1 where plant_code='$plant_code' and  doc_no=".$doc_no." and report_seq=".$reqseqid."";  
				$update_result = mysqli_query($link,$update_psl_query) or exit('Query Error');
				
				//$detailed_bundle_sticker=1;
				$check=0;               
                $barcode_qry="SELECT tran_id, doc_no,size, barcode,quantity, ops_code,num_id FROM $bai_pro3.emb_bundles where plant_code='$plant_code' and $query doc_no=".$doc_no." and report_seq=".$reqseqid."";
				// echo $barcode_qry;		
