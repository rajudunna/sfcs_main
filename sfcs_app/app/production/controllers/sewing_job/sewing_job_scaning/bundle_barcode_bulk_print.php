...
<div class = "panel-heading">Cut Bundle Barcode Print</div>
...
if(isset($_POST['submit']))
{
    ...
    for($s=0;$s<sizeof($s_tit);$s++)
        ...
        $sql="select *,bund.docket as docno from $bai_pro3.act_cut_bundle as bund left join $bai_pro3.order_cat_doc_mk_mix as mix on bund.docket=mix.doc_no where order_tid='".$orde_tid."' and category in ($in_categories) group by bund.docket";
        $sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
        $sql_num_check=mysqli_num_rows($sql_result);
		if($sql_num_check>0)
		{
			while($sql_row=mysqli_fetch_array($sql_result))
			{
				...
				echo "<tr><td>".chr($sql_row['color_code']).leading_zeros($sql_row['acutno'],3)."</td><td>".$sql_row['docno']."</td>"; 
				for($s=0;$s<sizeof($s_tit);$s++)
				{
				...
				// {
					// $get_print_status_qry="select print_status from $bai_pro3.emb_bundles where doc_no=".$sql_row['docno']."";
					...
					// }
						...
						// {
							echo "<td><a href=\"$path?doc_no=".$sql_row['docno']."&id=0\" onclick=\"Popup1=window.open('$path?doc_no=".$sql_row['docno']."&id=0','Popup1','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes, width=920,height=400, top=23'); if (window.focus) {Popup1.focus()} return false;\" class='btn btn-sm btn-primary'>Print with 4*2 </a></td>";
						
							echo "<td><a href=\"$path2?doc_no=".$sql_row['docno']."&id=0\" onclick=\"Popup1=window.open('$path2?doc_no=".$sql_row['docno']."&id=0','Popup1','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes, width=920,height=400, top=23'); if (window.focus) {Popup1.focus()} return false;\" class='btn btn-sm btn-primary'>Print with 2*1</a></td>";			   
							echo "</tr>";
						// }
						// else
						// {
