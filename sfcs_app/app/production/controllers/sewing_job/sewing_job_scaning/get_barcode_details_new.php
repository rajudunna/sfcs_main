                       ...
                       while($row23 = $result_pre_operation_check->fetch_assoc()) 
                       {
                        ...
                        if($operation_code == 100 || $operation_code == 129)
                        {
                            ...
                            if($result_searching_query_in_imslog->num_rows > 0)
                            {
                               ...
                               //For 900 Operation
                                $get_qty_details1="select sum(if(operation_id = $operation_code,recevied_qty,0)) as input,sum(if(operation_id = $output_ops_code,recevied_qty,0)) as output,sum(if(operation_id = $output_ops_code,rejected_qty,0)) as output_rej From $brandix_bts.bundle_creation_data where  bundle_number=$b_tid[$i]";
