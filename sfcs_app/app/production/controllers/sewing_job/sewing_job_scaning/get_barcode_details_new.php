                            ...
                            if($result_select_send_qty->num_rows >0)
                            {
                                ...
                                if($result_query)
                                {
                                    if($b_rep_qty[$key] > 0)
                                    {
                                        ...
                                        if($gate_pass_no>0)
                                        {
                                            $sql_gate="insert into $brandix_bts.`gatepass_track` (`gate_id`, `bundle_no`, `bundle_qty`, `style`, `schedule`, `color`, `size`,operation_id,plant_code,created_user) values ('".$gate_pass_no."', ".$b_tid[$key].", '".$previously_scanned."', '".$b_style."','".$b_schedule."','".$b_colors[$key]."','".$b_sizes[$key]."','".$b_op_id."-4','".$plant_code."','".$username."')";
                                            $result_sql_temp = $link->query($sql_gate) or exit('Gate_pass_child query error in updating');
