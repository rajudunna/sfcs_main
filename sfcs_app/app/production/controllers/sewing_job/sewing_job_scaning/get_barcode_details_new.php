                ...
                if($b_tid[$i] == $bundle_no)
                {
                  //  if($b_op_id == 100 || $b_op_id == 129)
                    ...
                    if($hout_ops_result->num_rows > 0)
                    {
                        ...
                        {
                            $hout_insert_qry = "insert into $pps.hout2(out_date, out_time, team, qty, status, remarks, rep_start_time, rep_end_time,
                             time_parent_id, style,color,smv,bcd_idplant_code,created_user,created_at)
                             values('$tod_date','$cur_hour','$b_module[$i]','$b_rep_qty[$i]', '1', 'NA', '$plant_start_timing', '$plant_end_timing', '$plant_time_id'
                             ,'$b_style','$b_colors[$i]','$smv','$b_tid[$i]','$plantcode','$username','".date('Y-m-d').")";
                            $hout_insert_result = $link->query($hout_insert_qry);                       
                        }
                    }
