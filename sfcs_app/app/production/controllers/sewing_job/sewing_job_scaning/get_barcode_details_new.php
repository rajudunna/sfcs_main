                        ...
                        if($result_select_send_qty->num_rows >0)
                        {
                            ...
                            if($result_query)
                            {
                                if($b_rep_qty[$key] > 0)
                                {
									...
									if($gate_pass_no>0)
									{
										$sql_gate="insert into $brandix_bts.`gatepass_track` (`gate_id`, `bundle_no`, `bundle_qty`, `style`, `schedule`, `color`, `size`,operation_id) values ('".$gate_pass_no."', ".$b_tid[$key].", '".$b_rep_qty[$key]."', '".$b_style."','".$b_schedule."','".$b_colors[$key]."','".$b_size_code[$key]."','".$b_op_id."')";
                    ...
                    foreach($pre_ops_code_temp as $index => $op_code)
                    {
                        if($op_code != $b_op_id)
                        {
                            ...
                            //change values here in query....
								...
								if($gate_pass_no>0)
								{
									$sql_gate="insert into $brandix_bts.`gatepass_track` (`gate_id`, `bundle_no`, `bundle_qty`, `style`, `schedule`, `color`, `size`,operation_id) values ('".$gate_pass_no."', ".$b_tid[$key].", '".$send_qty."', '".$b_style."','".$b_schedule."','".$b_colors[$key]."','".$b_size_code[$key]."','".$b_op_id."')";
									$result_sql_temp = $link->query($sql_gate) or exit('Gate_pass_child query error in updating');								
								}								
                            ...
                            if($result_select_send_qty->num_rows >0)
                            {
                                ...
                                if($result_query)
                                {
                                    if($b_rep_qty[$key] > 0)
                                    {
										...
										if($gate_pass_no>0)
										{
											$sql_gate="insert into $brandix_bts.`gatepass_track` (`gate_id`, `bundle_no`, `bundle_qty`, `style`, `schedule`, `color`, `size`,operation_id) values ('".$gate_pass_no."', ".$b_tid[$key].", '".$previously_scanned."', '".$b_style."','".$b_schedule."','".$b_colors[$key]."','".$b_size_code[$key]."','".$b_op_id."')";
											$result_sql_temp = $link->query($sql_gate) or exit('Gate_pass_child query error in updating');
