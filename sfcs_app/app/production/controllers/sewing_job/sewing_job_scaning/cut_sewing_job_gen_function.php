...
{    
	...
	if(mysqli_num_rows($sql_result_new) == 0)
	{
		...
		if(mysqli_num_rows($sql_result_new11) == 0)
		{
			...
			$cps_qry= "select * from $bai_pro3.plan_cut_bundle where style='$style' and schedule='$schedule' and color='$color'";
			...
			if($new_logic){
				$sewing_bundle_generation = plan_logical_bundles($docs,$jobcount,$bundle_qty,$inserted_id,$schedule,$cuts);
			} else {
...
{	
	...
	if($eligible_to_generate == 1){
		...
		{	
			...
			foreach($remaval_dockets as $key => $docket_no)
			{
				if(in_array($docket_no,$doc_list_new))
				{
					...
					$plan_ids1=array_values(array_unique($plan_ids));					
					if($excess_cut == 1){
						$old_jobs_count_qry = "SELECT MAX(CAST(input_job_no AS DECIMAL))+1 as result FROM $bai_pro3.pac_stat_log_input_job WHERE input_job_no_random LIKE '%".$schedule."%'";
						// echo $old_jobs_count_qry;
						$old_jobs_count_res = mysqli_query($link, $old_jobs_count_qry) or exit("Issue while Selecting SPB".mysqli_error($GLOBALS["___mysqli_ston"]));
						...
						{				
							...
							for($jj=0;$jj<sizeof($plan_ids);$jj++)
							{
								...
								{
									$sql_cps_query="select * from $bai_pro3.plan_cut_bundle where doc_no=$docket_no and id=$plan_ids[$jj]";
									$sql_resutlt_cps=mysqli_query($link, $sql_cps_query) or exit("Issue in Inserting SPB.".mysqli_error($GLOBALS["___mysqli_ston"]));
									if(mysqli_num_rows($sql_resutlt_cps)>0){
		...
		foreach($remaval_dockets as $key => $docket_no)
		{
			...
			if(in_array($docket_no,$doc_list_new))
			{
				// var_dump($plan_ids1.'plan_ids1<br/>');
				// var_dump($docket_no,'excess_docket<br/>');
				if($excess_cut == 2 && $excess > 0){
					// echo 'excess2';
					//get input job number for each schedule
					...
					{				
						...
						$input_job_no=$input_job_num;
						$input_job_num_rand=$schedule.date("ymd").$input_job_no;
						for($jj=0;$jj<sizeof($plan_ids1);$jj++)
						{
							// var_dump($fill_qty[$plan_ids1[$jj]][$j],'fill_qtyt<br/>');
							if($fill_qty[$plan_ids1[$jj]][$j]>0)
							{
								$sql_cps_query="select * from $bai_pro3.plan_cut_bundle where doc_no=$docket_no and id=$plan_ids1[$jj]";
								$sql_resutlt_cps=mysqli_query($link, $sql_cps_query) or exit("Issue in Inserting SPB.".mysqli_error($GLOBALS["___mysqli_ston"]));
								if(mysqli_num_rows($sql_resutlt_cps)>0){
									$barcode="SPB-".$docket_no."-".$input_job_no."-".$bundle_seq."";
									//Plan Logical Bundle				
									$ins_qry =  "INSERT INTO `bai_pro3`.`pac_stat_log_input_job`(doc_no,size_code,carton_act_qty,input_job_no,input_job_no_random,destination,packing_mode,old_size,doc_type,pac_seq_no,sref_id,plan_cut_bundle_id,barcode_sequence,tran_user,barcode,style,color,schedule,tran_ts,type_of_sewing)VALUES(".$docket_no.", '".$sizes[$plan_ids1[$jj]]."', ".$fill_qty[$plan_ids1[$jj]][$j].", '".$input_job_no."', '".$input_job_num_rand."', '".$destination."', 1, '".$size_codes[$plan_ids1[$jj]]."','N', '-1', $inserted_id, $plan_ids1[$jj],$bundle_seq,'".$username."','".$barcode."','".$style."','".$color."','".$schedule."','".date('Y-m-d H:i:s')."',$j)";
									//echo $ins_qry."<br>";
									$result_ins_qry=mysqli_query($link, $ins_qry) or exit("Issue in Inserting SPB".mysqli_error($GLOBALS["___mysqli_ston"]));
									$pac_tid= mysqli_insert_id($link);
									foreach($operation_codes as $index => $op_code)
									{
										$send_qty = 0;
										if($index == 0) {
											$send_qty = $fill_qty[$plan_ids1[$jj]][$j];
										}
										//Plan Logical Bundle Trn
										$b_query = "INSERT  INTO $brandix_bts.bundle_creation_data(`style`,`schedule`,`color`,`size_id`,`size_title`,`sfcs_smv`,`bundle_number`,`original_qty`,`send_qty`,`recevied_qty`,`rejected_qty`,`left_over`,`operation_id`,`docket_number`, `scanned_date`, `scanned_user`, `cut_number`, `input_job_no`,`input_job_no_random_ref`, `shift`, `assigned_module`, `remarks`, `mapped_color`,`barcode_sequence`,`barcode_number`) VALUES ('".$style."','". $schedule."','".$color."','". $size_codes[$plan_ids1[$jj]]."','".$sizes[$plan_ids1[$jj]]."','". $smv[$op_code]."',".$pac_tid.",".$fill_qty[$plan_ids1[$jj]][$j].",".$send_qty.",0,0,0,".$op_code.",'".$docket_no."','".date('Y-m-d H:i:s')."', '".$username."','".$docket_cut[$docket_no]."','".$input_job_num."','".$input_job_num_rand."','','','Normal','".$color."',".$bundle_seq.",'".$barcode."')";
										mysqli_query($link, $b_query) or exit("Issue in inserting BCD".mysqli_error($GLOBALS["___mysqli_ston"]));
									}
									$count++;
									$bundle_seq++;	
