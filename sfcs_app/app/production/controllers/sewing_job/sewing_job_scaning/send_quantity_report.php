...
?>
<form method ='POST' action='index.php?r=<?= $_GET['r']; ?>'>
<div class='container'>
...
if(isset($_POST['SUBMIT']))
{
	...
	if($data_insert){
		header("Location:".getFullURLLevel($_GET['r'],'send_quantity_report.php',0,'N')."&Message=true");
	}else {
		header("Location:".getFullURLLevel($_GET['r'],'send_quantity_report.php',0,'N')."&Message=false");
	}
}
...
$(document).ready(function(){
	...
	$("#color").change(function()
	{
		...
		$.ajax
			({
					type: "POST",
					url:"<?= getFullURLLevel($_GET['r'],'functions_send.php',0,'R'); ?>?r=getdata&color="+params_cut,
					dataType: 'Json',
	...
	$("#get_details").click(function()
	{
		...
		if(flag==0)
		{
			...
			$.ajax
				({
						type: "POST",
						url:"<?= getFullURLLevel($_GET['r'],'functions_send.php',0,'R'); ?>?r=getdata&params="+params,
						dataType: 'Json',
