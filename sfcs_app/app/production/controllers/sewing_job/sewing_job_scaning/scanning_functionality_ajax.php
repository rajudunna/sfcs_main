		...
		if($table_name == 'bundle_creation_data')
		{
			...
			foreach ($b_tid as $key => $tid) 
			{
				if($b_rep_qty[$key] > 0 || $b_rej_qty[$key] > 0)
				{
					...
					{
						$query_post = "UPDATE $brandix_bts.bundle_creation_data SET `send_qty` = '".$final_rep_qty."' where bundle_number =$b_tid[$key] and operation_id = ".$post_ops_code;
			...
			if($b_rep_qty[$i] > 0)
			{
			   ...
			   // echo $checking_output_ops_code_out;
				...
				if($b_op_id == $output_ops_code_out)
				{
					...
					) values ('".$b_module[$i]."','".$sec_head."','".$b_rep_qty[$i]."','".date("Y-m-d H:i:s")."','".$bac_dat."','".$b_shift."','".$b_style."','Active','".$log_time."','".$buyer_div."','".$b_schedule."','".$b_colors[$i]."',USER(),'".$b_doc_num[$i]."','".$sfcs_smv."','".$b_rep_qty[$i]."','ims_log','".$b_op_id."','".$nop."','".$bundle_op_id."','".$b_op_id."','".$b_inp_job_ref[$i]."')";
					...
					if($qry_status)
					{
						...
						bac_shift,bac_style,bac_stat,log_time,buyer,delivery,color,loguser,ims_doc_no,smv,".$sizevalue.",ims_table_name,ims_tid,nop,ims_pro_ref,ope_code,jobno
						) values ('".$b_module[$i]."','".$sec_head."','".$b_rep_qty[$i]."','".date("Y-m-d H:i:s")."','".$bac_dat."','".$b_shift."','".$b_style."','Active','".$log_time."','".$buyer_div."','".$b_schedule."','".$b_colors[$i]."',USER(),'".$b_doc_num[$i]."','".$sfcs_smv."','".$b_rep_qty[$i]."','ims_log','".$b_op_id."','".$nop."','".$b_op_id."','".$b_op_id."','".$b_inp_job_ref[$i]."')";
