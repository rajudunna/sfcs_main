...
// echo 'Seaaion[Form_Secret]=='.$_SESSION["form_secret"].'-';
// if(isset($_SESSION["form_secret"])) 
{
	// if(strcasecmp($form_secret, $_SESSION["form_secret"]) === 0) 
	{
		...
		if(isset($_POST['date']))
		{
			...
			$note="";
			// echo "<script>alert('into date condition');</script>";
			// echo $dat.'-'.$mno_x.'-'.$mst_x;
			// echo "<br><br>";
			// var_dump($mno_x);
			// echo "<br><br>";
			// var_dump($mst_x);
				...
				{					
					...
					{		
						...
						if($rew_qty>=0 and $rew_qty!="")
						{
							...
							$sql="insert into $bai_pro.bai_quality_log (bac_no, bac_sec, bac_qty, bac_lastup, bac_date, bac_shift, bac_style, bac_remarks,  log_time, color, buyer, delivery, loguser) values ($mno, $sec, $rew_qty, \"$dat\", \"$dat\", \"$msh\", \"$mst\", \"$rem\",  \"$ldate\",  \"$color\", \"$buyer\", \"$schedule\",\"$username\")"; 
							// echo $sql."<br/>";
							// die();
							$note.=$sql."<br/>";
	...
	{
		//Invalid secret key
		// echo "Invalid Secret Key. Please dont try to cheat the system.";
	}
}
// else 
{
