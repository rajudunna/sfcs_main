...
//generate_bundles('QCIFG001','426935','BLACK',1); 
		...
		if($bundle_cnt_res->cnt==0){
			...
			$plandoc_stat_log_qry="SELECT * FROM $bai_pro3.order_cat_doc_mix WHERE TRIM(order_style_no)='$style' AND TRIM(order_del_no)='$schedule' AND TRIM(order_col_des)='$color' and acutno=$cut_num and category in ($category)";
