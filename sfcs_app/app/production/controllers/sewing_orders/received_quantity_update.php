...
$result_rejections = $conn->query($qery_rejection_resons); 
...
$result_parent_selecing= $conn->query($parent_selecing);
...
$result_checking_main_query= $conn->query($checking_main_query);
...
foreach ($reson_desc as $key => $value)
{
	if($key != 0)
	{
		...
		$result1 = $conn->query($checking_qry);
		...
		if($count >= 0)
		{
			...
			$executed = $conn->query($inserting_query);
		}
		$tot_qty += $qty[$key];
		...
		//echo $sfcs_reason;
		$result_sfcs_reason = $conn->query($sfcs_reason);
		...
		$query_bundle_cre_data = "select * from bundle_creation_data where style ='$style' and schedule ='$scheule' and color=$color and cut_number=$cut and size_title='$size'";
...
if ($conn->connect_error) { 
	...
	foreach ($updatable_id as $key => $value)
	{
			...
			$result_edit_next_send_sew_qty = $conn->query($edit_next_send_sew_qty);
			...
			$result_checking_qry_ops_code = $conn->query($checking_qry_ops_code);
 ...
 getschedule();
	...
	function getcut()
	{
		...
		$.ajax
			({
					type: "POST",
					url:"functions_send.php?r=getdata&colors="+color,
					dataType: 'Json',
	...
	function getops()
	{
		...
		$.ajax
			({
					type: "POST",
					url:"functions_send.php?r=getdata&params1="+params,
					dataType: 'Json',
	...
	function get_full_data()
	{
		...
		$.ajax
			({
					type: "POST",
					url:"functions_send.php?r=getdata&act_params="+act_params,
					dataType: 'Json',
