...
$sql="select coalesce((a_xs+a_s+a_m+a_l+a_xl+a_xxl+a_xxxl)*p_plies,0) as \"pilot\" from 
       $bai_pro3.plandoc_stat_log where order_tid=\"$order_tid\" and remarks=\"Pilot\""; //20110911
...
mysqli_query($link, $sql) or exit("Sql Errorccc".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	for($i=0;$i<sizeof($order_qtys);$i++)
  	{	
		if($order_qtys[$i]>0)
		{
			...
			$carton_nodes=array();
			$x=1;
			$sql="select * from $bai_pro3.pac_stat_log where doc_no in (".implode(",",$docs_db).") and size_code=\"".strtolower($size_titles_qry[$i])."\" order by doc_no, carton_mode desc";
			//echo $sql;
			...
			while($sql_row=mysqli_fetch_array($sql_result))
			{
				...
				if($bgcolor=="RED")
				{
					...
					$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error15".mysqli_error($GLOBALS["___mysqli_ston"]));
...
$sql="select * from (select packing_summary.*, sum(carton_act_qty) as \"carton_qty\" from 
      $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=\"$schedule\" and container is null group by doc_no_ref) as t order by carton_qty ASC";
//echo $sql.'<br>';
mysqli_query($link, $sql) or exit("Sql Error16".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_result=mysqli_query($link, $sql) or exit("Sql Error17".mysqli_error($GLOBALS["___mysqli_ston"]));
while($sql_row=mysqli_fetch_array($sql_result))
{
...
else
{
	...
	//echo $sql;
			...
			if(sizeof($temp_doc_ref)>0)
			{
				...
				//echo $sql."<br/>";
				mysqli_query($link, $sql) or exit("Sql Error22".mysqli_error($GLOBALS["___mysqli_ston"]));
				$container++;
			}
			...
			$max_size_code="";
			$sql="select packing_summary.size_code as \"size_code\", sum(carton_act_qty) as \"carton_qty\" from $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=\"$schedule\" and container is null group by doc_no_ref";
			//echo $sql."<br/>";
...
<!--<table class="new">
	...
	--?  <?php
	   /* $sql="select container,group_concat(tid) as \"label_id\", count(*) as \"count\" from packing_summary where order_style_no=\"$style\" and order_del_no=\"$schedule\" group by container";
		mysql_query($sql,$link) or exit("Sql Error".mysql_error());
		$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
		$total=0;
		while($sql_row=mysql_fetch_array($sql_result))
