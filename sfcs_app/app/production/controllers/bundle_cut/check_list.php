...
if(isset($_POST['submit']))
{
	...
	if($check_club>0)
	{
		...
		while($m=mysqli_fetch_array($sql_result1))
		{
			...
			*/
			$sql="select color,mini_order_num,count(*) as bun,sum(quantity) as qty from tbl_miniorder_data where docket_number in (".implode(",",$doc_no).") ";
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	else
	{
		...
		$sql1="select color,docket_number,cut_num,mini_order_num,count(*) as bundles,sum(quantity) as qty from tbl_miniorder_data where mini_order_ref=\"$miniord_ref\" and SUBSTRING_INDEX(mini_order_num,'.',-1)=0 group by mini_order_num";
		$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
		//echo $sql."<br>";
		$rowcount=1;
		echo '<table border="1" border="1px">';
