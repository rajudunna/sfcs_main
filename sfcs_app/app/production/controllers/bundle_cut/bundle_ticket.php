...
<div class="panel-heading">Bundle Tag Generation</div>
...
?>
 ...
 <?php
echo "<div class='col-md-3 col-sm-3 col-xs-12' style='margin-top: 18px;'>";
...
if(isset($_POST['submit']))
{
	...
	if($check_club>0)
	{
		...
		while($m=mysqli_fetch_array($sql_result1))
		{
			...
			$sql="select color,mini_order_num,count(*) as bundles,sum(quantity) as qty from $brandix_bts.tbl_miniorder_data where docket_number in (".implode(",",$doc_no).") ";
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($row=mysqli_fetch_array($sql_result))
			{
				...
				if($docket_status>0)
				{
					...
					else
					{
						...
						echo "<td><a href='$url&cut_num=".$m['cut_num']."&mini_order_num=".$row['mini_order_num']."&doc_no=".$docket."&ops=2' target='_blank'>Print</a></td></tr>";
					}
				}
				else
				{
	...
	else
	{
		...
		$sql1="select mini_order_num,color,cut_num,docket_number,count(*) as bundles,sum(quantity) as qty from $brandix_bts.tbl_miniorder_data where mini_order_ref=\"$miniord_ref\" group by mini_order_num";
		$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
		//echo $sql."<br>";
		$rowcount=1;
		echo '<table class="table table-bordered">';
		...
		while($m=mysqli_fetch_array($sql_result1))
		{
			...
			if($docket_status>0)
			{
				...
				{
					$url=getFullURLLevel($_GET['r'],'common/lib/mpdf7/bundle_tag_gen.php',4,'N');
					echo "<td><a href='$url&mini_order_num=".$m['mini_order_num']."&mini_order_ref=".$miniord_ref."&ops=1' target='_blank'>Print</a></td></tr>";
				}
			}
			else
			{
