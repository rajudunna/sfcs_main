...
{
	//alert("report");
	window.location.href ="../mini_order_report/mini_order_priority.php?style="+document.mini_order_report.style.value
}
...
<body>
<div id="page_heading"><span style="float: left"><h3>Mini Order Scheduling</h3></span><span style="float: right"><b>?</b>&nbsp;</span></div>
<?php 
...
if(isset($_POST['submit']))
{
	...
	if($type=='1')
	{		
		$check_trim=echo_title("bai_pro3.trims_dashboard","max(tid)","mini_ord_ref",$mini_order_ref,$link);
		//echo $check_trim."<bR>";
		if($check_trim == 0)
		{
			if($bundles>0)
			{
				$sql="select mini_order_num,min(docket_number) from tbl_miniorder_data where mini_order_ref='".$mini_order_ref."' group by mini_order_num";
				$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
				echo '<form name="input" method="post" action="mini_order_priority.php" onsubmit="return check_val1();">';
				echo "<table border=1px><tr><th>Style</th><th>Schedule</th><th>Module</th><th>Plan Date</th><th>Control</th></tr>";
				...
				{
					echo "<td><input type=\"text\" name='module' value='' ></td>";
				}
				echo "<td><input id=\"plandate\" onclick=\"javascript:NewCssCal('plandate','yyyymmdd','dropdown')\" onclick=\"javascript:NewCssCal('plandate','yyyymmdd','dropdown')\" type=\"text\" class='textbox' name=\"plandate\" size=8 value=\"".$cut_date_ref."\">";
				...
				echo "</select>";
				echo "</td><td><input type=\"submit\" value=\"Allocate\" name=\"schedule_save\">";
				echo "</td></tr>";
				echo "</table>";
				echo "</form>";
			}
	...
	else
	{
		...
		if($bundles>0)
		{
			$sql="select mini_order_num from tbl_miniorder_data where mini_order_ref='".$mini_order_ref."' and  planned_module<>0 group by mini_order_num order by mini_order_num";
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			echo '<form name="input" method="post" action="mini_order_priority.php" onsubmit=" return check_val1();">';
			$j=0;
			if(mysqli_num_rows($sql_result)>0 or in_array($username,$authorized_to_modify))
			{
				...
				{
					$sql="select mini_order_num from tbl_miniorder_data where mini_order_ref='".$mini_order_ref."' group by mini_order_num order by mini_order_num";
					$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));	
				}
				echo "<table border=1px><tr><th>Style</th><th>Schedule</th><th>Mini Order</th><th>Allocate modules</th><th>Planned Date & Time</th></tr>";
				echo "<input type=\"hidden\" value=\"$mini_order_ref\" name=\"mini_order_ref\">";
				while($row=mysqli_fetch_array($sql_result))
				{
					...
					$sql1="select planned_module from tbl_miniorder_data where planned_module<>0 and mini_order_ref='".$mini_order_ref."' and mini_order_num=".$row['mini_order_num']." group by planned_module order by planned_module";
					//echo $sql1."<br>";
					$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
					$rows=mysqli_num_rows($sql_result1);
					if($rows==0 && in_array($username,$authorized_to_modify))
					{
						echo "<td><input type=\"text\" name='module[".$j."]' value='$module' ></td>";
					}
					else
					{
						echo "<td>";
					...
					if($check_trim>0)
					{
						...
						if(in_array($username,$authorized_to_modify))
						{
							...
							echo "<td><input size=\"20\" id=\"plan_d".$i."\" onclick=\"javascript:NewCssCal('plan_d".$i."','yyyymmdd','dropdown')\" onclick=\"javascript:NewCssCal('plan_d".$i."','yyyymmdd','dropdown')\" type=\"text\" class='textbox' name=\"plan_d[".$i."]\" size=8 value=\"".$plan_time."\">";
							echo "<select name='hrs[$i]'>";
							for($ii=0;$ii<sizeof($hrs);$ii++)
							{
								//echo $hour."--".$hrs[$ii]."<br>";
								if($hour == $hrs[$ii])
					...
					{
						echo "<td><input size=\"20\" id=\"plan_d".$i."\" onclick=\"javascript:NewCssCal('plan_d".$i."','yyyymmdd','dropdown')\" onclick=\"javascript:NewCssCal('plan_d".$i."','yyyymmdd','dropdown')\" type=\"text\" class='textbox' name=\"plan_d[".$i."]\" size=8 value=\"\">";
						echo "<select name='hrs[$i]'>";
						for($ii=0;$ii<sizeof($hrs);$ii++)
						{
							echo "<option value='".$hrs[$ii]."'>".$hrs[$ii]."</option>";
				...
				echo "<input type=\"submit\" value=\"Allocate\" name=\"save\">";
			}
			else
			{
...
if(isset($_POST['schedule_save']))
{
	...
	//{
		$sql="SELECT mini_order_num,min(planned_module) AS module FROM tbl_miniorder_data WHERE mini_order_ref='".$mini_order_ref."' group by mini_order_num";
		//echo $sql."<br>";
