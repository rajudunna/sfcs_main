...
{
	//alert("report");
	var ajax_url ="../mini_order_report/bundle_ticket.php?style="+document.mini_order_report.style.value;
	Ajaxify(ajax_url);
...
{
	//alert('test');
	var ajax_url ="../mini_order_report/bundle_ticket.php?style="+document.mini_order_report.style.value+"&schedule="+document.mini_order_report.schedule.value;
	Ajaxify(ajax_url);
...
if(isset($_POST['submit']))
{
	...
	if($check_club>0)
	{
		...
		while($m=mysqli_fetch_array($sql_result1))
		{
			...
			$sql="select color,mini_order_num,count(*) as bundles from tbl_miniorder_data where docket_number in (".implode(",",$doc_no).") ";
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($row=mysqli_fetch_array($sql_result))
			{
				...
				if($docket_status>0)
				{
					...
					$ticket_status = echo_title("brandix_bts.tbl_miniorder_data","count(*)","docket_number IN (".implode(",",$doc_no).") and mini_order_num=".$m['mini_order_num']." and mini_order_status ",1,$link);
					if($ticket_status>0)
					{
					...
					{
						echo "<td><a href='..\..\packing\labels\mpdf50\\examples\bundle_tag_gen.php?cut_num=".$m['cut_num']."&mini_order_num=".$m['mini_order_num']."&doc_no=".$docket."&ops=2' target='_blank'>Print</a></td></tr>";
					}
				}
				else
				{
	...
	else
	{
		...
		$sql1="select mini_order_num,color,cut_num,docket_number,count(*) as bundles from tbl_miniorder_data where mini_order_ref=\"$miniord_ref\" group by mini_order_num";
		$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
		//echo $sql."<br>";
		$rowcount=1;
		echo '<table border="1" border="1px">';
