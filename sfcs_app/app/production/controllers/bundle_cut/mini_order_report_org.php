...
if(isset($_POST['submit']))
{
	...
	echo "<right><strong><a href=\"export_excel.php?style=$style_code&schedule=$schedule&flag=mini_orders\">Export to Excel</a></strong></right>";			
	//getting carton qty
	...
	$sql="select group_concat(distinct(ref_size_name) order by ref_size_name) as size_id,group_concat(distinct(size_title) order by size_title) as size_name from tbl_orders_sizes_master where parent_id in (SELECT id FROM `tbl_orders_master` WHERE ref_product_style =$style_code and `product_schedule`=$schedule_result) ORDER BY `ref_size_name` ASC";
	//echo $sql."<br>";
	$sql_result=mysqli_query($link, $sql) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	$rowcount = "SELECT md.mini_order_ref,md.mini_order_num,md.color,sum(md.quantity) as Qnty,";
	...
	if($count>0)
	{
		...
		while($r=mysqli_fetch_array($sql_result1))
		{
			...
			if($miniorder!=$old_miniorder)
			{
				echo '<div style="background-color:#ffffff;color:#000000;border: 1px solid #000000; float: left; margin: 5px; padding: 5px;">';
				echo "<table width=\"350\" height=\"75\" border=\"1px\" >";
