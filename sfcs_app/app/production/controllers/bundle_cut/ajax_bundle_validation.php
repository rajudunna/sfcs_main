...
function bundle_quantity($bundle_barcode,$link)
{
	...
	$docket=echo_title("$brandix_bts.tbl_miniorder_data","docket_number","bundle_number",$bundle_id,$link);
	
	$schedule=echo_title("$brandix_bts.tbl_cut_master","product_schedule","doc_num",$docket,$link);
	...
	$mini_ref=echo_title("$brandix_bts.tbl_miniorder_data","mini_order_ref","bundle_number",$bundle_id,$link);
...
//	}
	...
	if($docket<>'')
	{
		if($operation_id==1)
		{
			$bundle_size=echo_title("$brandix_bts.tbl_miniorder_data","size","bundle_number",$bundle_id,$link);
			$color=echo_title("$brandix_bts.tbl_miniorder_data","color","bundle_number",$bundle_id,$link);
			$act_size=strtolower(echo_title("$brandix_bts.tbl_orders_size_ref","size_name","id",$bundle_size,$link));
			if($check_doc == 'R')
			{
				$doc_id=substr($docket,1,10);
				//$docket_status=echo_title("bai_pro3.recut_v2","act_cut_status","doc_no",substr($doc_id,1,10),$link);
				$sql12="select ((a_".$act_size.")*a_plies) as sum_qty from $bai_pro3.recut_v2 where act_cut_status='DONE' and doc_no='".$doc_id."'";
				//echo $sql12."<br>";
			...
			else
			{
				...
				$doc_id=$docket;
				//$docket_status=echo_title("bai_pro3.plandoc_stat_log","act_cut_status","doc_no",$doc_id,$link);
				$sql12="select ((a_".$act_size.")*a_plies) as sum_qty from $bai_pro3.plandoc_stat_log where act_cut_status='DONE' and doc_no='".$doc_id."'";
				//echo $sql12."<br>";
				$sql_result12=mysqli_query($link, $sql12) or exit("Sql Error-1".mysqli_error($GLOBALS["___mysqli_ston"]));
				while($result12=mysqli_fetch_array($sql_result12))
				{
			...
			//$quantity_act=25;
			$sql="select bundle_number from $brandix_bts.tbl_miniorder_data where color='".$color."' and docket_number='".$docket."' and size='".$bundle_size."'";
			...
			$quantity_n=echo_title("$brandix_bts.tbl_miniorder_data","sum(quantity)","mini_order_ref=$mini_ref and bundle_number",$bundle_id,$link);
	...
	$bundle_ops_cnt=echo_title("$brandix_bts.bundle_transactions_20_repeat","count(*)","bundle_id",$bundle_id,$link);
	
	$order_id=echo_title("$brandix_bts.tbl_orders_ops_ref","ops_order","id",$operation_id,$link);
	...
	if(1 < $operation_id)
	{
		$barcode_p=echo_title("$brandix_bts.bundle_transactions_20_repeat","count(*)","operation_id='".$ops_p."' and bundle_id",$bundle_id,$link);
	}
	else
	{
		$barcode_p=100;
				...
				//{
					if(0 < $barcode_p)
					{
							...
							else
							{
								if($operation_id==1)
								{
									$barcode_qty=echo_title("$brandix_bts.tbl_miniorder_data","sum(quantity)","mini_order_ref=$mini_ref and bundle_number",$bundle_id,$link);
									$ret_val=$barcode_qty;
									return $ret_val;
								}
								/*
								...
								*/
								...
								{
									$barcode_qty=echo_title("$brandix_bts.bundle_transactions_20_repeat","quantity","operation_id=$ops_p and bundle_id",$bundle_id,$link);
									$ret_val=$barcode_qty;
									return $ret_val;
								}
							}
