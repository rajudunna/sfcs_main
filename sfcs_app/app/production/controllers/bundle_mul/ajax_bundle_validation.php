...
function bundle_quantity($bundle_barcode,$link)
{
	...
	if($docket<>'')
	{
		if($operation_id==1)
		{
			if($check_doc == 'R')
			{
				$docket_status=echo_title("bai_pro3.act_cut_status_recut_v2","count(*)","doc_no",substr($docket,1,20),$link);
			}
			else
			{
			...
			$color=echo_title("tbl_miniorder_data","color","bundle_number",$bundle_id,$link);
