...
if(isset($_POST['save']))
{
	...
	*/
	...
	for($i=1;$i<=sizeof($color);$i++)
	{
		...
		//$color=$color[$i];
		$size=$size_id[$i];
		$size_title_query="select size_title from tbl_orders_sizes_master where order_col_des='".$color[$i]."' and ref_size_name=$size and parent_id in(select id from tbl_orders_master where ref_product_style=$style_id and product_schedule='$schedule') limit 0,1";
		...
		for($k=0;$k<sizeof($lines_select)-1;$k++)
		{
			...
			if($total[$i]<=$quantity[$i])
			{
				...
				if($bundle_update_count!='' or $bundle_update_count!=0)
				{
					...
					$get_bundle_query="select bundle_number from tbl_miniorder_data where color='".$color[$i]."' and size=$size and mini_order_num=$mini_order_num and mini_order_ref=$mini_order_ref and (planned_module=0 or planned_module is NULL) order by bundle_number limit $bundle_update_count";
					...
					while($b = mysqli_fetch_array($bundle_result))
					{
						...
						$update_module="update tbl_miniorder_data set planned_module=$line_num where bundle_number=$bundle_num";
...
*/
	echo "</table>";
	echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",180); function Redirect() {  location.href = \"bundle_allocation.php?color_code=$color_code&color_code=$color_code&style_id=$style_id&sch_id=$schedule_id&mini_order_num=$mini_order_num&mini_order_ref=$mini_order_ref&ops\"; }</script>";
...
if(isset($_POST['update']))
{
	...
	*/	
	...
	for($i=1;$i<=sizeof($color);$i++)
	{
		...
		$module_update="update tbl_miniorder_data set planned_module=0 where color='".$color[$i]."' and size='".$size."' and mini_order_num='".$mini_order_num."' and mini_order_ref='".$mini_order_ref."'";
	...
	echo "</table>";
	echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",180); function Redirect() {  location.href = \"bundle_allocation.php?color_code=$color_code&style_id=$style_id&sch_id=$schedule_id&mini_order_num=$mini_order_num&mini_order_ref=$mini_order_ref&ops\"; }</script>";
}
?>
<style>
