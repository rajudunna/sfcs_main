...
$view_access=user_acl("SFCS_0244",$username,1,$group_id_sfcs); 
...
if(isset($_POST['submit']))
{
	...
	while($row=mysqli_fetch_array($sql_result))
	{
		...
		$sql1="select bundle_number from tbl_miniorder_data where mini_order_ref='".$miniord_ref."' and mini_order_num='".$row['mini_order_num']."'";
		...
		$sql2="select count(*) as cnt from brandix_bts.bundle_transactions_20_repeat where bundle_id in ('".implode("','",$scan_bundle)."') group by bundle_barcode";
		$sql_result2=mysqli_query($link2, $sql2) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		...
		$diff=$row['bundles']-$scan_total;
		echo "<td>".$stylecode."</td><td>".$schedule_result."</td><td>".$row['mini_order_num']."</td><td>".$row['bundles']."</td><td>".$scan_total."</td><td>".$diff."</td>";
