    ...
    if($doc_no > 0){
        ...
        if(mysqli_num_rows(mysqli_query($link,$doc_query)) > 0){
            ...
            if($row['org_doc_no'] > 0)
                ...
                $verify_scan_query = "SELECT id from $brandix_bts.bundle_creation_data where docket_number = $doc_no and recevied_qty > 0 and operation_id IN ($SEWIN,$SEWOUT) limit 1";                    
