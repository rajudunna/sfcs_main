...
if($count == 0){
	$sql1="insert INTO $pts.pro_atten_hours (date,shift,start_time,end_time,plant_code,created_user,created_at) VALUES ('".$date."','".$shift."','".$shift_start_time."','".$shift_end_time."','$plantcode','$username','".date('Y-m-d')."')";
...
for($i=0;$i<sizeof($modules_array);$i++)
{
	...
	$sqla="Select * from $pts.pro_attendance where plant_code='$plantcode' and date=\"$date\" and module=\"$modules_array[$i]\" and shift='".$shift."'";
	// echo $sqla."</br>";
	$sqlresa=mysqli_query($link, $sqla) or exit("Sql Errord $sql1".mysqli_error($GLOBALS["___mysqli_ston"]));
	if(mysqli_num_rows($sqlresa)==0)
	{
		$sql1="INSERT INTO $pts.pro_attendance (date,module,shift,plant_code,created_user,created_at) VALUES ('".$date."','$modules_array[$i]','".$shift."','$plantcode','$username','".date('Y-m-d')."')";
		// echo $sql1."</br>";
		...
		$sql22="update $pts.pro_attendance set present='".$_POST[$pra_id]."',absent='".$_POST[$aba_id]."',updated_user='$username',updated_at='".date('Y-m-d')."' where plant_code='$plantcode' and date='".$date."' and module='$modules_array[$i]' and shift='".$shift."'";
		// echo $sql22."</br>";
