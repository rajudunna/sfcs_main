    ...
    if(mysqli_num_rows($modules_result) > 0)
    {
        ...
        $sql_result1=mysqli_query($link, $sql1) or exit ("Sql Error: $Sql1".mysqli_error($GLOBALS["___mysqli_ston"]));
                ...
                if(in_array($authorized,$has_permission))
                    ...
                    <?php
                    $now_day=date('Y-m-d');
                    $previous_day=date('Y-m-d', strtotime($now_day . ' -1 day'));
                       $get_working_days="select DATE_FORMAT(last_up,'%Y-%m-%d') AS last_up  FROM $bai_pro.pro_attendance  where date='$date' GROUP BY date ORDER BY last_up DESC LIMIT 1  ";
                       ...
                       while($sql_row11=mysqli_fetch_array($result_get_working_day))
                                 ...
                                 $present_days="select sum(present) as presents  FROM $bai_pro.pro_attendance where date='$date' GROUP BY date  ";
                                 $result_present_days=mysqli_query($link, $present_days) or exit ("Sql Error: $Sql1".mysqli_error($GLOBALS["___mysqli_ston"]));     
                                 while($sql_row111=mysqli_fetch_array($result_present_days))
                                 {
                                 $total_presents=$sql_row111['presents'];
                                
                                 }

                                 if( $final_date==$previous_day){
                    ...
                    echo "<input type=\"hidden\" name=\"break_hours\" value=\"$break_hours\">";
                     ...
                     $previous_day=date('Y-m-d', strtotime($now_day . ' -1 day'));
                        ...
                        while($sql_row11=mysqli_fetch_array($result_get_working_day))
                                  ...
                                  if( $final_date==$previous_day || $now_day==$date){
                                     echo'<input type="submit" id="submit" class="btn btn-primary" value="Submit" >';
                                 }
