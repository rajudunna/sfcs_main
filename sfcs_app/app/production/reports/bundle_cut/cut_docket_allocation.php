...
{
	//alert("report");
	window.location.href ="../mini_order_report/cut_docket_allocation.php?style="+document.mini_order_report.style.value
}

function secondbox()
{
	//alert('test');
	window.location.href ="../mini_order_report/cut_docket_allocation.php?style="+document.mini_order_report.style.value+"&schedule="+document.mini_order_report.schedule.value
}
...
//	$bundles = echo_title("$brandix_bts.tbl_miniorder_data","count(*)","mini_order_ref",$miniord_ref,$link);
	...
	$sql="select GROUP_CONCAT(ref_size_name ORDER BY ref_size_name SEPARATOR \"-\" ) as size_code,GROUP_CONCAT(size_title ORDER BY ref_size_name SEPARATOR \"-\" ) as size_title from $brandix_bts.tbl_orders_sizes_master where order_col_des='".$color."' and parent_id='".$shcedule_code."'";
	...
	$lay_plan_query="SELECT tbl_cut_master.id,style_id,product_schedule,color,doc_num,actual_plies,tbl_cut_master.cut_num,
		...
		FROM $brandix_bts.tbl_cut_master 
		LEFT JOIN $brandix_bts.tbl_cut_size_master ON $brandix_bts.tbl_cut_size_master.parent_id=$brandix_bts.tbl_cut_master.id
	...
	{	
		...
		$mini_orders=echo_title("$brandix_bts.tbl_miniorder_data","group_concat(distinct mini_order_num order by mini_order_num)","docket_number",$docket_number,$link);
