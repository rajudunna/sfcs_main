<?php
	...
	while ($temp_res = $get_temp_data_res->fetch_assoc())
	{
		...
		$order_tid_temp[] = $style1."-".$schedule1."-".$color1."-".$size1;
	}
	$get_style_wip_data="select style,schedule,color,size FROM $brandix_bts.open_style_wip where style<>'' and status='open' and schedule = '773579' group by style,schedule,color,size";
	...
	$difference = array_diff($order_tid_temp,$order_tid_wip);
	$difference1 = array_diff($order_tid_wip,$order_tid_temp);
	$final = array_unique(array_merge($difference,$difference1));
	// var_dump($final);
