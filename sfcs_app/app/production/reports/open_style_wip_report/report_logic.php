<?php
	...
	$get_code= "SELECT operation_code FROM $brandix_bts.`tbl_ims_ops` where appilication='IPS'";
	$resultdes = $link->query($get_code);
    while($row312 = $resultdes->fetch_assoc())
	{
		$check_code = $row312['operation_code'];
	}
    ...
    $today=date("Y-m-d");
	$get_style_wip_data="select style,schedule,color,size FROM $brandix_bts.open_style_wip where (good_qty>0 or rejected_qty>0) group by style,schedule,color,size";
	...
	while ($row1 = $get_style_data_result->fetch_assoc())
	{
        ...
        $get_size_title = "select size_id from $brandix_bts.bundle_creation_data_temp where style='$style' and schedule='$schedule' and color='$color' and size_title='$size' and operation_id=$check_code limit 1";
