...
{
	//alert("report");
	window.location.href ="../mini_order_report/bundle_allocation_report.php?style="+document.mini_order_report.style.value
}
...
if(isset($_POST['submit']))
{
	...
	if($bundles>0)
	{
		...
		//echo $carton_ref."<br>";
		//getting carton qty
		$sql_cart="SELECT parent_id,color,GROUP_CONCAT(ref_size_name ORDER BY ref_size_name SEPARATOR \"-\" ) AS sizes,GROUP_CONCAT(quantity ORDER BY ref_size_name SEPARATOR \"-\") as ratio FROM $brandix_bts.tbl_carton_size_ref WHERE parent_id IN ($carton_ref) GROUP BY color";
		//echo $sql_cart."<br>";
		...
		{		
			...
			{
			//echo $sql2."<br>"; // freshdesk id due to over shipment mini order num not captur-5501
			}
			//exit;
