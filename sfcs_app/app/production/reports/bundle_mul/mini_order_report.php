...
<body>
<div id="page_heading"><span style="float: left"><h3>Mini Order Report</h3></span><span style="float: right"><b>?</b>&nbsp;</span></div>
...
$view_access=user_acl("SFCS_0262",$username,1,$group_id_sfcs); 
?>
...
if($miniord_ref=='' || $bundles==0)
{
	if($miniord_ref=='')
	{
		echo "<h2>Mini orders not yet generated.</h2>";
	}
	else
	{
		echo "<h2>Cut Allocation not completed to Mini orders.</h2>";
	}
	...
	$val2=$val-$val1;
				...
				$sql.=" FROM $brandix_bts.`tbl_orders_sizes_master` md where parent_id in (SELECT id FROM `tbl_orders_master` WHERE ref_product_style =$style_code and `product_schedule`=$schedule_result)";
				...
				while($row2=mysqli_fetch_array($sql_result12))
				{
					...
					for($j=0;$j<sizeof($sizesarray);$j++)
					{
						...
						{
							$qry_vallidate_size2="select id,ref_size_name from $brandix_bts.tbl_orders_sizes_master where parent_id in (SELECT id FROM `tbl_orders_master` WHERE ref_product_style =$style_code and `product_schedule`=$schedule_result) and order_col_des='".$row2['color']."' and size_title='".$sizesnames_array[$j]."'";
...
{	
	...
	//$c_block="TEST";
	//getting carton qty
	$qry_cartonqty_result = echo_title("$brandix_bts.tbl_min_ord_ref","carton_quantity","ref_crt_schedule=".$schedule." and ref_product_style",$style_code,$link);
	//echo $qry_cartonsizes."</br>";
	$sql="select group_concat(distinct(ref_size_name) order by ref_size_name) as size_id,group_concat(distinct(size_title) order by ref_size_name) as size_name from $brandix_bts.tbl_orders_sizes_master where parent_id in (SELECT id FROM `tbl_orders_master` WHERE ref_product_style =$style_code and `product_schedule`=$schedule_result) ORDER BY `ref_size_name` ASC";
	//echo $sql."<br>";
	$sql_result=mysqli_query($link, $sql) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	$val2=$val-$val1;
				...
				while($row2=mysqli_fetch_array($sql_result12))
				{
					...
					for($j=0;$j<sizeof($sizesarray);$j++)
					{
						if($sizesnames_array[$j]!='')
						{
							$qry_vallidate_size2="select id,ref_size_name from $brandix_bts.tbl_orders_sizes_master where parent_id in (SELECT id FROM $brandix_bts.`tbl_orders_master` WHERE ref_product_style =$style_code and `product_schedule`=$schedule_result) and order_col_des='".$row2['color']."' and size_title='".$sizesnames_array[$j]."'";
							$sql_result22=mysqli_query($link, $qry_vallidate_size2) or exit("Sql Error2".mysqli_error($GLOBALS["___mysqli_ston"]));
							//echo $qry_vallidate_size2."<br>";
							while($row22=mysqli_fetch_array($sql_result22))
							{
	...
	$rowcount.=" FROM $brandix_bts.`tbl_miniorder_data` md where `mini_order_ref` in($miniord_ref)";
		...
		while($r=mysqli_fetch_array($sql_result1))
		{
			...
			{	
				...
				{	
					...
					{		
						echo"<td><b><u>".$minisizes_sum[$sizesnames_array[$i]]."</u></b></td>";
						$minisizes_sum[$sizesnames_array[$i]]=0;
					}
			...
			if($miniorder!=$old_miniorder)
			{
				...
				echo "<tr><th colspan=".$val."><center><b> Mini order No:".$miniorder.$name."</b></center></th></tr>";
				//echo "<tr><td colspan=6><center><h3 style=\"background-color: #A9BCF5 ;\">Mini order No:".$miniorder."</h3></center></td></tr>";
				
				echo "<tr><th>Color</th>";
				for($i=0;$i<sizeof($sizesnames_array);$i++)
				{		
					if($sizesnames_array[$i] !='')
					{	
						echo "<th><u>".$sizesnames_array[$i]."</u></th>";
			...
			for($i=0;$i<sizeof($sizesnames_array);$i++)
			{
				...
				if($sizesnames_array[$i]!='')
				{
					$qry_vallidate_size="select id,ref_size_name from $brandix_bts.tbl_orders_sizes_master where parent_id in (SELECT id FROM `tbl_orders_master` WHERE ref_product_style =$style_code and `product_schedule`=$schedule_result) and order_col_des='".$r['color']."' and size_title='".$sizesnames_array[$i]."'";
					...
					{		
						...
						$minisizes_sum[$sizesnames_array[$i]]+=$r['size'.$size_id_n.'_qty'];
					}
					else
					{
						echo "<td>0</td>";
				...
				{
					$minisizes_sum[$sizesnames_array[$i]]+=$r['size'.$sizesarray[$i].'_qty'];
					echo "<td>".$r['size'.$sizesarray[$i].'_qty']."</td>";
					
				}
				
				//$sizes_sum[$sizesnames_array[$i]]+=$r['size'.$sizesarray[$i].'_qty'];
				$cnt=0;
			}
