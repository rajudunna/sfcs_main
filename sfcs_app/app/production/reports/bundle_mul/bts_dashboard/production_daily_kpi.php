...
<?php
// As per freshdesk ID - 5175 , We have removed validation.
// As per freshdesk ID - 5371 , We have removed validation.
include("dbconf.php");
...
$sch=echo_title("bai_pro3.bai_orders_db","count(distinct order_del_no)","order_style_no",$_GET['filterstyle'],$link);
$sch_con=echo_title("bai_pro3.bai_orders_db_confirm","zfeature not like count(distinct order_del_no)","order_style_no",$_GET['filterstyle'],$link);
//echo $sch."--".$sch_con."<br>";
if($sch>$sch_con)
{
	$schedules=$sch;
...
//echo $schedules."--<br>";
$sql1="select order_tid from bai_pro3.bai_orders_db where order_style_no='".$_GET['filterstyle']."' group by order_tid";
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	...
	$sql="SELECT * FROM (
	SELECT  DISTINCT tbl_miniorder_data_mini_order_num,SUM(IF(tbl_orders_ops_ref_operation_code='INI',bundle_transactions_20_repeat_quantity,0)) AS mini_order_qty,
	SUM(IF(tbl_orders_ops_ref_operation_code='LNO',bundle_transactions_20_repeat_quantity,0)) AS output,
	SUM(IF(tbl_orders_ops_ref_operation_code='BUO',bundle_transactions_20_repeat_quantity,0)) AS bunout,
	...
	) AS tmp WHERE (output+rejected)<mini_order_qty AND bunout>0 ORDER BY achiv DESC";
	mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	$sql="SELECT COUNT(*) AS totsch, SUM(IF((output+rejected)<(miniorderbundle) AND bundleout>0,1,0)) AS wip, SUM(IF((output+rejected)>=(miniorderbundle),1,0)) 
AS completed, COUNT(*)-(SUM(IF((output+rejected)>=(miniorderbundle),1,0))) AS balance
,SUM(order_qty) AS order_qty,SUM(output) AS output,SUM(bundling) AS bundling,SUM(bundleout) AS bundleout,SUM(linein) AS linein,
SUM(miniorderbundle) AS miniorderbundle
 FROM (
	SELECT * FROM (
	SELECT tbl_orders_master_product_schedule,SUM(order_qty) AS order_qty,SUM(output) AS output,SUM(bundling) AS bundling,SUM(bundleout) AS bundleout,
	SUM(linein) AS linein,SUM(miniorderbundle) AS miniorderbundle,SUM(rejected) AS rejected
...
(
SELECT  *,1 AS totmini, (IF((output+rejected)<(miniorderbundle) and bundleout>0,1,0)) AS wipmini, (IF((output+rejected)>=(miniorderbundle),1,0)) AS completedmini, 1-((IF((output+rejected)>=(miniorderbundle),1,0))) AS balancemini FROM	
...
$daten_mtd=array();	$monthn_mtd=array();	$output_mtd=array();	$bundling_mtd=array();	$bundleout_mtd=array();	$linein_mtd=array();	$miniorderbundle_mtd=array();	$a_output_mtd=array();	$b_output_mtd=array();	$sah_mtd=array(); $rejected_bts_mtd=array();
	...
	while($sql_row2=mysqli_fetch_array($sql_result2))
	{
		...
		$daten[]=$sql_row2['daten'];	$monthn[]=$sql_row2['monthn'];	$output[$sql_row2['daten']]=$sql_row2['output'];	$bundling[$sql_row2['daten']]=$sql_row2['bundling'];	$bundleout[$sql_row2['daten']]=$sql_row2['bundleout'];	$linein[$sql_row2['daten']]=$sql_row2['linein'];	$miniorderbundle[]=$sql_row2['miniorderbundle'];	$a_output[$sql_row2['daten']]=$sql_row2['a_output'];	$b_output[$sql_row2['daten']]=$sql_row2['b_output'];	$sah[$sql_row2['daten']]=$sql_row2['sah']; $rejected_bts[$sql_row2['daten']]=$sql_row2['rejected'];
...
$log_date_mtd=array();	$qms_style_mtd=array();	$qms_schedule_mtd=array();	$rejected_qty_mtd=array();	$monthn_mtd=array();
	...
	while($sql_row2=mysqli_fetch_array($sql_result2))
	{
		...
		$log_date[]=$sql_row2['log_date'];	$qms_style[]=$sql_row2['qms_style'];	$qms_schedule[]=$sql_row2['qms_schedule'];	$rejected_qty[$sql_row2['log_date']]=$sql_row2['rejected_qty'];	$monthn[]=$sql_row2['monthn'];
	...
	$sql="SELECT DATE as daten,style,SCHEDULE,sum(cpk_qty) as cpk_qty,LEFT(DATE,7) AS monthn FROM $view_set_4_snap WHERE style='$style' group by date ORDER BY DATE DESC
";
	mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
	$sql_result2=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
...
$date_mtd=array();	$style_cpk_mtd=array();	$schedule_mtd=array();	$cpk_qty_mtd=array();	$monthn_mtd=array();
	...
	while($sql_row2=mysqli_fetch_array($sql_result2))
	{
		...
		$date[]=$sql_row2['daten'];	$style_cpk[]=$sql_row2['style'];	$schedule[]=$sql_row2['schedule'];	$cpk_qty[$sql_row2['daten']]=$sql_row2['cpk_qty'];	$monthn[]=$sql_row2['monthn'];
...
?>  
 ...
 for($j=0;$j<5;$j++)
 {
		$daten=date("Y-m-d",strtotime("-$j days"));
?> 	
 
 <tr height=20 style='height:15.0pt'>
  <td height=20 class=xl6327816 style='height:15.0pt'></td>
  <td class=xl7327816 style='border-top:none'><?=date("d-m-y", strtotime($daten));?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$bundling[$daten];?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$bundleout[$daten];?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$linein[$daten];?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$output[$daten];?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$rejected_qty[$daten]; ?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$cpk_qty[$daten]; ?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$sah[$daten];?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$a_output[$daten];?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$b_output[$daten];?></td>
  <td class=xl6427816 style='border-top:none;border-left:none'><?=$rejected_bts[$daten];?></td>
  <td class=xl6327816></td>
 </tr>
 <?php
 $day--;
...
WHERE (cpk_qty_new IS NULL OR cpk_qty_new<order_qty ) "; // as per freshdesk ticket 4173 showing any WIP schedules.
	mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
...
*/
	...
	$sql="SELECT DATE as daten,style,SCHEDULE,sum(cpk_qty) as cpk_qty,LEFT(DATE,7) AS monthn FROM $view_set_4_snap WHERE style='$style' and schedule=$tbl_orders_master_product_schedule group by date ORDER BY DATE DESC
";
	mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
	$sql_result2=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
