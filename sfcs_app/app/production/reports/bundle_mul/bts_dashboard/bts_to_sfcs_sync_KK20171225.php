...
$sql="SELECT MAX(bundle_transactions_20_repeat_id) as max_id FROM $tblsnap WHERE bundle_transactions_20_repeat_operation_id IN (3,4,6,7)"; 
//echo $sql."<br>";
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	...
	$sql1="SELECT tbl_orders_size_ref_size_name,bundle_transactions_20_repeat_quantity
	...
	,tbl_shifts_master_shift_name, bundle_transactions_20_repeat_operation_id,bundle_transactions_20_repeat_act_module,tbl_miniorder_data_docket_number
	 FROM $tblsnap WHERE bundle_transactions_20_repeat_id=$var_sync_rep_id AND bundle_transactions_20_repeat_operation_id IN (3,4,6,7)";
	// echo $sql1."<br/><br/>";
	 //echo $sql1."<br/><br/>";
	...
	if(strlen($var_size_name)!=0)
	{
		...
		switch($var_case_tag)
		{
			case 30:
			{				
				$sql1="SELECT rand_track FROM bai_pro3.ims_log WHERE bai_pro_ref=".$var_sync_bundle_id;
			...
			/*	else
				{
					if($emb_status==1)
					{
						...
						$resutlt1211=mysql_query($sql12,$link) or exit("Sql Error22".mysql_error());
						while($row12=mysql_fetch_array($resutlt1211))
						{
							$sql="UPDATE bai_pro3.ims_log SET ims_date = '".$row12['bundle_transactions_date_time']."' , ims_mod_no = '".$row12['bundle_transactions_module_id']."' , ims_shift = '".$row12['tbl_shifts_master_shift_name']."' , ims_qty = '".$row12['bundle_transactions_20_repeat_quantity']."' , ims_status = '' , ims_log_date = '".$row12['bundle_transactions_date_time']."' WHERE bai_pro_ref = '".$row12['tbl_miniorder_data_bundle_number']."'";
							$resutlt12=mysql_query($sql,$link) or exit("Sql Error21".mysql_error());
						}
			...
			*/	
			...
			case 31:
			{
				...
				if(mysqli_affected_rows($link)>0)
				{
					...
					sfcs_qty,sfcs_log_user,m3_op_des,sfcs_tid_ref,sfcs_mod_no,sfcs_shift,sfcs_job_no) select NOW(),tbl_orders_style_ref_product_style,
					tbl_orders_master_product_schedule,tbl_miniorder_data_color,tbl_orders_size_ref_size_name,tbl_orders_sizes_master_size_title,	
			        '".$doc_no."',bundle_transactions_20_repeat_rejection_quantity*-1,'".$user_name."@localhost','SIN',
					bundle_transactions_20_repeat_id,bundle_transactions_module_id,tbl_shifts_master_shift_name,'".$job_no."' from 
					".$tblsnap." where bundle_transactions_20_repeat_id=".$var_sync_rep_id." and bundle_transactions_20_repeat_operation_id=".$var_sycn_operation_id;
					// echo $sql1."<br/><br/>";
			...
			case 41:
			{
				...
				while($row12=mysqli_fetch_array($resutlt12))
				{
					...
					sfcs_qty,sfcs_log_user,m3_op_des,sfcs_tid_ref,sfcs_mod_no,sfcs_shift,sfcs_job_no) select NOW(),tbl_orders_style_ref_product_style,
					tbl_orders_master_product_schedule,tbl_miniorder_data_color,tbl_orders_size_ref_size_name,tbl_orders_sizes_master_size_title,	
					tbl_miniorder_data_docket_number,bundle_transactions_20_repeat_rejection_quantity*-1,'".$user_name."@localhost','SOT',
					bundle_transactions_20_repeat_id,bundle_transactions_20_repeat_act_module,tbl_shifts_master_shift_name,'".$job_no."' from ".$tblsnap."
			...
			case 60:
			{				
			//	$sql1="SELECT rand_track FROM bai_pro3.ims_log WHERE bai_pro_ref=".$var_bts_tran_id;
				//echo $sql1."<br>";
			...
			//	{
					...
					tbl_shifts_master_shift_name,'-1',bundle_transactions_date_time,
					concat('a_',tbl_orders_size_ref_size_name),tbl_orders_style_ref_product_style,tbl_miniorder_data_color,
					...
					'-1',tbl_shifts_master_shift_name,'".$job_no."' from ".$tblsnap." where bundle_transactions_20_repeat_id=".$var_sync_rep_id." and bundle_transactions_20_repeat_operation_id=".$var_sycn_operation_id;
					mysqli_query($link, $sql1) or exit("Sql Error10".mysqli_error($GLOBALS["___mysqli_ston"]));
					//echo $sql1."<br>";
					$sql1="UPDATE brandix_bts.bts_to_sfcs_sync SET sync_status=1 WHERE bts_tran_id=".$var_bts_tran_id;
					mysqli_query($link, $sql1) or exit("Sql Error9".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	else
	{
		Switch($var_case_tag)
		{
			...
			case 32:
			{
				//if($emb_status==1)
				//{
				//	$sql1="UPDATE bai_pro3.ims_log SET ims_status='EPR',ims_mod_no='-1' where bai_pro_ref=".$var_sync_bundle_id;
