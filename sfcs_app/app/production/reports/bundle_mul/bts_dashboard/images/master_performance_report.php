...
Report Type:<select name="report_type">
	...
	<option value="5" <?php echo ($report_type==5)?"Selected":""; ?>>Schedule wise Carton Pack WIP</option>	
	</select> 
...
if(isset($_REQUEST['submit']))
{
	...
	$sql="
	select 
	style,schedule,	color,	daten,	sum(output) as output,	sum(bundling) as bundling,	sum(bundleout) as bundleout,	sum(linein) as linein,	sum(rejected) as rejected,	sum(sah) as sah,	sum(cpk_qty) as cpk_qty
	from 
	(
...
WHERE 1=1 and (DATE(bundle_transactions_date_time) between '$sdate' and '$edate' )  AND tbl_orders_master_product_schedule>0 
...
UNION 
SELECT style,SCHEDULE,color,DATE AS daten,0 AS output, 0 AS bundling, 0 AS bundleout, 0 AS linein, 0 AS rejected,SUM(cpk_qty) AS cpk_qty , 0 AS sah FROM `$view_set_6_snap` where 1=1  GROUP BY style,SCHEDULE,color,DATE
...
//echo $sql;
	...
	$sql="
	select 
	style,schedule,	color,	daten,	sum(output) as output,	sum(bundling) as bundling,	sum(bundleout) as bundleout,	sum(linein) as linein,	sum(rejected) as rejected,	sum(sah) as sah,	sum(cpk_qty) as cpk_qty, shift,module, operation
	from 
	(
	SELECT  tbl_orders_style_ref_product_style AS style,tbl_orders_master_product_schedule AS SCHEDULE,tbl_miniorder_data_color AS color,DATE(bundle_transactions_date_time)  AS daten,
SUM(IF(tbl_orders_ops_ref_operation_code='LNO',bundle_transactions_20_repeat_quantity,0)) AS output,
...
UNION 
SELECT style,SCHEDULE,color,DATE AS daten,0 AS output, 0 AS bundling, 0 AS bundleout, 0 AS linein, 0 AS rejected,SUM(cpk_qty) AS cpk_qty , 0 AS sah, '' as shift, '' as module,'CPK' as operation FROM `$view_set_6_snap` where 1=1  GROUP BY style,SCHEDULE,color,DATE
...
//echo $sql;
	...
	$sql="
	select 
	style,schedule,	color,	daten,	sum(output) as output,	sum(bundling) as bundling,	sum(bundleout) as bundleout,	sum(linein) as linein,	sum(rejected) as rejected,	sum(sah) as sah,	sum(cpk_qty) as cpk_qty, shift,module, operation,bundle_number, size
	from 
	(
	SELECT  tbl_orders_style_ref_product_style AS style,tbl_orders_master_product_schedule AS SCHEDULE,tbl_miniorder_data_color AS color,DATE(bundle_transactions_date_time)  AS daten,
SUM(IF(tbl_orders_ops_ref_operation_code='LNO',bundle_transactions_20_repeat_quantity,0)) AS output,
...
FROM $view_set_snap_1_tbl WHERE 1=1 and tbl_orders_ops_ref_operation_code in ('LNO','LNI') AND (tbl_orders_master_product_schedule IS NOT NULL) AND tbl_orders_master_product_schedule>0 
...
Redirect();
	...
	$table_data="";
	$table_data.="<h2>Style wise Inlay WIP </h2>";	
	$table_data.="<h4>LU: ".date("Y-d-m H:i:s")." </h4>";	
	$table_data.="<h3>$title</h3>";
	$table_data.="<table id='table_format'>";
	$table_data.="<tr>";
	...
	$table_data_export="";
	$table_data_export.="<h2>Style wise Inlay WIP </h2>";	
	$table_data_export.="<h4>LU: ".date("Y-d-m H:i:s")." </h4>";	
	$table_data_export.="<h3>$title</h3>";
	$table_data_export.="<table id='table_format'>";
	$table_data_export.="<tr>";
	...
	$table_data_export.="</tr>";
	/*
	$SQL="SELECT tmp.style,tmp.schedule,tmp.color,tmp.size_disp,tmp.quantity AS order_qty,tmp.bun,tmp.bno,tmp.input,tmp.output,view_set_6_snap.cpk_qty
FROM 
(
SELECT `tbl_orders_style_ref_product_style` AS style,
tbl_orders_master_product_schedule AS SCHEDULE,
...
SUM(IF(tbl_orders_ops_ref_operation_code='LNI',bundle_transactions_20_repeat_quantity,0)) AS input,
SUM(IF(tbl_orders_ops_ref_operation_code='LNO',bundle_transactions_20_repeat_quantity,0)) AS output
FROM view_set_snap_1_tbl GROUP BY `tbl_orders_master_product_schedule`,`tbl_miniorder_data_color`,`size_disp`
) AS tmp LEFT JOIN `view_set_6_snap` ON view_set_6_snap.schedule=tmp.schedule AND
view_set_6_snap.color=tmp.color AND view_set_6_snap.size=tmp.size_disp
GROUP BY tmp.schedule,tmp.color,tmp.size_disp";
	*/
			$sql="SELECT tmp.style,tmp.color,tmp.size_disp,tmp.quantity AS order_qty,tmp.bun,tmp.bno,tmp.input,tmp.output,tmp.rejected,$view_set_6_snap.cpk_qty
				FROM 
				(
					SELECT `tbl_orders_style_ref_product_style` AS style,
					tbl_orders_master_product_schedule as schedule,
					...
					SUM(IF(tbl_orders_ops_ref_operation_code='LNI',bundle_transactions_20_repeat_quantity,0)) AS input,
					SUM(IF(tbl_orders_ops_ref_operation_code='LNO',bundle_transactions_20_repeat_quantity,0)) AS output,
					SUM(bundle_transactions_20_repeat_rejection_quantity) AS reject_bts
					FROM $view_set_snap_1_tbl GROUP BY `tbl_orders_master_product_schedule`,`tbl_miniorder_data_color`,`size_disp`
				) AS tmp LEFT JOIN `$view_set_6_snap` ON $view_set_6_snap.schedule=tmp.schedule AND
				$view_set_6_snap.color=tmp.color AND $view_set_6_snap.size=tmp.size_disp
				GROUP BY tmp.color,tmp.size_disp";
	...
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		...
		$style=$sql_row['style'];
	$color=$sql_row['color'];		$size_disp=$sql_row['size_disp'];	$order_qty=$sql_row['order_qty'];	$bun=$sql_row['bun'];	$bno=$sql_row['bno'];	$input=$sql_row['input']; $output=$sql_row['output']; $cpk_qty=$sql_row['cpk_qty']; $rejected=$sql_row['rejected']; 
