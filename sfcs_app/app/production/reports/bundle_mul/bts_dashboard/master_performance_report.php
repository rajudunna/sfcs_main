...
UNION 
SELECT style,SCHEDULE,color,DATE AS daten,0 AS output, 0 AS pan_send, 0 AS pan_receive, 0 AS bundling, 0 AS bundleout, 0 AS linein, 0 AS rejected,SUM(cpk_qty) AS cpk_qty , 0 AS sah, shift,'' as module,'CPK' as operation FROM `$view_set_6_snap` where 1=1  GROUP BY DATE,style,SCHEDULE,color,shift
) as tmp where 1=1 ".$filter_query." group by daten,style,schedule,color,module,shift,operation order by daten";
//echo $sql;
//Added group by "shift" in the CPK Query // Fresh desk -4368
	$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
	while($sql_row=mysqli_fetch_array($sql_result))
	{
...
Redirect();
	...
	// Due to distinct query data has shown wrongly now we have corrected - Freshdesk #3686
	$order_qty=echo_title("brandix_bts_uat.view_set_2_snap","sum(tbl_orders_sizes_master_order_quantity)","tbl_orders_sizes_master_order_col_des='".$color."' and tbl_orders_sizes_master_size_title='".$size_disp."' and tbl_orders_style_ref_product_style",$style,$link);
