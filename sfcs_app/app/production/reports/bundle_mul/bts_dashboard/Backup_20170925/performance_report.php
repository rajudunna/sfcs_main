...
SUM(IF(tbl_orders_ops_ref_operation_code='LNO' AND tbl_shifts_master_shift_name='B',bundle_transactions_20_repeat_quantity,0)) AS b_output
FROM $view_set_snap_1_tbl 
WHERE 1=1 AND tbl_orders_style_ref_product_style IS NOT NULL ".$filter_query." GROUP BY tbl_orders_master_product_schedule,tbl_miniorder_data_mini_order_num,tbl_miniorder_data_color,size_disp order by tbl_orders_master_product_schedule,tbl_miniorder_data_mini_order_num,tbl_miniorder_data_color,size_disp";
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
while($sql_row=mysqli_fetch_array($sql_result))
{
	$tbl_orders_master_product_schedule=$sql_row['tbl_orders_master_product_schedule'];	
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	...
	$table_data.="<td>$rejected</td>";
	$table_data.="<td ".know_bg_color($miniorder_cum,$cpk_qty).">$miniorder_cum</td>";
	$table_data.="<td ".know_bg_color($miniorder_cum,$cpk_qty).">$cpk_qty</td>";

	$table_data.="<td>".($cpk_qty>=$miniorder_cum?"Completed":"Pending")."</td>";
	$table_data.="</tr>";
