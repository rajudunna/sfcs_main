...
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$fil_min=array();$cpk_qty=0;$mini_array=array();
//As per freshddesk Id -5410, We have cleared the concern.
while($sql_row=mysqli_fetch_array($sql_result))
{
	$tbl_orders_master_product_schedule=$sql_row['tbl_orders_master_product_schedule'];	
	$max_mini=echo_title("brandix_bts_uat.view_set_3_snap","ROUND(MAX(FLOOR(tbl_miniorder_data_mini_order_num)),2)","tbl_orders_master_product_schedule",$tbl_orders_master_product_schedule,$link);
	$mini_array_tmp=echo_title("brandix_bts_uat.view_set_3_snap","group_concat(distinct round(tbl_miniorder_data_mini_order_num,2))","tbl_miniorder_data_mini_order_num>=$max_mini and tbl_orders_master_product_schedule",$tbl_orders_master_product_schedule,$link);
	$mini_array=explode(",",$mini_array_tmp);
	
	// if($username=='sfcsproject1')
	// {
		// echo $max_mini."--".$tbl_orders_master_product_schedule."--".$mini_array_tmp."<br>";
	// }
	$size_disp=$sql_row['size'];	
	$tbl_miniorder_data_color=$sql_row['color'];	
	$output=$sql_row['output'];
	...
	if(in_array($tbl_miniorder_data_mini_order_num,$mini_array))
	{
		$table_data.="<td>Excess</td>";
	}
	else
	{
		$table_data.="<td>".($cpk_qty>=$miniorderbundle?"Completed":"Pending")."</td>";
	}
	$table_data.="</tr>";
	unset($mini_array);
}
