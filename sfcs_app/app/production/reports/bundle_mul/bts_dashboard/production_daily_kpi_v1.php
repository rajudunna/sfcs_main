...
$sch_con=echo_title("bai_pro3.bai_orders_db_confirm","count(distinct order_del_no)","order_style_no",$_GET['filterstyle'],$link);
//echo $sch."--".$sch_con."<br>";
if($sch>$sch_con)
{
	$schedules=$sch;
...
while($row1=mysqli_fetch_array($sql_result1))
{
	...
	//echo $schedule_order."--".$row1['order_tid']."<br>";
}
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	...
	) AS tmp WHERE (output+rejected)<>mini_order_qty AND linein>0   ORDER BY achiv DESC";
	mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
	...
	$sql="SELECT COUNT(*) AS totsch, SUM(IF((output+rejected)<>(miniorderbundle) AND linein>0,1,0)) AS wip, SUM(IF((output+rejected)=(miniorderbundle),1,0)) 
...
(
SELECT  *,1 AS totmini, (IF((output+rejected)<>(miniorderbundle) and linein>0,1,0)) AS wipmini, (IF((output+rejected)>=(miniorderbundle),1,0)) AS completedmini, 1-((IF((output+rejected)>=(miniorderbundle),1,0))) AS balancemini FROM	
