...
<head>
	...
	<title>Hourly Production Report- Section Wise New</title>
	<style>
		.text-right {
			text-align: right;
		}
							...
							<div class="table-area">
								<table class="table table-bordered">
									...
									<?php
										...
										$groupedSecQry = "SELECT distinct monthly_production_plan.group FROM $pps.monthly_production_plan LEFT JOIN $pps.monthly_production_plan_upload_log as upload_log ON upload_log.monthly_production_plan_upload_log_id = monthly_production_plan.monthly_production_plan_upload_log_id where  plant_code = '" . $plantCode . "' and planned_date ='".$frdate."'";
										$res_groupedSecQry = mysqli_query($link, $groupedSecQry);
										while ($row_groupedSecQry = mysqli_fetch_array($res_groupedSecQry)) {
											$sql = "SELECT * FROM $pps.monthly_production_plan LEFT JOIN $pps.monthly_production_plan_upload_log as upload_log ON upload_log.monthly_production_plan_upload_log_id = monthly_production_plan.monthly_production_plan_upload_log_id where  plant_code = '" . $plantCode . "' and planned_date ='".$frdate."' and monthly_production_plan.group = '".$row_groupedSecQry['group']."' ";
