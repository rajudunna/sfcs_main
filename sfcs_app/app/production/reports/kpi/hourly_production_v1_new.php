		...
		<div class="table-responsive">
			<table class="table table-bordered">
				...
				while($Sec=mysqli_fetch_array($section_result))
				{
					...
					$sql="SELECT * from $bai_pro3.module_master WHERE section='$section'";
					// echo $sql.'<br>';
					...
					if (mysqli_num_rows($res) > 0) 
					{
						...
						$sql12="SELECT hours,team,style,smv,fr_qty FROM $bai_pro2.`fr_data` WHERE DATE(frdate)='$frdate' and team='$team' GROUP BY team,style,smv ORDER BY team*1";
						...
						{ 
							...
							// echo $frdate;
						    $date=$frdate;
						    $style=trim($row12['style']);
							$smv=round($row12['smv'],4);
							$frqty=$row12['fr_qty'];
							$hours=$row12['hours'];
							// echo $date;
							...
							$get_nop_query="SELECT fix_nop FROM $pts.pro_plan WHERE plant_code='$plantcode' and date='$frdate' and mod_no='$team'";
							// echo $get_nop_query;
							...
							$sumscqty=0;
							unset($out);
							$sql3="SELECT time_parent_id,sum(qty) as outqty FROM $pps.hout2 where plant_code='$plantcode' and out_date='$frdate' AND team='$team' and trim(style)='".$style."' and smv='".$row12['smv']."' group by time_parent_id";
						//	echo $sql3."<br>";
