...
<?php
		if(isset($_GET['submit']))
		{
			...
			while($row=mysqli_fetch_array($res))
			{
				...
				//$sumscqty=$row['out'];
				$sumscqty=0;
				$sql3="SELECT time_parent_id,sum(qty) as outqty FROM $pps.hout2 where  plant_code='$plantcode' and out_date='$frdate' AND team='$team' and trim(style)='".$style."' and smv='".$row['smv']."' group by time_parent_id";
				//echo $sql3."<br>";
						...
						//	echo $row."<br>";
							...
							else if ($row < round($pcsphr))
							{
								if ($row == 0)
								{
									...
									$sql6_2x="SELECT distinct(reason_id) FROM $pps.hourly_downtime WHERE DATE='$frdate' AND time BETWEEN TIME('".$start_time[$i]."') AND TIME('".$end_time[$i]."') AND team='$team';";
