...
<?php
		if(isset($_GET['submit']))
		{
			...
			while($row=mysqli_fetch_array($res))
			{
				...
				// $sql6="SELECT out_time,qty,status FROM $bai_pro2.hout2 where out_date='$frdate' AND team='$team'";
				// $res6=mysqli_query($link,$sql6);
				
				// $sql6="SELECT $query remarks FROM $bai_pro2.hout2 where out_date='$frdate' AND team='$team'";
				// echo $sql6.'<br><br>';
				// $res6=mysqli_query($link,$sql6);
			  ...
			  <tbody>
				<tr>
					...
					<?php
						for ($i=0; $i < sizeof($time_display); $i++)
						{
							$row=echo_title("$bai_pro2.hout2","SUM(qty)","out_date='$frdate' AND rep_start_time = TIME('".$start_time[$i]."') AND rep_end_time = TIME('".$end_time[$i]."') and team",$team,$link);
