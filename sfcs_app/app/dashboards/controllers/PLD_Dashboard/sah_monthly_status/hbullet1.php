...
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "select sum(act_sth),sum(plan_sth) FROM grand_rep where date between \"$start\" and \"$end\" and section=\"1\"");
//echo "<br>select sum(act_sth),sum(plan_sth) FROM grand_rep where date between \"$start\" and \"$end\" and section=1<br><br>";
while($row=mysqli_fetch_array($sql))
{
...
$message= "<chart palette='1' lowerLimit='0' upperLimit='$plan_sth' majorTMNumber='0' canvasLeftMargin='120' caption='S-1' subcaption='$per%' showBorder='0' showValue='1' showTickMarks='0' showTickValues='0' targetColor='FF0000' plotFillColor='$col' ticksOnRight='1'>
    <colorRange>
 ...
 //To Write File
	...
	//To Write File
	//echo "test";
	echo "<script type=\"text/javascript\"> setTimeout(\"Redirect()\",0); function Redirect() {  location.href = \"hbullet2.php\"; }</script>";
?>