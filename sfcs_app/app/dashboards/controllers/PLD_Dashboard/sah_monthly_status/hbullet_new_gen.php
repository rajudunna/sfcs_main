...
$total="330000";
$sections=array();
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "select sum(act_sth),sum(plan_sth),section FROM grand_rep where date between \"$start\" and \"$end\"  group by section order by round(sum(act_sth)*100/sum(plan_sth),0)");
...
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "select sum(act_sth),sum(plan_sth) FROM grand_rep where date between \"$start\" and \"$end\" and section=$section_id");
