...
$sql2=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(plan_sth) FROM grand_rep WHERE DATE=\"$dat\" and section=\"1\"");
while($row2=mysqli_fetch_array($sql2))
{
	$plan=round($row2["SUM(plan_sth)"]/16,0);
	$plan_act=round($row2["SUM(plan_sth)"],0);
}

$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(act_sth) FROM grand_rep WHERE DATE=\"$dat\" and section=\"1\"");
while($row=mysqli_fetch_array($sql))
{
	$actl=round($row["SUM(act_sth)"]/16,0);
...
$sql1=mysqli_query($GLOBALS["___mysqli_ston"], "select max(bac_lastup) from bai_log where bac_date=\"$dat\" and bac_sec=\"1\"");
...
switch($hour)
{
	case "0":
	{
		$var="<point startValue='0' endValue='".($act*1)."' color='FFCCDD' alpha='40' radius='199' innerRadius='5' showBorder='0'/>";
		break;
	}
	case "1":
	{
		$var="<point startValue='".($act*1)."' endValue='".(round($act*2,0))."' color='00BBBB' alpha='40' radius='125' innerRadius='5' showBorder='0'/>";
		break;
	}
	case "2":
	{
...
{
$message= "<chart showTickMarks='0' showTickValues='1' adjustTM='1' majorTMNumber='8' minorTMNumber='0' pivotFillColor='HDFEDD' lowerLimit='0' upperLimit='$final' lowerLimitDisplay='0' upperLimitDisplay='$final' gaugeStartAngle='180' gaugeEndAngle='0' palette='1' numberSuffix='' tickValueDistance='25' showValue='0' decimals='0' editMode='0' baseFontColor='F1f1f1' baseFontSize ='18' bold='1' bgColor='333333' bgAlpha='100' showBorder='0'><trendpoints>".$var."</trendpoints><colorRange><color minValue='0' maxValue='".($act*1)."' code='000088'/><color minValue='".($act*1)."' maxValue='".(round($act*2,0))."' code='000088'/><color minValue='".(round($act*2,0))."' maxValue='".(round($act*3,0))."' code='000088'/><color minValue='".(round($act*3,0))."' maxValue='".(round($act*4,0))."' code='000088'/><color minValue='".(round($act*4,0))."' maxValue='".(round($act*5,0))."' code='000088'/><color minValue='".(round($act*5,0))."' maxValue='".(round($act*6,0))."' code='000088'/><color minValue='".(round($act*6,0))."' maxValue='".(round($act*7,0))."' code='000088'/><color minValue='".(round($act*7,0))."' maxValue='".(round($act*8,0))."' code='000088'/><color minValue='".(round($act*8,0))."' maxValue='".(round($act*9,0))."' code='000088'/><color minValue='".(round($act*9,0))."' maxValue='".(round($act*10,0))."' code='000088'/><color minValue='".(round($act*10,0))."' maxValue='".(round($act*11,0))."' code='000088'/><color minValue='".(round($act*11,0))."' maxValue='".(round($act*12,0))."' code='000088'/><color minValue='".(round($act*12,0))."' maxValue='".(round($act*13,0))."' code='000088'/><color minValue='".(round($act*13,0))."' maxValue='".(round($act*14,0))."' code='000088'/><color minValue='".(round($act*14,0))."' maxValue='".(round($act*15,0))."' code='000088'/><color minValue='".(round($act*15,0))."' maxValue='".(round($act*16,0))."' code='000088'/></colorRange><dials><dial id='CS' baseWidth='20' bgColor='FF0000' value='$act_act' rearExtension='10'/></dials><styles><definition><style type='font' name='myValueFont' bgColor='333333' borderColor='333333' /><style type='font' name='myValueback' bgColor='333333' borderColor='333333' /></definition><application><apply toObject='Value' styles='myValueFont' /><apply toObject='Background' styles='myValueback' /></application></styles></chart>";
}
else
{
