...
)*p_plies) AS cutting_wip FROM bai_pro3.order_cat_doc_mix WHERE print_status>\"2013-01-01\" AND act_cut_status='DONE' AND act_cut_issue_status<>'DONE' AND category IN ('".implode("','",$in_categories)."')";
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
while($sql_row=mysqli_fetch_array($sql_result))
{
