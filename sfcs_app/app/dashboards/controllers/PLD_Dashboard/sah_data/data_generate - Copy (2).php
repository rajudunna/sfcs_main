...
//From date and end date changed according to requirement 
...
$plan_sah_mod=array("65322","69677","65322","69677");
...
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(act_sth),count(distinct date) as dat FROM grand_rep WHERE DATE BETWEEN \"".$start."\" AND \"".date("Y-m-d")."\"");
echo "SELECT SUM(act_sth),count(distinct date) as dat FROM grand_rep WHERE DATE BETWEEN \"".$start."\" AND \"".date("Y-m-d")."\"";
while($row=mysqli_fetch_array($sql))
{
...
{	
	$sql_mod=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(act_sth) FROM grand_rep WHERE section=\"".$sections[$i]."\" and DATE BETWEEN \"".$start."\" AND \"".date("Y-m-d")."\"");
	echo "SELECT SUM(act_sth) FROM grand_rep WHERE section=\"".$sections[$i]."\" and DATE BETWEEN \"".$start."\" AND \"".date("Y-m-d")."\""."<br>";
...
$table1="<table border=1 align='center' style='border-collapse: collapse;border: 1px solid black;'>";
