...
2014-12-03 / kirang/ Service Request #929299 / Add section-5 in December Dhamaka Screen 

2014-12-17 / kirang / service request #121226 / capturing the achievement % and section hod details based on section performance

2015-01-04 / kirang / service request #121226 / Revised the SAH target and data range for January month

2015-01-04 / kirang / service request #121226 / Took the actual working days(removed the holidays from working days) to calculate the each day plan and averages.

2015-01-04 / kirang / service request #121226 / Removed the section 5 in Report

2015-01-04 / kirang / service request #121226 / Make the Report Dynamic : Collecting the dates and sections dynamically

2015-03-04 / kirang / service request #121226 / Revised the Plan SAH of a Sections

2015-03-04 / kirang / service request #121226 / Automated the target plan and Section head names
...
$sqlq=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT DISTINCT section as sec FROM grand_rep WHERE DATE BETWEEN \"".$start."\" AND \"".$end."\" order by section+0");
...
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(act_sth),count(distinct date) as dat FROM grand_rep WHERE DATE BETWEEN \"".$start."\" AND \"".$yest."\"");
...
{	
	...
	$sql_mod=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(plan_sth),SUM(act_sth) FROM grand_rep WHERE section=\"".$sections[$i]."\" and DATE BETWEEN \"".$start."\" AND \"".$yest."\"");
	...
	$sql_mod1=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(plan_sth),SUM(act_sth) FROM grand_rep WHERE section=\"".$sections[$i]."\" and DATE BETWEEN \"".$start."\" AND \"".$today."\"");
...
{
	$today_act_sec=0;
	$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(act_sth) FROM grand_rep WHERE DATE=\"$today\" and section=".$sections[$i]);
	while($row=mysqli_fetch_array($sql))
	{
	...
	$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT round(SUM(bac_qty*smv)/60,0) as sah FROM bai_log_buf WHERE bac_date=\"$today\" and hour(bac_lastup)=".(date("H")-1)." and bac_sec=".$sections[$i]);
	while($row=mysqli_fetch_array($sql))
	{
	...
	$sql_mod11=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT ROUND(SUM(act_sth)*100/".round($per_day_plan*$date_count,0).",0)AS per,section FROM grand_rep WHERE DATE BETWEEN \"".$start."\" AND \"".$yest."\" and section=\"".$sections[$i]."\" GROUP BY section ORDER BY ROUND(SUM(act_sth)*100/".round($per_day_plan*$date_count).",0) desc");
