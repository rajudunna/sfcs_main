...
//$dt="2012-01-28";
$dt=date("Y-m-d");
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "select sec_id,sec_head,sec_mods from $bai_pro3.sections_db where sec_id=3");
...
for($i=0;$i<sizeof($modules);$i++)
{
	...
	for($j=06;$j<=21;$j++)
	{
		...
		if($k == 8 || $k == 16)
		{
			...
			$sql_stat=mysqli_query($GLOBALS["___mysqli_ston"], "select count(bac_stat) from $database.$table where bac_date=\"$dt\" and bac_no=\"$modules[$i]\" and bac_lastup between \"$dt $sta:00:00\" and \"$dt $end:00:00\" and bac_stat=\"Active\"");
