...
<?php
$dt=date("Y-m-d");
$sql22=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT GROUP_CONCAT(`module_name` ORDER BY module_name+0 ASC) AS sec_mods,section AS sec_id FROM $bai_pro3.`module_master` GROUP BY section ORDER BY section + 0");
//echo $sql;
while($result22=mysqli_fetch_array($sql22))
{
	...
	for($i=0;$i<sizeof($modules);$i++)
	{
		...
		for($j=06;$j<=21;$j++)
		{
			$query = "select sum(bac_qty),bac_style,delivery,bac_shift,bac_stat from $bai_pro.$bai_log_buf where bac_date=\"$dt\" and bac_no=\"$modules[$i]\" and bac_lastup=\"$dt $j:00:00\"";
			...
			if($k == 8 || $k == 16)
			{
				...
				{
					echo "<th><div style=\"font-size:12px; text-align:center;\" title=\"$title\"><img src='".getFullURLLevel($_GET['r'],'common/images/120px-Cross_mark_red_setting_white_smoke_svg.png',2,'R')."'></img></div></th>";
				}
				else
				{
					echo "<th><div style=\"border-style:solid;border-width:2px;border-color:black;font-size:12px;width:28px;height:28px;background-color:white;color:black;\" title=\"$title\"><center>".$rew."</center></div></th>";
