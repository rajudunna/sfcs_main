...
while($sql_rowx=mysqli_fetch_array($sql_resultx))
{
	...
	for($x=0;$x<sizeof($mods);$x++)
	{
		...
		$sql1="SELECT clubbing,act_cut_status,act_cut_issue_status,rm_date,cut_inp_temp,doc_no,order_tid,order_style_no,order_del_no,order_col_des,total,acutno,color_code,st_status,emb_stat from $bai_pro3.plan_dash_doc_summ where module=$module and (rm_date is null or hour(rm_date)=0 or minute(rm_date)=0) order by priority limit 4";
		...
		while($sql_row1=mysqli_fetch_array($sql_result1))
		{
			...
			if($cut_input_new=="DONE") { $cut_input_new="T";	} else { $cut_input_new="F"; }
				...
				$sql11="select st_status from $bai_pro3.bai_orders_db_confirm where order_del_no=\"$schedule\" and $order_joins_in_2";
				$sql_result11=mysqli_query($link, $sql11) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
				
				if(mysqli_num_rows($sql_result11)>0)
				{
					$sql11="select st_status from $bai_pro3.bai_orders_db_confirm where order_joins=\"J$schedule\"";
					$sql_result11=mysqli_query($link, $sql11) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
					while($sql_row11=mysqli_fetch_array($sql_result11))
					{
