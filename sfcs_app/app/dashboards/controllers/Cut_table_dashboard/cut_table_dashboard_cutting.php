  ...
  $blink_minimum=0;
    ...
    $sql1="SELECT * from $bai_pro3.cut_tbl_dash_doc_summ ct
       ...
       if($_GET["view_div"]=="ALL" or $_GET["view_div"]=="")
      {
        $sql1="SELECT * from $bai_pro3.cut_tbl_dash_doc_summ ct
        left join $bai_pro3.plandoc_stat_log psl ON ct.doc_no = psl.doc_no
        where psl.org_doc_no <2 and doc_no in (".implode(",",$doc_no_ref).") order by field(doc_no,".implode(",",$doc_no_ref)."),log_time asc";
      }
      else
      {
        $dash = $_GET["view_div"];
        ...
        $sql1="SELECT * from $bai_pro3.cut_tbl_dash_doc_summ ct
            left join $bai_pro3.plandoc_stat_log psl ON ct.doc_no = psl.doc_no
            where psl.org_doc_no <2 and order_style_no  in (select order_style_no from $bai_pro3.bai_orders_db_confirm where order_div = ".'"'.$buyer_identity.'"'.") and doc_no in (".implode(",",$doc_no_ref).") order by field(doc_no,".implode(",",$doc_no_ref)."),log_time asc"; 
      }
