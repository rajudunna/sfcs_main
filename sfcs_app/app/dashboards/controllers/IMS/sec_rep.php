...
<?php
		...
		while($sql_row=mysqli_fetch_array($sql_result))
		{
			...
			$get_details="select style,schedule,color,size_title,size_id,cut_number,input_job_no,bundle_number,remarks,docket_number,sum(recevied_qty) as input,sum(recevied_qty) as output From $brandix_bts.bundle_creation_data where assigned_module=$module_ref and input_job_no_random_ref = '$input_job' and operation_id in ($input_code,$output_code) and (recevied_qty >0 or rejected_qty >0)";	
