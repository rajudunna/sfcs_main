...
<?php
    ...
    $bundles = '';
    //getting bundle_numbers
    $retreaving_bcd_data = "SELECT bundle_number FROM `brandix_bts`.`bundle_creation_data` WHERE id IN ($input_job) ORDER BY barcode_sequence";
    $retreaving_bcd_data_res = $link->query($retreaving_bcd_data);
    while($row_bcd = $retreaving_bcd_data_res->fetch_assoc()) 
    {
        $bundles .= $row_bcd['bundle_number'].',';
    }
	...
	$html = '
		...
		$barcode_qry="select * from $bai_pro3.packing_summary_input where order_del_no='".$schedule."' and tid in($bundles) order by doc_no,old_size,barcode_sequence";
