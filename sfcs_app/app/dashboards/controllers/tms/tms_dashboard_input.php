...
while($sql_rowx=mysqli_fetch_array($sql_resultx))
{
	...
	{		
		...
		echo "<tr><th colspan=2><h2><a href=\"$url_path&section_no=$section \" >SECTION - $section</a></h2></th></th></tr>";
		...
		for($x=0;$x<sizeof($mods);$x++)
		{
			...
			while($row=mysqli_fetch_array($result))
			{
				...
				$title=str_pad("Style:".$style,80)."\n".str_pad("Schedule:".$schedule,80)."\n".str_pad("Job_No:".'J'.leading_zeros($input_job_no,3),80);
					...
					if(in_array($authorized,$has_permission))
					{
						echo "<div id=\"S$schedule\" style=\"float:left;\"><div id=\"SJ$input_job_no\" style=\"float:left;\"><div id=\"$input_job_no_random_ref\" class=\"$id\" style=\"font-size:12px; text-align:center; color:$id\" title=\"$title\" ><a href=\"../ ".getFullURL($_GET['r'],'trims_status_update_input.php','N')."&jobno=$input_job_no&style=$style&schedule=$schedule&module=$module&section=$section&doc_no=$input_job_no_random_ref&isinput=0\"  if (window.focus) {Popup.focus()} return false;\"><font style=\"color:black;\">$letter</font></a></div></div></div>";
					}
					else
					{
