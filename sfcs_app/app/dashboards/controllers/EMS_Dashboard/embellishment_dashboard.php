...
//  x=document.getElementById('view_cat').value;
  ...
  window.location = "<?= getFullURL($_GET['r'],'embellishment_dashboard.php','N') ?>"+"&view=2&view_div="+y;
}
...
function redirect_dash()
{
  ...
  window.location = "<?= getFullURL($_GET['r'],'embellishment_dashboard.php','N') ?>"+"&view="+z+"&view_cat="+x+"&view_div="+y;
}
...
echo "<div class='panel-heading'><span style='float'><strong>EMS Dashboard</strong></a>
...
while($sql_rowx=mysqli_fetch_array($sql_resultx))
{
  ...
  // start style wise display by dharani 10-26-2013 
    ...
    $sql1="SELECT * from $bai_pro3.plan_dash_doc_summ_embl where doc_no in (".implode(",",$doc_ref).") and act_cut_status<>'DONE' order by field(doc_no,".implode(",",$doc_ref).")";
      ...
      else
      {
        ...
        $sql1="SELECT * from $bai_pro3.plan_dash_doc_summ_embl where order_style_no  in (select order_style_no from $bai_pro3.bai_orders_db_confirm where order_div = ".'"'.$buyer_identity.'"'.") and doc_no in (".implode(",",$doc_ref).") and act_cut_status<>'DONE' order by field(doc_no,".implode(",",$doc_ref).")"; 
  ...
  //Ticket #177328 add the Blinking Option for Exceeding Fabric Request Dockets and IU module show "IU" in that boxes and if it is emblishment display "IX"
    $sqlt="SELECT * from $bai_pro3.plan_dash_doc_summ_embl where  doc_no in ($doc_no) and act_cut_issue_status<>'DONE' and (order_style_no like 'L%Y%' or order_style_no like 'L%Z%' or order_style_no like 'O%Y%' or order_style_no like 'O%Z%') order by field(doc_no,$doc_no) " ;
    //echo "query=".$sqlt;
