...
<script type="text/javascript">
  ...
  window.location
  =
  "<?= getFullURL($_GET['r'], 'embellishment_dashboard_receive_operation.php', 'N') ?>" + "&view=2&view_div=" + y;
  }
  function
  redirect_dash()
  {
  ...
  window.location
  =
  "<?= getFullURL($_GET['r'], 'embellishment_dashboard_receive_operation.php', 'N') ?>" + "&view=" + z + "&view_cat=" + x + "&view_div=" + y;
  }
  function
  blink_new3(x)
  {
...
foreach ($workstations as $emb_key => $emb_value) {
  ...
  foreach ($task_job_ids as $task_job_id => $task_header_id_j) {
    ...
    $task_job_trans = "SELECT * FROM $tms.task_job_transactions where task_jobs_id ='$task_job_id'  and job_group=".DepartmentTypeEnum::EMBELLISHMENT." order by operation_seq desc limit 0,1";
    ...
    $task_job_trans = "SELECT * FROM $tms.task_job_transactions where task_jobs_id ='$task_job_id' and job_group=" . DepartmentTypeEnum::EMBELLISHMENT . " order by operation_seq desc limit 1,1";
    $task_job_trans_result = mysqli_query($link_new, $task_job_trans) or exit("Sql Error at task_job_trans_result" . mysqli_error($GLOBALS["___mysqli_ston"]));
    while ($row_res = mysqli_fetch_array($task_job_trans_result)) {
