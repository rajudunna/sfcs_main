...
//if(isset($_POST['submit']))
{
	...
	$time_query=" AND HOUR(bac_lastup) in (".implode(",",$hour_filter1).") "; 
	...
	for ($j=0;$j<sizeof($sections);$j++)
	{
		...
		while($sql_row=mysqli_fetch_array($sql_result))
		{
			...
			if($tot_del>0)
			{
				...
				if($option1==1)
				{
					//echo "<td>".round($plan_sah_hr)."-".round($psth)."-".$hoursa."</td>";		
		...
		if($option1==1)
		{ 
			echo "<tr class=\"total\"><td colspan=4>HOURLY SAH</td>"; 
		} 
		else 
		{  
		...
		if($option1==1)
		{	
			echo "<tr class=\"total\"><td colspan=4>HLY EFF%</td>"; 
		}
		else
		{
		...
		if($option1==1)
		{	
			echo "<tr class=\"total\"><td colspan=4>AVG-Pcs/HR</td>"; 
		}
		else
		{ 
