...
$sql2=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(plan_pro) FROM pro_plan WHERE DATE=\"$dat\" and sec_no=\"4\"");
while($row2=mysqli_fetch_array($sql2))
{
	$plan=$row2["SUM(plan_pro)"]/16;
}

$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT SUM(act_out) FROM grand_rep WHERE DATE=\"$dat\" and section=\"4\"");
while($row=mysqli_fetch_array($sql))
{
	$act=$row["SUM(act_out)"]/16;
}

$sql1=mysqli_query($GLOBALS["___mysqli_ston"], "select max(bac_lastup) from bai_log where bac_date=\"$dat\" and bac_sec=\"4\"");
