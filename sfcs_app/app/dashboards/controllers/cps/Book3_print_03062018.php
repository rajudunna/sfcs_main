...
<?php
		...
		//$order_tid=trim($_GET['order_tid']);
		$order_tid = str_replace(" ", "", trim($order_tid1));
		//echo $order_tid;
...
include($_SERVER['DOCUMENT_ROOT'].'/sfcs_app/common/config/config.php');
error_reporting(0);
$sql="select * from $bai_pro3.bai_orders_db_confirm where REPLACE(order_tid , ' ','')=\"$order_tid\"";
//echo $sql;
...
$sql="select * from $bai_pro3.cat_stat_log where REPLACE(order_tid , ' ','')=\"$order_tid\" and tid=$cat_ref";
//echo $sql;
mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_num_check=mysqli_num_rows($sql_result);
...
$sql="select sum(cuttable_s_xs) as \"cuttable_s_xs\", sum(cuttable_s_s) as \"cuttable_s_s\", sum(cuttable_s_m) as \"cuttable_s_m\", sum(cuttable_s_l) as \"cuttable_s_l\", sum(cuttable_s_xl) as \"cuttable_s_xl\", sum(cuttable_s_xxl) as \"cuttable_s_xxl\", sum(cuttable_s_xxxl) as \"cuttable_s_xxxl\" from $bai_pro3.cuttable_stat_log where REPLACE(order_tid , ' ','')=\"$order_tid\" and cat_id=$cat_ref";
//echo $sql;
mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_num_check=mysqli_num_rows($sql_result);
...
$sql="select *,fn_savings_per_cal(DATE,cat_ref,'$delivery','$color') as savings from $bai_pro3.plandoc_stat_log where REPLACE(order_tid , ' ','')=\"$order_tid\" and cat_ref=$cat_ref and doc_no=$doc_id";
//echo $sql;
mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_num_check=mysqli_num_rows($sql_result);
while($sql_row=mysqli_fetch_array($sql_result))
{
 ...
 //echo $sql;
	...
	$sql2="select COALESCE(binding_con,0) as \"binding_con\" from $bai_pro3.bai_orders_db_remarks where REPLACE(order_tid , ' ','')=\"$order_tid\"";
	//echo $sql2;
	$sql_result2=mysqli_query($link, $sql2) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
	while($sql_row2=mysqli_fetch_array($sql_result2))
	{
		$bind_con=$sql_row2['binding_con']*($a_ratio_tot*$plies);
	}
	//echo $a_ratio_tot;
	//Binding Consumption / YY Calculation
...
<link rel=File-List href="<?= $_SERVER['DOCUMENT_ROOT'].'/sfcs_app/app/dashboards/common/images/filelist.xml'?>">
...
<table border=0 cellpadding=0 cellspacing=0 width=758 style='border-collapse:
 ...
 <tr height=21 style='height:15.75pt'>
  <td height=21 class=xl1515551 width=24 style='height:15.75pt;width:18pt' colspan=8>
  <?php 
  $doc_id=$_GET['doc_id'];
  echo '<div id="bcTarget1" style="width:auto;"></div><script>$("#bcTarget1").barcode("D'.$doc_id.'", "code39",{barWidth:2,barHeight:15,moduleSize:5,fontSize:0});</script>'; ?></td>
...
">
 ...
 <tr class=xl6315551 height=26 style='mso-height-source:userset;height:19.5pt'>
  ...
  <td class=xl6315551></td>
  <td class=xl6315551></td>
  <td colspan=6 class=xl9215551>Cutting Docket</td>
...
" o:insetmode="auto">
 ...
 <tr class=xl6315551 height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td colspan=2 class=xl10015551><?php if($remarks=="Normal") { echo chr($color_code).leading_zeros($cutno, 3); } else {if($remarks=="Pilot") { echo "Pilot";}}?></td>
...
">
 ...
 <tr class=xl6415551 height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td class=xl7715551>Pattern Version</td>
...
{ $extra=round((($purlength*$plies)*$savings),2); }  echo round((($purlength*$plies)+$extra+$bind_con),2); //Extra 1% added to avoid cad saving manual mrn claims. ?></td>
  <td rowspan=2 class=xl9615551 width=67 style='border-bottom:.5pt solid black;
  border-top:none;width:50pt'><?php echo $actwidth; ?></td>
  <td rowspan=2 class=xl9615551 width=64 style='border-bottom:.5pt solid black;
...
$ctex_width=array();
$tkt_width=array();
$sql="select distinct * from $bai_rm_pj1.docket_ref where doc_no=$doc_id and doc_type='normal' group by roll_id";
// echo $sql;
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
while($sql_row=mysqli_fetch_array($sql_result))
{
...
echo "</tr>";
echo "</table>";
//echo $lot_ref; echo "MK File: $mk_file";
echo "</td>";
echo "</tr>";
 ...
 <tr class=xl6315551 height=20 style='mso-height-source:userset;height:15.0pt'>
   <td height=20 class=xl6415551 style='height:15.0pt'></td>
   <td rowspan="3" colspan=11 class=xl7815551 style='border-top:.5pt solid black;' >Inspection Comments:
   <?php
   echo ($bind_con>0)?"Binding/Rib Quantity: $bind_con $fab_uom":"";
