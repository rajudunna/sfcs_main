...
if(isset($_POST['allocate_new']))
{
			...
			for($j=0;$j<sizeof($tid_ref);$j++)
			{
				if($tid_ref[$j]>0 && $issued_ref[$j]>0)
				{
					...
					if(strtolower($roll_splitting) == 'yes' && $total_qty == 0)
					{
						...
						$sql121="update wms.store_in set qty_issued=qty_issued+".$issued_ref[$j].",updated_by= '".$username."' where plant_code='".$plant_code."' and tid=".$tid_ref[$j];
						// echo $sql121."<br>";
                    ...
                    if(mysqli_num_rows($sql_result111)>0)
                    {
                        while($row2=mysqli_fetch_array($sql_result111))
                        {
							...
							$sql121="update $wms.store_in set status=$status,allotment_status=$status,updated_by= '".$username."' where plant_code='".$plant_code."' and 'tid=".$tid_ref[$j];
	...
	$update_parent="update $pps.binding_consumption set status='Allocated',status_at='".date("Y-m-d H:i:s")."',updated_by= '".$username."' where id=$row_id_new and plant_code='".$plant_code."'";
