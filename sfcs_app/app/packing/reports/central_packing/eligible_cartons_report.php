									...
									if (mysqli_num_rows($barcode_result) > 0)
									{
										while($row_result=mysqli_fetch_array($barcode_result))
										{
											...
											if($quantity == $reported_quantity) {
												...
												// get mo's for carton
												$qty = 0;
												$carton_mos_sql="SELECT mo_number,quantity FROM $pps.`jm_pack_container_line` WHERE jm_pack_container_id ='$external_ref_id' AND plant_code = '$plantcode'";
												...
												{													
													...
													{
														$carton_qty = $row_result['quantity'];
														$fg_id_sql="SELECT finished_good_id FROM $pts.`finished_good` WHERE mo_number= ".$row_result['mo_number']." AND plant_code = '$plantcode'";
