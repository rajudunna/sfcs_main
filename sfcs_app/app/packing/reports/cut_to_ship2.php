...
<?php 
	...
	$view_access=user_acl("SFCS_0034",$username,1,$group_id_sfcs);
...
<!--
Ex-factory Start Date <input id="demo1" onclick="javascript:NewCssCal('demo1','yyyymmdd','dropdown')" type="text" name="sdate" size="8" value="<?php if(isset($_POST['sdate'])) { echo $_POST['sdate']; } else { echo date("Y-m-d"); } ?>"> End Date <input id="demo2" onclick="javascript:NewCssCal('demo2','yyyymmdd','dropdown')" type="text" size="8" name="edate" value="<?php if(isset($_POST['edate'])) { echo $_POST['edate']; } else { echo date("Y-m-d"); } ?>"><input type="submit" name="filter" value="Filter"> 
-->
<?php
...
// echo "<br/>";
echo "<div class='row'>
<div class='col-md-3'><label>Select Style:</label>
...
echo "<div class='col-md-3'><label>Select Schedule:</label> 
...
echo "<div class='col-md-3'><label>Select Color: </label><select name=\"color\" onchange=\"thirdbox();\" class='form-control'>";
...
if(isset($_POST['filter']) or isset($_POST['filter2']))
{
	...
	if ($style=='NIL' or $schedule=='NIL' or $color=='NIL') {
		...
		<th class=\"total\" style='background-color:#29759C;color:white;'>Panel Room <br/>Missing</th>
		...
		if(isset($_POST['filter2']))
		{
			...
			$sql="select style,schedule_no,color,ssc_code_new,priority,buyer_division,ex_factory_date_new,sections 
			    from $bai_pro4.bai_cut_to_ship_ref where schedule_no=\"$sch_db\" and trim(color)=\"$color\" order by ex_factory_date_new limit 1";
		}
		else
		{
			$sql="select style,schedule_no,color,ssc_code_new,priority,buyer_division,ex_factory_date_new,sections from 
			$bai_pro4.bai_cut_to_ship_ref where ex_factory_date_new between \"$sdate\" and \"$edate\" order by ex_factory_date_new";
		}
		//echo $sql;	
		$sql_result=mysqli_query($link,$sql) or exit("Sql Error".mysqli_error());
		while($sql_row=mysqli_fetch_array($sql_result))
		{
				...
				$sql1="select coalesce(sum(a_xs*a_plies),0) as \"a_xs\", coalesce(sum(a_s*a_plies),0) as \"a_s\", coalesce(sum(a_m*a_plies),0) as \"a_m\", coalesce(sum(a_l*a_plies),0) as \"a_l\", coalesce(sum(a_xl*a_plies),0) as \"a_xl\", coalesce(sum(a_xxl*a_plies),0) as \"a_xxl\", coalesce(sum(a_xxxl*a_plies),0) as \"a_xxxl\", coalesce(sum(a_s01*a_plies),0) as \"a_s01\", coalesce(sum(a_s02*a_plies),0) as \"a_s02\", coalesce(sum(a_s03*a_plies),0) as \"a_s03\", coalesce(sum(a_s04*a_plies),0) as \"a_s04\", coalesce(sum(a_s05*a_plies),0) as \"a_s05\", coalesce(sum(a_s06*a_plies),0) as \"a_s06\", coalesce(sum(a_s07*a_plies),0) as \"a_s07\", coalesce(sum(a_s08*a_plies),0) as \"a_s08\", coalesce(sum(a_s09*a_plies),0) as \"a_s09\", coalesce(sum(a_s10*a_plies),0) as \"a_s10\", coalesce(sum(a_s11*a_plies),0) as \"a_s11\", coalesce(sum(a_s12*a_plies),0) as \"a_s12\", coalesce(sum(a_s13*a_plies),0) as \"a_s13\", coalesce(sum(a_s14*a_plies),0) as \"a_s14\", coalesce(sum(a_s15*a_plies),0) as \"a_s15\", coalesce(sum(a_s16*a_plies),0) as \"a_s16\", coalesce(sum(a_s17*a_plies),0) as \"a_s17\", coalesce(sum(a_s18*a_plies),0) as \"a_s18\", coalesce(sum(a_s19*a_plies),0) as \"a_s19\", coalesce(sum(a_s20*a_plies),0) as \"a_s20\", coalesce(sum(a_s21*a_plies),0) as \"a_s21\", coalesce(sum(a_s22*a_plies),0) as \"a_s22\", coalesce(sum(a_s23*a_plies),0) as \"a_s23\", coalesce(sum(a_s24*a_plies),0) as \"a_s24\", coalesce(sum(a_s25*a_plies),0) as \"a_s25\", coalesce(sum(a_s26*a_plies),0) as \"a_s26\", coalesce(sum(a_s27*a_plies),0) as \"a_s27\", coalesce(sum(a_s28*a_plies),0) as \"a_s28\", coalesce(sum(a_s29*a_plies),0) as \"a_s29\", coalesce(sum(a_s30*a_plies),0) as \"a_s30\", coalesce(sum(a_s31*a_plies),0) as \"a_s31\", coalesce(sum(a_s32*a_plies),0) as \"a_s32\", coalesce(sum(a_s33*a_plies),0) as \"a_s33\", coalesce(sum(a_s34*a_plies),0) as \"a_s34\", coalesce(sum(a_s35*a_plies),0) as \"a_s35\", coalesce(sum(a_s36*a_plies),0) as \"a_s36\", coalesce(sum(a_s37*a_plies),0) as \"a_s37\", coalesce(sum(a_s38*a_plies),0) as \"a_s38\", coalesce(sum(a_s39*a_plies),0) as \"a_s39\", coalesce(sum(a_s40*a_plies),0) as \"a_s40\", coalesce(sum(a_s41*a_plies),0) as \"a_s41\", coalesce(sum(a_s42*a_plies),0) as \"a_s42\", coalesce(sum(a_s43*a_plies),0) as \"a_s43\", coalesce(sum(a_s44*a_plies),0) as \"a_s44\", coalesce(sum(a_s45*a_plies),0) as \"a_s45\", coalesce(sum(a_s46*a_plies),0) as \"a_s46\", coalesce(sum(a_s47*a_plies),0) as \"a_s47\", coalesce(sum(a_s48*a_plies),0) as \"a_s48\", coalesce(sum(a_s49*a_plies),0) as \"a_s49\", coalesce(sum(a_s50*a_plies),0) as \"a_s50\" 	
				    from $bai_pro3.order_cat_doc_mix where order_tid=\"$order_tid\" and category in ($in_categories) and act_cut_status=\"DONE\"";
				$sql_result1=mysqli_query($link,$sql1) or exit("Sql Error".mysqli_error());
				while($sql_row2=mysqli_fetch_array($sql_result1))
				{
				...
				$or_ratio_db=array();
				$good_panels=array();
				$sql1="select sum(rejected) as \"rejected\", sum(replaced) as \"replaced\", sum(sample_room) as \"sample_room\", sum(good_garments) as \"or_ratio\", sum(good_panels) as \"good_panels\", qms_size  from 
				       $bai_pro3.bai_qms_day_report where qms_schedule=\"".$sql_row['schedule_no']."\" and qms_color=\"".$color."\" group by qms_size";
				//echo $sql1."<br/>";
				$sql_result1=mysqli_query($link,$sql1) or exit("Sql Error".mysqli_error());
				while($sql_row1=mysqli_fetch_array($sql_result1))
				{
				...
				//echo $sqlx1."<br/>";
				$sql_resultx1=mysqli_query($link,$sqlx1) or exit("Sql Error".mysqli_error());
				...
				$sqlx1="select SUM(ims_qty) as \"ims_qty\", ims_size from $pms.ims_log_backup where plant_code='$plantcode' and ims_schedule=\"$schedule\" and ims_color=\"$color\" and ims_mod_no>0 group by ims_size";
				//echo $sqlx1."<br/>";
				$sql_resultx1=mysqli_query($link,$sqlx1) or exit("Sql Error".mysqli_error());
		...
		if(mysqli_num_rows($sql_result) > 0){
			echo "<br/><div class='col-md-4'><table class='table table-bordered'>";
			echo "<tr><td colspan=2 align=left style='background-color:#29759C; color:white;'><strong>Summary of Garments</strong></td></tr>";
			echo "<tr><td>Total Good Panels: </td><td><span class='label label-danger'>$display_total_good_panels</span></td></tr>";
			echo "<tr><td>Total Rejected Panels/Garments: </td><td><span class='label label-danger'>$display_total_rejected_panels</span></td></tr>";
			echo "<tr><td>Total Missing Panels: </td><td><span class='label label-danger'>$display_total_missing_panels</span></td></tr>";
			echo "</table></div>";
		}
		//die();
	}
