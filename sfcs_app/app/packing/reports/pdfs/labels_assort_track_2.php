...
//To fix carton qty as 240

for($i=0;$i<sizeof($carton_qtys);$i++)
{
	//$carton_qtys[$i]=240;
	$carton_qtys[$i]=$track_qty;
}
...
,$o_s_s48,$o_s_s49,$o_s_s50);
	...
	$order_grand_total+=array_sum($order_qtys);
	...
	for($i=0;$i<sizeof($order_qtys);$i++)
	{
		if($order_qtys[$i]>0)
		{
			do
			{
				...
				$carton_nodes[]=$style."$".$delivery."$".$color."$".$size_titles[$i]."$".$tempx;	
			}while($order_qtys[$i]>0);
		}
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	if($order_grand_total!=$sql_row['carton_act_qty'])
	{
...
<body>';
			...
			$html.= '<div><table>';
			$html.= '<tr><td align=right>Style:</td><td class="new_td" width="50%">'.$node_detail[0].'</td><td align=right>Size:</td><td class="new_td">'.$node_detail[3].'</td></tr>';
			$html.= '<tr><td align=right>Sch.:</td><td class="new_td">'.$node_detail[1].'</td><td align=right>Qty:</td><td class="new_td">'.$node_detail[4].'</td></tr>';
			$html.= '<tr><td align=right>Color.:</td><td class="new_td">'.$node_detail[2].'</td><td align=right>Sign:</td><td class="new_td2"></td></tr>';
$html.= '<tr><td align=right>Date.:</td><td class="new_td2"></td><td align=right>Mod.:</td><td class="new_td2"></td></tr>';
			$html.= '</table></div>';
