...
<?php
 ...
 $sql="select remarks, min(carton_act_qty) as \"packs\", min(tid) as \"tid\", sum(carton_act_qty) as \"carton_act_qty\", doc_no, group_concat(distinct trim(order_col_des)  ORDER BY tid SEPARATOR \" + \") as \"color_title\",group_concat(trim(carton_act_qty)  ORDER BY tid SEPARATOR \" + \") as \"pcs_title\" from $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=$schedule group by doc_no_ref ";
//echo $sql;
			//mysql_query($sql,$link) or exit("Sql Error".mysql_error());
			...
			while($sql_row=mysqli_fetch_array($sql_result))
			{
				...
				$sql2="SELECT group_concat(distinct(color)) as color,group_concat(concat(sfcs_size,': ',ratio_qty)) as ratio FROM $bai_pro3.ratio_input_update where schedule=$schedule";
				$sql_result2=mysqli_query($link, $sql2) or exit("Sql Error7".mysqli_error($GLOBALS["___mysqli_ston"]));
				while($sql_row2=mysqli_fetch_array($sql_result2))
				{
					$color_title=(strlen(trim($sql_row2['color']))>150?substr(trim($sql_row2['color']),0,150)."...":trim($sql_row2['color']));
					//$color_title="DARK NAVY-410 ,SOFT BLUE-459 ,STREAM-427 ,DARK NAVY-410 ,SOFT BLUE-459 ,STREAM-427 ,DARK NAVY-410 ,SOFT BLUE-459 ,STREAM-427 ,STREAM-427 ,DARK NAVY-410 ,SOFT BLUE-459 ,STREAM-427 ";
					$ratio=$sql_row2['ratio'];
					//$ratio="M: 12,L: 8,XL: 4,M: 12,L: 8,XL: 4,M: 12,L: 8,XL: 4,M: 12,L: 8,XL: 4,M: 12,L: 8,XL: 4,M: 12,L: 8,XL: 4,XL: 4,M: 12,L: 8,XL: 4,M: 12,L: 8,XL: 4";
					//$ratio=1;
	...
	//}
			...
			for($j=0;$j<sizeof($carton_nodes);$j++)
			{
					  ...
					  for($l=0;$l<sizeof(array_unique($doc_nos));$l++)
					  {
...
$html.= '<tr><td class="new_td3">&nbsp; Style:'.trim($style).'</td><td class="new_td3">Schedule:'.trim($delivery).'</td></tr>';
$html.= '<tr><td colspan=4><font size=2>&nbsp;Color:  '.$node_detail[5].'</font></td></tr>';
$html.= '<tr><td colspan=4><font size=2>&nbsp;Packing Ratio:('.$node_detail[7].')'.'</font></td></tr>';
$html.= '<tr><td colspan=2><font size=2>&nbsp; Jobs: <font size=2>'.implode(",",$job_nos).'</font></td><td >Qty:'.$node_detail[2].'</td></tr>';
