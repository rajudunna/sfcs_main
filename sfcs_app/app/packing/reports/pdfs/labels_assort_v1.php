...
<body>';
 ...
 $sql="select remarks, min(carton_act_qty) as \"packs\", min(tid) as \"tid\", sum(carton_act_qty) as \"carton_act_qty\", doc_no, group_concat(distinct left(trim(order_col_des),7)  ORDER BY tid SEPARATOR \"+\") as \"color_title\",group_concat(trim(carton_act_qty)  ORDER BY tid SEPARATOR \" + \") as \"pcs_title\" from $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=$schedule and size_code=\"".strtolower($size_titles_qry[$i])."\" group by doc_no_ref";
//echo $sql;
			mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row=mysqli_fetch_array($sql_result))
