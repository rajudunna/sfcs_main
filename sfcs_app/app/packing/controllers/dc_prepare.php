		...
		<div class='panel-body'>		
			<form name="input" method="post" action="<?php echo '?r='.$_GET['r']; ?>">
				...
				$plantcode=$_SESSION['plantCode'];
					...
					{
						$sel_value=$_POST['sel'];
						$get_tids = "SELECT ship_tid FROM $pps.ship_stat_log where plant_code='$plantcode' and ship_up_date in ("."'" . implode ( "', '", $sel_value ) . "'".")";
...
if(isset($_POST['submit']))
{
	...
	if($seal!='' && $vehicle!='')
	{
		...
		$sql="update $pps.ship_stat_log set disp_note_no=$iLastID, ship_status=2,updated_user='$username',updated_at='".date('Y-m-d')."' where plant_code='$plantcode' and ship_tid in ($ship_id)";
