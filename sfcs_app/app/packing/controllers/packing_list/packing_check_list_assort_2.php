...
$sql_result=mysqli_query($link, $sql) or exit("Sql Error1".$sql);
...
//echo "query =".$sql."<br>";
mysqli_query($link, $sql) or exit("Sql Error2".$sql);
...
//ERROR CHECK POINT
//echo "<br/>order qty=".array_sum($order_qtys);
//echo "<br/>Carton qty=".$sql_row['carton_act_qty'];
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	//echo "SUM=".array_sum($order_qtys)."-".$sql_row['carton_act_qty'];
	if(array_sum($order_qtys)!=$sql_row['carton_act_qty'])
	{
	//	echo "<br/>order qty".array_sum($order_qtys);
	//	echo "<br/>packing summary".$sql_row['carton_act_qty'];
...
<table border=0 cellpadding=0 cellspacing=0 width=1210 class=xl6619400
 ...
 <tr class=xl7219400 height=22 style='mso-height-source:userset;height:16.5pt'>
  ...
  <td colspan=12 class=xl7019400_new style='border-left:none'><?php echo $color; ?></td>
  <td class=xl7119400></td>
  <td class=xl7119400></td>
...
$max_size_code="";
$count=0;
$sql="select packing_summary.*, sum(carton_act_qty) as \"carton_qty\" from $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=\"$schedule\" and order_col_des=\"$color\" and container is null group by doc_no_ref";
//echo $sql;
mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
while($sql_row=mysqli_fetch_array($sql_result))
...
<table class="new">
	  ...
	  <?php
	  	$total=0;
		$sql1="select container,group_concat(tid) as \"label_id\", count(*) as \"count\" from $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=\"$schedule\" and order_col_des=\"$color\" group by container";
		mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		$sql_result1=mysqli_query($link, $sql1) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		while($sql_row1=mysqli_fetch_array($sql_result1))
		{
	    	$container=$sql_row1['container'];
		$sql="select container,group_concat(label_id) as \"label_id\", count(*) as \"count\" from (select container,min(tid) as \"label_id\", count(*) as \"count\" from $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=\"$schedule\" and order_col_des=\"$color\" and container=$container group by doc_no_ref) as t group by container";
		mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
		while($sql_row=mysqli_fetch_array($sql_result))
		{
