...
<?php
$sql="select * from $bai_pro.bai_orders_db_confirm where order_style_no=\"$style\" and order_del_no=$schedule";
...
<div id="page_heading"><span style="float: left"><h3>FG Packing List</h3></span><span style="float: right"><b>?</b>&nbsp;</span></div>
	...
	for($i=0;$i<sizeof($order_qtys);$i++)
  	{	
		if($order_qtys[$i]>0)
		{
			...
			$carton_nodes=array();
			$x=1;
			$sql="select status,min(tid) as \"tid\",doc_no,sum(carton_act_qty) as \"carton_act_qty\" 
				from $bai_pro3.pac_stat_log where doc_no in (".implode(",",$docs_db).") and size_code=\"".strtolower($size_titles_qry[$i])."\" group by doc_no_ref order by doc_no, carton_mode,carton_act_qty desc";
			//echo $sql;
			mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row=mysqli_fetch_array($sql_result))
