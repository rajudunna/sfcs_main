...
//echo "pilot = ".$pilot_qty."<br>";

$pilot_qty=0; //EXCEPTION

//echo "Pilot = ".$pilot_qty;
...
while($sql_row=mysqli_fetch_array($sql_result))
{
	//echo array_sum($order_qtys)." ".$pilot_qty." ".$sql_row['carton_act_qty'];
...
<?php
 ...
 <?php
	...
	while($sql_row=mysqli_fetch_array($sql_result))
	{
		...
		if(substr($sql_row['remarks'],0,1)=="P")
		{
			//$doc_type[]=substr($sql_row['remarks'],0,1);
			$doc_type[]="<strong>**</strong>";
		}
		else
		{
			$doc_type[]="";
  	...
  	{	
		if($order_qtys[$i]>0)
		{
			...
			while($sql_row=mysqli_fetch_array($sql_result))
			{
				...
				$tid=$sql_row['tid'];
				$carton_nodes[]=$x."-".$doc_type[array_search($doc_no,$docs_db)].chr($color_code).leading_zeros($cutno_db[array_search($doc_no,$docs_db)],3)."-".$carton_act_qty."-".$tid;
				$x++;
			}
 ...
 ?>
 <tr class=xl6553519400 height=20 style='height:15.0pt'>
  <td height=20 class=xl7919400 style='height:15.0pt' colspan=2>** - Pilot Cut</td>
