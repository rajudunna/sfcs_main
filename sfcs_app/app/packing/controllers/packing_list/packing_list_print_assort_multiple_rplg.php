 ...
 // 	{	
	...
	//	{
			...
			$sql="select status,min(tid) as \"tid\",doc_no,sum(carton_act_qty) as \"carton_act_qty\" 
				 from $bai_pro3.pac_stat_log where doc_no in (".implode(",",$docs_db).") group by doc_no_ref order by doc_no, carton_mode,carton_act_qty desc";
			//echo $sql;
			mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			$sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
			while($sql_row=mysqli_fetch_array($sql_result))
...
echo "<h3>Carton Pack Ratio Quantities </h3>";
//$packpcs=array();
//$assort_color=array();
