<head>
	...
	{
		//alert("report");
		window.location.href =url1+"&style="+encodeURIComponent(window.btoa(document.pack_load.style.value))
	}

	function secondbox()
	{
		//alert('test');
		window.location.href =url1+"&style="+encodeURIComponent(window.btoa(document.pack_load.style.value))+"&schedule="+document.pack_load.schedule.value
	}
	...
	else
	{
		...
		<div class="col-md-12">
            ...
            if($_GET['schedule'] && $_GET['seq_no'] && $_GET['option'] && short_shipment_status($_GET['style'],$_GET['schedule'],$link))
			{
				...
				{
					echo '<script>swal("Sewing Job Generated","Delete Sewing Jobs and Try Again","warning")</script>';
				}
			}
			...
			{	
				...
				$check_sewing_job_for_pack = echo_title("$bai_pro3.pac_stat_input","count(*)","schedule",$schedule,$link);
