<!--
kirang/ 8-22-2014 / Change Request# 149 : change the priniting sticker format in packing list generation (reduce the font size for schedule,SRP qty and remove the 'SRP' qty at 'Carton#' row, placed at after 'Packs'. ) to avoid the spliting stickers.
-->
<?php
...
1) New change for SRP qty reference from the standard carton qty interface/db. - Kirang -2013-06-18 - Origin: EdwinR
...
<?php
$sql="select * from bai_orders_db_confirm where order_style_no=\"$style\" and order_del_no=$schedule";
...
{
	$sql="select * from bai_orders_db where order_style_no=\"$style\" and order_del_no=$schedule";
}
mysql_query($sql,$link) or exit("Sql Error".mysql_error());
$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
$sql_num_=mysql_num_rows($sql_result);
...
$sql="select sum(carton_act_qty) as \"carton_act_qty\" from packing_summary where order_del_no=$delivery";
mysql_query($sql,$link) or exit("Sql Error".mysql_error());
$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
while($sql_row=mysql_fetch_array($sql_result))
{
...
$sql="select * from shipment_plan_summ where ssc_code=\"$order_tid\"";
mysql_query($sql,$link_new) or exit("Sql Error".mysql_error());
$sql_result=mysql_query($sql,$link_new) or exit("Sql Error".mysql_error());
while($sql_row=mysql_fetch_array($sql_result))
{
	$cpo=$sql_row['CPO'];
...
<body>';
	...
	$sql="select distinct doc_no_ref from packing_summary where order_style_no=\"$style\" and order_del_no=$schedule";
	mysql_query($sql,$link) or exit("Sql Error".mysql_error());
	$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
	$total_cartons=mysql_num_rows($sql_result);
 ...
 $sql="select remarks, min(carton_act_qty) as \"packs\", min(tid) as \"tid\", sum(carton_act_qty) as \"carton_act_qty\", doc_no, group_concat(distinct trim(order_col_des)  ORDER BY tid SEPARATOR \" + \") as \"color_title\",group_concat(trim(carton_act_qty)  ORDER BY tid SEPARATOR \" + \") as \"pcs_title\" from packing_summary where order_style_no=\"$style\" and order_del_no=$schedule and size_code=\"".strtolower($size_titles_qry[$i])."\" group by doc_no_ref";
//echo $sql;
			mysql_query($sql,$link) or exit("Sql Error".mysql_error());
			$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
			while($sql_row=mysql_fetch_array($sql_result))
			...
			for($j=0;$j<sizeof($carton_nodes);$j++)
			{
					  ...
					  $sql2="select doc_no_ref from packing_summary where tid=\"".$node_detail[3]."\"";
					  $result2=mysql_query($sql2,$link) or die("Error = ".mysql_error());
					  ...
					  {
					  	  $sql3="select color_code,acutno from order_cat_doc_mk_mix where doc_no=\"".$doc_nos[$l]."\" ";
...
$html.= '<tr><td colspan=2>Assort:&nbsp;&nbsp;<font size=2>'.$packing_method." ".$node_detail[1].'('.$node_detail[7].')'.'</font></td><td>Packs:'.substr($node_detail[6],0,2).'</td><td class="new_td3">SRP:'.$string.'</td></tr>';
...
$html.= '<tr><td>CO Number:</td><td>'.$co_no.'</td></tr>';
$html.= '</table></div>';
...
$sql="update bai_orders_db_confirm set carton_print_status=1 where order_del_no=$schedule";
$sql_result=mysql_query($sql,$link) or exit("Sql Error".mysql_error());
