...
echo "data".array_sum($order_qtys)."-".$sql_row['carton_act_qty'];
while($sql_row=mysqli_fetch_array($sql_result))
{
	echo "data".array_sum($order_qtys)."-".$sql_row['carton_act_qty'];	
...
$sql="select * from $pps.shipment_plan_summ where   plant_code='$plantcode' andssc_code=\"$order_tid\"";
...
<body>';
 ...
 $sql="select remarks, min(carton_act_qty) as \"packs\", min(tid) as \"tid\", sum(carton_act_qty) as \"carton_act_qty\", doc_no, group_concat(distinct trim(order_col_des)  ORDER BY tid SEPARATOR \" + \") as \"color_title\",group_concat(trim(carton_act_qty)  ORDER BY tid SEPARATOR \" + \") as \"pcs_title\" from $bai_pro3.packing_summary where order_style_no=\"$style\" and order_del_no=$schedule and order_col_des=\"$color\" and size_code=\"".strtolower($size_titles_qry[$i])."\" group by doc_no_ref";
//echo $sql;
...
$sql="update $bai_pro3.bai_orders_db_confirm set carton_print_status=1 where order_del_no=$schedule and order_col_des=\"$color\"";
