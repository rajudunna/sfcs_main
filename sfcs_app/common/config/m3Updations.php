...
function updateM3TransactionsLay($ref_id,$op_code,$qty,$job_no)
{
    $obj = new rest_api_calls();    
    include($_SERVER['DOCUMENT_ROOT'].'/sfcs_app/common/config/config_ajax.php');
            ...
            if($balance_max_updatable_qty > 0)
            {
                ...
                if(strtolower($is_m3) == 'yes')
                {
                    ...
                    if($enable_api_call == 'YES'){
                        ...
                        $api_url = $host.":".$port."/m3api-rest/execute/PMS070MI/RptOperation?CONO=$company_num&FACI=$plant_code&MFNO=$mo_number&OPNO=$main_ops_code&DPLG=$work_station_id&MAQA=$to_update_qty&REMK=$insert_id&DSP1=1&DSP2=1&DSP3=1&DSP4=1&EMNO=$job_no";
                        $api_data = $obj->getCurlAuthRequest1($api_url,$insert_id);
                        $decoded = json_decode($api_data,true);
                        $type=$decoded['@type'];
                        $code=$decoded['@code'];
