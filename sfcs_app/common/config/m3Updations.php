    ...
    while($nop_qry_row=mysqli_fetch_array($qry_nop_result))
    {
        ...
        if($total_bundle_present_qty > 0)
        {
            ...
            if($balance_max_updatable_qty > 0)
            {
                ...
                VALUES ('$current_date','$mo_number',$to_update_qty,'','Normal','$logged_user','','$b_module','$b_shift',$op_code,'$ops_des',$id,'$work_station_id','$main_ops_code','')";
                ...
                if($enable_api_call == 'YES'){
                    ...
                    if($type!='ServerReturnedNOK'){
                        ...
                        $qry_transactionslog="INSERT INTO $brandix_bts.`transactions_log`(`transaction_id`,`response_message`,`created_by`,`created_at`) VALUES ('$insert_id','$message','$logged_user','$current_date')"; 
        ...
        if($total_bundle_present_qty > 0)
        {
            ...
            {                    
                ...
                $inserting_into_m3_tran_log = "INSERT INTO $bai_pro3.`m3_transactions` (`date_time`,`mo_no`,`quantity`,`reason`,`remarks`,`log_user`,`tran_status_code`,`module_no`,`shift`,`op_code`,`op_des`,`ref_no`,`workstation_id`,`response_status`,`m3_ops_code`) 
                 VALUES ('$current_date','$mo_number','$to_update_qty','','Normal','$logged_user','',$plan_module,'$input_shift',$op_code,'',$id,'$work_station_id','','$main_ops_code')";
                // echo $inserting_into_m3_tran_log;
                mysqli_query($link,$inserting_into_m3_tran_log) or exit("While inserting into m3_tranlog".mysqli_error($GLOBALS["___mysqli_ston"]));
                $insert_id=mysqli_insert_id($link);
        ...
        while($nop_qry_row=mysqli_fetch_array($qry_nop_result))
        {
            ...
            if($total_bundle_present_qty > 0)
            {
                if($balance_max_updatable_qty > 0)
                {
                    ...
                    $inserting_into_m3_tran_log = "INSERT INTO $bai_pro3.`m3_transactions` (`date_time`,`mo_no`,`quantity`,`reason`,`remarks`,`log_user`,`tran_status_code`,`module_no`,`shift`,`op_code`,`op_des`,`ref_no`,`workstation_id`,`response_status`,`m3_ops_code`) 
                    VALUES ('$current_date','$mo_number',$to_update_qty,'$r_reasons[$key]','Normal','$logged_user','',$b_module,'$b_shift',$op_code,'',$id,'$work_station_id','','$main_ops_code')";
                    ...
                    else
                    {
                        ...
                        //insert transactions details into transactions_log
                        $qry_transactionslog="INSERT INTO $brandix_bts.`transactions_log` (`transaction_id`,`response_message`,`created_by`,`created_at`,`updated_at`) VALUES ('$insert_id','$message','$logged_user','$current_date')"; 
                        mysqli_query($link,$qry_transactionslog) or exit("While inserting into M3 transaction log".mysqli_error($GLOBALS["___mysqli_ston"]));
                    }
                }
            }
