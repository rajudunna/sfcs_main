...
function roll_splitting_function($roll_id,$total_roll_qty,$issued_qty,$plant_code,$username)
{
	...
	if($balance_qty>0){
		...
		$sql22="update bai_rm_pj1.store_in set barcode_number='".$facility_code."-".$new_tid."' where tid=".$new_tid." and plant_code='".$plant_code."'";
		mysqli_query($link, $sql22) or exit("Sql Error3: $sql".mysqli_error($GLOBALS["___mysqli_ston"]));
	}
	$sql="update bai_rm_pj1.store_in set qty_rec=".$issued_qty.",qty_allocated=".$issued_qty.",status=2, allotment_status=2 where tid=".$roll_id." and plant_code='".$plant_code."'";
