...
function getOpsWiseJobQtyInfo($schedule, $bundle_types) {
    ...
    while($sql_row=mysqli_fetch_array($sql_result))
    {
        ...
        $size = $sql_row["size"];
        $sql_barcodes = "SELECT GROUP_CONCAT(CONCAT('''', trans.`parent_ext_ref_id`, '''' )) AS parent_ext_ref_id, resource_id FROM pts_prod.`transaction_log` trans 
        ...
        while($sql_row_barcodes = mysqli_fetch_array($sql_result_barcodes))
        {
            ...
            $sql_tms_in = "SELECT operation_code FROM tms_prod.`task_jobs` tj LEFT JOIN tms_prod.`task_job_transaction` trans ON trans.task_jobs_id = tj.task_jobs_id WHERE task_job_reference IN ($jg_header_ids) ORDER BY operation_seq  ASC LIMIT 0,1";
            ...
            $sql_pts_trans = "SELECT sum(good_quantity)as good_qty, sum(rejected_quantity)as rej_qty FROM pts_prod.`transaction_log` trans 
            LEFT JOIN pts_prod.`barcode` barcode  ON barcode.`barcode_id` = trans.`barcode_id` 
            ...
            $sql_pts_trans_out = "SELECT sum(good_quantity)as good_qty, sum(rejected_quantity)as rej_qty FROM pts_prod.`transaction_log` trans 
            LEFT JOIN pts_prod.`barcode` barcode  ON barcode.`barcode_id` = trans.`barcode_id` 
            WHERE barcode.`external_ref_id` IN ($aplbIds) AND barcode.`barcode_type` = 'APLB' 
