...
function getBulkSchedules($get_style,$plantcode){
    ...
    $qry_mp_color_detail="SELECT master_po_details_id FROM $pps.mp_color_detail WHERE plant_code='$plantcode' AND style='$get_style' AND is_active=1";
    $mp_color_detail_result=mysqli_query($link_new, $qry_mp_color_detail) or exit("Sql Error at mp_color_detail".mysqli_error($GLOBALS["___mysqli_ston"]));
    $mp_color_detail_num=mysqli_num_rows($mp_color_detail_result);
    if($mp_color_detail_num>0){
        while($mp_color_detail_row=mysqli_fetch_array($mp_color_detail_result))
...
function getBulkColors($get_schedule,$plantcode){
    ...
    global $pps;
    $color=array();
    $qry_mp_mo_qty="SELECT color FROM $pps.mp_mo_qty WHERE plant_code='$plantcode' AND schedule='$get_schedule' AND is_active=1";
...
function getMpos($get_schedule,$get_color,$plantcode){
    ...
    global $pps;
    $master_po_details_id=array();
    $qry_mmp_mo_qty="SELECT master_po_details_id FROM $pps.`mp_mo_qty` WHERE plant_code='$plantcode' AND color='$get_color' AND schedule='$get_schedule' AND is_active=1";
...
function getMpSchedulewise($get_schedule,$plantcode){
    ...
    global $pps;
    $master_po_details_id=array();
    $qry_mmp_mo_qty="SELECT master_po_details_id FROM $pps.`mp_mo_qty` WHERE plant_code='$plantcode' AND schedule='$get_schedule' AND is_active=1";
    $mp_mo_qty_result=mysqli_query($link_new, $qry_mmp_mo_qty) or exit("Sql Error at mp_color_detail".mysqli_error($GLOBALS["___mysqli_ston"]));
    $mp_mo_qty_num=mysqli_num_rows($mp_mo_qty_result);
    /**From above query we get master po details id */
    if($mp_mo_qty_num>0){
    ...
    $qry_toget_podescri="SELECT master_po_description,master_po_number,mpo_serial FROM $pps.mp_order WHERE master_po_number IN ('".implode("','" , $master_po_number)."') AND plant_code='$plantcode' AND is_active=1";
    $toget_podescri_result=mysqli_query($link_new, $qry_toget_podescri) or exit("Sql Error at mp_order".mysqli_error($GLOBALS["___mysqli_ston"]));
    $toget_podescri_num=mysqli_num_rows($toget_podescri_result);
    if($mp_color_details_num>0){
        while($toget_podescri_row=mysqli_fetch_array($toget_podescri_result))
...
function getJobsStatus($sub_po,$tasktype,$plantcode){
    ...
    $check_job_status="SELECT task_status FROM $tms.task_header WHERE task_ref in ('".implode("','" , $jm_job_header_id)."') AND plant_code='$plantcode' AND task_type='$tasktype' AND (resource_id IS NULL OR  resource_id='') AND is_active=1";
    ...
    try
    {
        ...
        for($i=0;$i<sizeof($list_db);$i++)
        {
            ...
            {  
                ...
                $qryAllHeadersModuleWise="SELECT task_header_id FROM $tms.task_header WHERE resource_id='$items[0]' AND plant_code='$plantcode' AND is_active=1";
                ...
                $qryMaxprorityJob="SELECT MAX(priority) as lastPriority FROM $tms.task_jobs WHERE task_header_id IN ('$headers') AND plant_code='$plantcode' AND is_active=1";
                ...
                if(is_null($resource_id)){
                    ...
                    if($tasktype == $check_type)
                    {
                        $get_task_job_id="SELECT task_jobs_id FROM $tms.task_jobs WHERE task_header_id='$header_id' AND task_type='$tasktype' AND plant_code='$plantcode' AND is_active=1";
                    ...
                    if($tasktype == $check_type)
                    {
                        $get_task_job_id="SELECT task_jobs_id FROM $tms.task_jobs WHERE task_header_id='$uuid' AND task_type='$tasktype' AND plant_code='$plantcode' AND is_active=1";
                        $get_task_job_id_result=mysqli_query($link_new, $get_task_job_id) or exit("Sql Error at get_task_job_id".mysqli_error($GLOBALS["___mysqli_ston"]));
                        while($job_id_row=mysqli_fetch_array($get_task_job_id_result))
                        {
                           $task_id= $job_id_row['task_jobs_id'];
                ...
                /**getting dependency work stations for current work station*/
                if($items[0]!="allItems"){
                    $workStationId=getDependencyWorkstation($items[0],$plantcode);
                    /**if dependecny workstation exists only we can update*/
                    if($workStationId!=0){
                        /**getting docket from attribute */
                        //TO GET STYLE AND COLOR FROM TASK ATTRIBUTES USING TASK HEADER ID
                        $job_detail_attributes = [];
                        $qry_toget_style_sch = "SELECT attribute_name,attribute_value FROM $tms.task_attributes where task_jobs_id ='$task_jobs_id' and plant_code='$plantcode' and is_active=1";
                        $qry_toget_style_sch_result = mysqli_query($link_new, $qry_toget_style_sch) or exit("Sql Error at task attributes" . mysqli_error($GLOBALS["___mysqli_ston"]));
                        ...
                        $docketNumber = $job_detail_attributes['DOCKETNO'];
                        if($refDocket!=$docketNumber){
                            ...
                            if (mysqli_num_rows($resultDocketLineIds) > 0) {
                                ...
                                if (mysqli_num_rows($resulttaskjobs) > 0) {
                                    while ($taskJobid = mysqli_fetch_array($resulttaskjobs)) {
                                        $task_header_id = $taskJobid['task_header_id'];
                                    }
                                    $j=1;
                                    if($task_header_id!=''){
                                ...
                                if($tasktype == $check_type)
                                {
                                    ...
                                    if (mysqli_num_rows($resultGetCutJob) > 0) {
                                        ...
                                        {
                                            $jm_cut_job_id = $cutJobRow['jm_cut_job_id'];
                                        }
                                    }
                                    
                                    if($jm_cut_job_id!=''){
                                        /**getting jm jg header id wrt cut jobs*/
                                        $qrygetJmJgHeader="SELECT jm_jg_header_id AS jmjgheader  FROM $pps.jm_job_header JOIN $pps.jm_jg_header ON jm_jg_header.jm_job_header=jm_job_header.jm_job_header_id WHERE jm_job_header.ref_id='$jm_cut_job_id' AND jm_job_header.plant_code='$plantcode' AND jm_job_header.is_active=1";
                                        $resultJmJgHeaderID = mysqli_query($link_new,$qrygetJmJgHeader) or die(exception($qrygetJmJgHeader));
                                        if (mysqli_num_rows($resultJmJgHeaderID) > 0) {
                                            $j=1;
                                            while($cutJobRow=mysqli_fetch_array($resultGetCutJob)) 
                                            {
                                                $jmjgheader = $cutJobRow['jmjgheader'];
                                            
                                                /**Update qry for priority */
                                                $qryUpdateJobsPriority="UPDATE $tms.task_jobs SET priority=$j,updated_at=NOW() WHERE task_job_reference='$jmjgheader' AND task_type='$cutEmbJobType' AND plant_code='$plantcode'";
                                                $UpdateJobsPriority_result=mysqli_query($link_new, $qryUpdateJobsPriority) or exit("Sql Error at TaskjObs priority Update".mysqli_error($GLOBALS["___mysqli_ston"]));
                                                $j++;
                                            }
                                            ...
                                            if (mysqli_num_rows($resultTaskHeaderId) > 0) {
                                                ...
                                                {
                                                    $task_header_id = $taskHeaderRow['task_header_id'];
                                                }
                                                /**update query task header table to update workstation */
                                                /**resource id update */
                                                $j=1;
...
function getUnplannedJobs($sub_po,$tasktype,$plantcode){
   ...
   // echo $qry_toget_taskrefrence;
    ...
    $qry_toget_sewing_jobs="SELECT job_number,jm_jg_header_id FROM $pps.jm_jg_header WHERE job_group_type='$job_group_type' AND plant_code='$plantcode' AND jm_jg_header_id  IN('".implode("','" , $task_refrence)."') AND is_active=1 order by job_number";
...
function getCutNumber($jm_cut_job_id){
    global $link_new;
    global $pps;
    $QryCutNumbers="SELECT cut_number FROM $pps.jm_cut_job WHERE jm_cut_job_id='$jm_cut_job_id' AND is_active=1";
...
/** function to get dependency workstations
   * @param:workstaion_id
   * @return:dependency
   * */

function getDependencyWorkstation($workstation_id,$plantcode){
    global $link_new;
    global $pms;
    $qryWorkstationDependency = "SELECT destination_workstation FROM $pms.workstation_dependency WHERE source_workstation='$workstation_id' AND plant_code='$plantcode' AND is_active=1";
    $resultDependencyWorkstation = mysqli_query($link_new, $qryWorkstationDependency) or exit("Sql Error at task_header_id" . mysqli_error($GLOBALS["___mysqli_ston"]));
    $dependencyNum = mysqli_num_rows($resultDependencyWorkstation);
    if ($dependencyNum > 0) {
        while ($dependencyRow = mysqli_fetch_array($resultDependencyWorkstation)) {
            $dependencyWorkstation = $dependencyRow['destination_workstation'];
        }
        return $dependencyWorkstation;
    }else{
        return 0;
    }
}
