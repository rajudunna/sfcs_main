<?php
	...
	function deleteMOQuantitiesCut($schedule,$color){
		...
		mysqli_close($link);
		//echo "<span class='alert alert-warning'>LayPlan Deleted.<br/>Problem Encounterd while removing records from mo_operation_quantites.<span>";
		// -----Transaction End ---------
		return false;
		//die();
	}
	...
	function insertMOQuantitiesCut($ref_id,$op_code,$qty){
		...
		if(sizeof($mos) == 1){
			...
			foreach($mos as $mo=>$mo_qty){
				...
				//echo "$mo and its capcity = $mo_qty and is filled $filled_qty and available = $available<br/>";		 
    ...
    function insertMOQuantitiesPacking($schedule,$pack_ref){
        ...
        $packing_cat = 'PACKING';
		$op_codes_query = "SELECT category,group_concat(operation_code) as codes FROM $brandix_bts.tbl_orders_ops_ref 
                           WHERE default_operation='Yes' and category = '$packing_cat' group by category";
        $op_codes_result = mysqli_query($link,$op_codes_query) or exit('Problem in getting the op codes for sewing');	
        while($row = mysqli_fetch_array($op_codes_result)){
            $op_codes = $row['codes'];
        }	
	...
	*/
	...
	function doc_size_wise_bundle_insertion($doc_no_ref)
	{
		...
		foreach($category as $key => $value)
		{
			...
			//echo $fetching_ops_with_category.'<br/>';
