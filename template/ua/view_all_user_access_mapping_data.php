...
<?php
    ...
    include('../dbconf.php');
    session_start();
    if(isset($_SESSION["msg"]) && $_GET['role']) {
        $msg = $_SESSION["msg"];
...
<div class="panel panel-primary">
    ...
    <div class="panel-body">
        <form name="test" action="<?php echo getFullURLLevel($_GET['r'],'view_all_user_access_mapping_data.php','0','N'); ?>" method="post">
        <?php
        $sql="select * from $central_administration_sfcs.rbac_roles";	
        $sql_result=mysqli_query($link, $sql) or exit("Sql Error".mysqli_error($GLOBALS["___mysqli_ston"]));
        echo "<div class=\"row\"><div class=\"col-sm-3\"><label>Select Role:</label><select class='form-control' name=\"role\"  id=\"role\" id='role'>";
        echo "<option value='' disabled selected>Please Select</option>";
        while($sql_row=mysqli_fetch_array($sql_result))
        {
        
            if(str_replace(" ","",$sql_row['role_id'])==str_replace(" ","",$role))
            {
                echo "<option value=\"".$sql_row['role_id']."\" selected>".$sql_row['role_name']."</option>";
            }
            else
            {
                echo "<option value=\"".$sql_row['role_id']."\">".$sql_row['role_name']."</option>";
            }
        }
        echo "  </select></div><br/>
...
?>
    <div class="panel panel-primary App3" ng-app="brandix" id="" ng-init="mapping_data = <?= htmlspecialchars(json_encode($RolsMenusPermissionsMappingData)) ?> " ng-cloak>
        <div class="panel-heading" ng-init="rbac_permissions=<?= htmlspecialchars(json_encode($rbac_permissions)) ?>"></div>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    ...
                                    <input type="hidden" name='role' class="form-control" value=<?=$role;?> readonly/>
                                </div>
                                <div class="col-md-5">
