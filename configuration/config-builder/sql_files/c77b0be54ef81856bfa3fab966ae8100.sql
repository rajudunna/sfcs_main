...
MySQL - 5.5.5-10.1.28-MariaDB : Database - central_administration_sfcs
*********************************************************************
*/
...
/*Database structure for database `central_administration_sfcs` */
...
/*!50003 CREATE DEFINER=`baiall`@`%` FUNCTION `fun_emp_mod`(`id` VARCHAR(100)) RETURNS varchar(200) CHARSET latin1
    DETERMINISTIC
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `GetFunctionDetails`()
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `list_function`()
BEGIN
DECLARE v_fn_id INT;
...
/*!50003 CREATE DEFINER=`baiall`@`%` PROCEDURE `test1`()
begin
declare done int default false;
