USE bai_rm_pj1;
DROP VIEW IF EXISTS bai_rm_pj1.fabric_status_v3;

CREATE VIEW bai_rm_pj1.fabric_status_v3 AS (
SELECT
  ...
  `store_in`.`roll_status`         AS `roll_remarks`,
  ...
  ROUND(SUM(ROUND(ROUND(`store_in`.`qty_rec`,2) - ROUND(`store_in`.`partial_appr_qty`,2) - ROUND(`store_in`.`qty_issued`,2) + ROUND(`store_in`.`qty_ret`,2),2)),2) AS `balance`
FROM (`store_in`
   JOIN `sticker_report`)
