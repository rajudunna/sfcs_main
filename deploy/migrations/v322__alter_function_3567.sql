...
DROP FUNCTION IF EXISTS `input_job_input_status`;

CREATE FUNCTION `input_job_input_status`(in_job_rand_no_ref VARCHAR(30),operation_code INT(11)) RETURNS VARCHAR(10) CHARSET latin1
    DETERMINISTIC
BEGIN
	...
	IF((@rec_qty >= @job_qty) AND (@s_qty+@rc_qty+@rp_qty=@rec_qty+@rej_qty)) THEN
		RETURN 'DONE';
	ELSEIF((@rej_qty = @job_qty) AND (@s_qty+@rc_qty+@rp_qty=@rec_qty+@rej_qty)) THEN
		RETURN 'DONE';
	ELSEIF((@rej_qty + @rec_qty = @job_qty) AND (@s_qty+@rc_qty+@rp_qty=@rec_qty+@rej_qty)) THEN
		RETURN 'DONE';
	ELSE
		RETURN '';
	END IF;
