...
$sql2="Alter table `bai_pro3`.`ims_log`   
  change `input_job_no_ref` `input_job_no_ref` varchar(25) NOT NULL  COMMENT 'reference of input job number'";
...
$sql3="Alter table `bai_pro3`.`ims_log_backup`   
  change `input_job_no_ref` `input_job_no_ref` varchar(25) NOT NULL";
...
$sql4="Alter table `bai_pro`.`bai_log`   
  change `jobno` `jobno` varchar(25) NOT NULL";
...
$sql5="Alter table `brandix_bts`.`bundle_creation_data`   
  change `input_job_no` `input_job_no` varchar(25) NULL";
...
while($sql_row11=mysqli_fetch_array($sql_result11))
{
	...
	$sql16="update bai_pro.bai_log set jobno='$ijno' where SUBSTRING_INDEX(ims_pro_ref, '-', 1)=$pac_tid";
	$sql_result16=mysqli_query($link, $sql16) or exit("Sql Error16--".mysqli_error($GLOBALS["___mysqli_ston"]));
	
	$sql17="update bai_pro.bai_log_buf set jobno='$ijno' where SUBSTRING_INDEX(ims_pro_ref, '-', 1)=$pac_tid";
	$sql_result17=mysqli_query($link, $sql17) or exit("Sql Error17--".mysqli_error($GLOBALS["___mysqli_ston"]));
