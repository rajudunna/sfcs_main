...
while($row1 = mysqli_fetch_array($style_result))
{
	...
	$moq_query = "SELECT mo_no,sum(if(op_code=$sewing_last_opn,good_quantity,0)) as output,sum(if(op_code=200,good_quantity,0)) as carton from bai_pro3.mo_operation_quantites where mo_no in ($mo_no_values) group by mo_no";
	...
	while($row = mysqli_fetch_array($moq_result))
	{
		...
		if($output > 0)
		{
			...
			if($qty_inserting)
			{
				echo "</br><h4 style='color:blue;'>successfully updated : </h4>".$insert_qry;
			}
			else
			{
				echo "</br><h4 style='color:red;'>Failed to updated : </h4>".$insert_qry;
			}	
		}
